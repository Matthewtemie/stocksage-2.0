...
def check_typed_dict(
    ...
    if extra_keys := existing_keys - declared_keys:
        if (argtype := getattr(origin_type, "__extra_items__", NoExtraItems)) is NoExtraItems:
...
def check_signature_compatible(subject: type, protocol: type, attrname: str) -> None:
    ...
    if protocol_type == "instance" and subject_type != "instance":
        raise TypeCheckError(f"should be an instance method but it's a {subject_type} method")
    elif protocol_type != "instance" and subject_type == "instance":
        raise TypeCheckError(f"should be a {protocol_type} method but it's an instance method")
    ...
    has_varargs = any(
        param for param in subject_sig.parameters.values() if param.kind is Parameter.VAR_POSITIONAL
    )
    ...
    protocol_has_varkwargs = any(
        param for param in protocol_sig.parameters.values() if param.kind is Parameter.VAR_KEYWORD
    )
    subject_has_varkwargs = any(
        param for param in subject_sig.parameters.values() if param.kind is Parameter.VAR_KEYWORD
    )
    ...
    if not has_varargs:
        ...
        for protocol_arg, subject_arg in zip_longest(protocol_args, subject_args):
            ...
            if (
                ...
                raise TypeCheckError(
                    f"has an argument ({subject_arg.name}) that should not be " f"positional-only"
                )
...
origin_type_checkers: dict[Any, Callable[[Any, Any, tuple[Any, ...], TypeCheckMemo], None]] = {
