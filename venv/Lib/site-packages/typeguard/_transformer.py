...
class TypeguardTransformer(NodeTransformer):
    ...
    def visit_FunctionDef(
        ...
        with self._use_memo(node):
            ...
            if (
                ...
                func_name = self._get_import("typeguard._functions", "check_return_type_internal")
                return_node = Return(
                    Call(
                        func_name,
                        [
                ...
                if self._memo.parent and isinstance(self._memo.parent.node, ClassDef):
                    for decorator in node.decorator_list:
                        if isinstance(decorator, Name) and decorator.id == "staticmethod":
                            ...
                            memo_kwargs["self_type"] = Name(id=arglist[0].arg, ctx=Load())
                            break
                    else:
    ...
    def visit_Assign(self, node: Assign) -> Any:
        ...
        if isinstance(self._memo.node, (FunctionDef, AsyncFunctionDef)):
            preliminary_targets: list[PreliminaryNameTypePair | list[PreliminaryNameTypePair]] = []
            ...
            for node_target in node.targets:
                ...
                preliminary_targets.append(annotations_[0] if single_target else annotations_)
                ...
                for items in preliminary_targets:
                    if isinstance(items, list):
                        ...
                        for key, expression_or_none in items:
                            expression = expression_or_none or self._get_import("typing", "Any")
