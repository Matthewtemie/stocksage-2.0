...
class DirtyArbiter:
    ...
    SIGNALS = [
        getattr(signal, "SIG%s" % x) for x in "HUP QUIT INT TERM TTIN TTOU USR1 USR2 CHLD".split()
    ]
        ...
        for spec in self.cfg.dirty_apps:
            ...
            if worker_count is None:
                try:
                    ...
                    self.log.warning("Could not read workers attribute from %s: %s", import_path, e)
    ...
    def _signal_handler(self, sig, frame):
        ...
        if sig == signal.SIGCHLD:
            ...
            if self._loop:
                self._loop.call_soon_threadsafe(lambda: asyncio.create_task(self._handle_sigchld()))
            return
        ...
        if sig == signal.SIGHUP:
            # Reload workers
            if self._loop:
                self._loop.call_soon_threadsafe(lambda: asyncio.create_task(self.reload()))
            return
        ...
        if sig == signal.SIGTTIN:
            ...
            if self._loop:
                self._loop.call_soon_threadsafe(lambda: asyncio.create_task(self.manage_workers()))
            return
        ...
        if sig == signal.SIGTTOU:
            ...
            if self.num_workers <= min_workers:
                self.log.warning(
                    "SIGTTOU: Cannot decrease below %s workers " "(required by app specs)",
                    min_workers,
                )
                return
            self.num_workers -= 1
    ...
    async def route_request(self, request, client_writer):
        ...
        try:
            ...
            response = make_error_response(
                request_id, DirtyWorkerError(f"Request failed: {e}", worker_id=worker_pid)
            )
            await DirtyProtocol.write_message_async(client_writer, response)
        ...
        async def consumer():
            while self.alive:
                try:
                    ...
                    try:
                        await self._execute_on_worker(worker_pid, request, client_writer)
                        if not future.done():
    ...
    async def _execute_on_worker(self, worker_pid, request, client_writer):
        ...
        try:
            ...
            while True:
                try:
                    message = await asyncio.wait_for(
                        DirtyProtocol.read_message_async(reader), timeout=self.cfg.dirty_timeout
                    )
                except asyncio.TimeoutError:
                    response = make_error_response(
                        request_id, DirtyTimeoutError("Worker timeout", self.cfg.dirty_timeout)
                    )
                    await DirtyProtocol.write_message_async(client_writer, response)
                    return
                ...
                if msg_type in (DirtyProtocol.MSG_TYPE_RESPONSE, DirtyProtocol.MSG_TYPE_ERROR):
                    await DirtyProtocol.write_message_async(client_writer, message)
                    return
            ...
            response = make_error_response(
                request_id,
                DirtyWorkerError(f"Worker communication failed: {e}", worker_id=worker_pid),
            )
            await DirtyProtocol.write_message_async(client_writer, response)
    ...
    def spawn_worker(self, force_all_apps=False):
        ...
        if pid != 0:
            ...
            self.log.info("Spawned dirty worker (pid: %s) with apps: %s", pid, app_paths)
            return pid
        ...
        try:
            while True:
                ...
                if os.WIFEXITED(status):
                    ...
                    self.log.warning("Dirty worker (pid:%s) killed by signal %s", wpid, sig)
