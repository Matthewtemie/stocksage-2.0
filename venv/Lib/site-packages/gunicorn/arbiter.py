...
class Arbiter:
    ...
    SIGNALS = [
        getattr(signal, "SIG%s" % x) for x in "HUP QUIT INT TERM TTIN TTOU USR1 USR2 WINCH".split()
    ]
    SIG_NAMES = dict(
    ...
    def stop(self, graceful=True):
        ...
        """
        unlink = (
            self.reexec_pid == self.master_pid == 0 and not self.systemd and not self.cfg.reuse_port
        )
        ...
        if self.cfg.enable_backlog_metric:
            ...
            if backlog >= 0:
                self.log.debug(
                    ...
                    extra={"metric": "gunicorn.backlog", "value": backlog, "mtype": "histogram"},
                )
    ...
    def spawn_dirty_arbiter(self):
        ...
        self.dirty_arbiter = DirtyArbiter(self.cfg, self.log, pidfile=self.dirty_pidfile)
    ...
    def reap_dirty_arbiter(self):
        ...
        try:
            ...
            if os.WIFEXITED(status):
                exitcode = os.WEXITSTATUS(status)
                if exitcode != 0:
                    self.log.error("Dirty arbiter (pid:%s) exited with code %s", wpid, exitcode)
                else:
                    self.log.info("Dirty arbiter (pid:%s) exited", wpid)
            elif os.WIFSIGNALED(status):
                sig = os.WTERMSIG(status)
                self.log.warning("Dirty arbiter (pid:%s) killed by signal %s", wpid, sig)
