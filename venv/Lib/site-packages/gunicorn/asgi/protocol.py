...
class ASGIProtocol(asyncio.Protocol):
    ...
    def _check_h1c_protocol_available(cls):
        ...
        if cls._h1c_available is None:
            try:
                ...
                cls._h1c_limit_request_line = getattr(gunicorn_h1c, "LimitRequestLine", None)
                cls._h1c_limit_request_headers = getattr(gunicorn_h1c, "LimitRequestHeaders", None)
        ...
        if parser_setting == "python":
            ...
            if incompatible:
                raise RuntimeError(
                    ...
                    "Use http_parser='python' or disable these flags." % ", ".join(incompatible)
                )
    ...
    async def _handle_connection(self):
        ...
        try:
            ...
            while not self._closed:
                ...
                # Handle HTTP request
                keepalive = await self._handle_http_request(request, sockname, effective_peer)
    ...
    def _response_omits_body(method, status):
        ...
        """
        return method == "HEAD" or status in (204, 304) or 100 <= status < 200
