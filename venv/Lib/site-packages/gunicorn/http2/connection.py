...
class HTTP2ServerConnection:
    ...
    def _handle_priority_updated(self, event):
        ...
        if stream is not None:
            stream.update_priority(
                weight=event.weight, depends_on=event.depends_on, exclusive=event.exclusive
            )
    ...
    def __repr__(self):
        return f"<HTTP2ServerConnection " f"streams={len(self.streams)} " f"closed={self._closed}>"
