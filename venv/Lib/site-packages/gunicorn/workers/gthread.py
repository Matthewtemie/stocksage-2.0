...
class ThreadWorker(base.Worker):
    ...
    def enqueue_req(self, conn):
        ...
        fs.add_done_callback(lambda fut: self.method_queue.defer(self.finish_request, conn, fut))
    ...
    def run(self):
        ...
        self.poller.register(
            self.method_queue.fileno(), selectors.EVENT_READ, self.method_queue.run_callbacks
        )
    ...
    def finish_request(self, conn, fs):
        ...
        try:
            ...
            if result is _DEFER and self.alive:
                ...
                self.poller.register(
                    conn.sock, selectors.EVENT_READ, partial(self.on_pending_socket_readable, conn)
                )
                ...
                self.poller.register(
                    conn.sock, selectors.EVENT_READ, partial(self.on_client_socket_readable, conn)
                )
            else:
                self.nr_conns -= 1
    ...
    def handle_http2_request(self, req, conn, h2_conn):
        ...
        try:
            ...
            if pending_trailers:
                ...
                h2_conn.send_response(stream_id, resp.status_code, resp.headers, response_body)
