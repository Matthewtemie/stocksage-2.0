        ...
        *,
        ...
        def try_claim_writer() -> bool:
            with self._locks.state:
                _break_stale_marker(
                    self._paths.write, stale_threshold=self.stale_threshold, now=time.time()
                )
...
def _read_marker(
...
) -> tuple[_MarkerInfo | None, float] | None:
    ...
    try:
        fd = (
            os.open(name, flags, dir_fd=dir_fd)
            if _SUPPORTS_DIR_FD and dir_fd is not None
            else os.open(name, flags)
        )
    except OSError:
        return None
    try:
