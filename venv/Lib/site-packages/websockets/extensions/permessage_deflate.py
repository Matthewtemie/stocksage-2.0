...
class ClientPerMessageDeflateFactory(ClientExtensionFactory):
    ...
    def __init__(
        ...
        if compress_settings is not None and "wbits" in compress_settings:
            raise ValueError(
                "compress_settings must not include wbits, " "set client_max_window_bits instead"
            )
...
class ServerPerMessageDeflateFactory(ServerExtensionFactory):
    ...
    def __init__(
        ...
        if compress_settings is not None and "wbits" in compress_settings:
            raise ValueError(
                "compress_settings must not include wbits, " "set server_max_window_bits instead"
            )
...
def enable_server_permessage_deflate(
    ...
    if extensions is None:
        extensions = []
    if not any(
        ext_factory.name == ServerPerMessageDeflateFactory.name for ext_factory in extensions
    ):
        extensions = list(extensions) + [
            ServerPerMessageDeflateFactory(
