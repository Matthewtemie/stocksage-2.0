    ...
    * Other keyword arguments are used only for connecting to the proxy.
            ...
            if proxy_parsed.scheme[:5] == "socks":
                ...
                if proxy_parsed.scheme == "https":
                    ...
                    if proxy_kwargs.get("ssl") is None:
                        raise ValueError("proxy_ssl=None is incompatible with an https:// proxy")
                else:
                ...
                if ssl is not None:
                    new_transport = await loop.start_tls(transport, connection, ssl, **kwargs)
    ...
    def process_redirect(self, exc: Exception) -> Exception | str:
        ...
        if self.connection_kwargs.get("sock") is not None:
            return ValueError(f"cannot follow redirect to {new_uri} with a preexisting socket")
        ...
        if (
            ...
            if self.connection_kwargs.get("unix", False):
                return ValueError(
                    f"cannot follow cross-origin redirect to {new_uri} " f"with a Unix socket"
                )
