...
def register_store_backend(backend_name, backend):
    ...
    if backend is None or not issubclass(backend, StoreBackendBase):
        raise ValueError(
            "Store backend should inherit StoreBackendBase, '{0}' given.".format(backend)
        )
...
class MemorizedFunc(Logger):
    ...
    def _is_in_cache_and_valid(self, call_id):
        ...
        if self.cache_validation_callback is not None and not self.cache_validation_callback(
            metadata
        ):
    ...
    def _cached_call(self, args, kwargs, shelving):
        ...
        if self._verbose > 10:
            self.warn(
                f"Computing func {func_name}, argument hash {args_id} " f"in location {location}"
            )
    ...
    def _check_previous_func_code(self, stacklevel=2):
        ...
        if old_first_line == first_line == -1 or func_name == "<lambda>":
            ...
            warnings.warn(
                JobLibCollisionWarning(
                    "Cannot detect name collisions for function '{0}'".format(func_description)
                ),
        ...
        if self._verbose > 10:
            _, func_name = get_func_name(self.func, resolv_alias=False)
            self.warn(
                "Function {0} (identified by {1}) has changed.".format(func_name, self.func_id)
            )
...
class Memory(Logger):
    ...
    def cache(
        ...
        if self.store_backend is None:
            cls = AsyncNotMemorizedFunc if inspect.iscoroutinefunction(func) else NotMemorizedFunc
            return cls(func)
        if verbose is None:
            verbose = self._verbose
