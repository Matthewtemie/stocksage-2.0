...
def _enable_faulthandler_if_needed():
    if "PYTHONFAULTHANDLER" in os.environ:
        ...
        if faulthandler.is_enabled():
            ...
            mp.util.debug("Enabling faulthandler to report tracebacks on worker crashes.")
            faulthandler.enable()
...
class _ExecutorManagerThread(threading.Thread):
            ...
            for _ in range(n_children_to_stop - n_sentinels_sent):
                try:
                    ...
                    if cooldown_time > 5.0:
                        ...
                        raise e
                    mp.util.info(
                        "full call_queue prevented to send all sentinels at " "once, waiting..."
                    )
                    sleep(cooldown_time)
                    cooldown_time *= 1.2
                    break
    ...
    def join_executor_internals(self):
        ...
        with self.processes_management_lock:
            ...
            mp.util.debug(
                "executor management thread clean shutdown of " f"{n_joined_processes} workers"
            )
