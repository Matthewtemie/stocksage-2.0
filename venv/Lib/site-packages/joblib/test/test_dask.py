        ...
        with Client(s["address"], loop=loop) as client:  # noqa: F841
            ...
            results_native = [
                client.submit(
                    ...
                    **dict(
                        (key, scattered.get(id(value), value)) for (key, value) in kwargs.items()
                    ),
                    key=str(uuid4()),
                ).result()
    ...
    with cluster() as (s, [a, b]):
        with Client(s["address"], loop=loop_in_thread) as client:
            with parallel_config(backend="dask"):
                ...
                Parallel()(
                    delayed(noop)(data, data, i, opt=data) for i, data in enumerate(data_to_process)
                )
    ...
    with cluster() as (s, [a, b]):
        with Client(s["address"], loop=loop) as _:
            with parallel_config(backend="dask"):
                ...
                _ = Parallel()(
                    delayed(outer_function_joblib)(my_array[i:], i) for i in range(NUM_OUTER_TASKS)
                )
