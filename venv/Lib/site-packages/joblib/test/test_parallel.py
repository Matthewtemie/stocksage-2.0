...
def test_main_thread_renamed_no_warning(backend, monkeypatch):
    ...
    warninfo = [
        w
        for w in warninfo
        if "worker timeout" not in str(w.message) and not isinstance(w.message, DeprecationWarning)
    ]
...
def test_nested_parallel_warnings(parent_backend, child_backend, expected):
    ...
    Parallel(n_jobs=2, backend=parent_backend)(
        delayed(_assert_warning_nested)(backend=child_backend, inner_n_jobs=1, expected=False)
        for _ in range(5)
    )

    #  warnings if inner_n_jobs != 1 and expected
    res = Parallel(n_jobs=2, backend=parent_backend)(
        delayed(_assert_warning_nested)(backend=child_backend, inner_n_jobs=2, expected=expected)
        for _ in range(5)
    )
...
def test_backend_hinting_always_running(n_jobs, prefer):
    ...
    results = Parallel(n_jobs=n_jobs, prefer=prefer)(delayed(square)(i) for i in range(10))
...
def test_initializer_reused(n_jobs):
    ...
    for i in range(n_repetitions):
        results = Parallel(
            ...
            initializer=_set_initialized,
            initargs=(status,),
        )(delayed(_check_status)(status, n_jobs, wait_workers=True) for i in range(n_jobs))
        pids = pids.union(set(results))
    assert len(pids) == n_jobs, "The workers should be reused when the initializer is the same"
