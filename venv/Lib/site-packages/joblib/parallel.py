        ...
        if isinstance(backend, str):
            ...
            if backend not in BACKENDS:
                if backend in EXTERNAL_BACKENDS:
                    ...
                    raise ValueError(
                        f"Invalid backend: {backend}, expected one of " f"{sorted(BACKENDS.keys())}"
                    )
    ...
    def __init__(
        ...
        **backend_kwargs,
        ...
        if isinstance(self._backend_kwargs["max_nbytes"], str):
            self._backend_kwargs["max_nbytes"] = memstr_to_bytes(self._backend_kwargs["max_nbytes"])
        ...
        if backend is default_parallel_config["backend"] or backend is None:
            ...
            try:
                ...
                raise ValueError(
                    "Invalid backend: %s, expected one of %r" % (backend, sorted(BACKENDS.keys()))
                ) from e
        ...
        if not isinstance(backend, SequentialBackend):
            if self.return_generator and not backend.supports_return_generator:
                raise ValueError(
                    "Backend {} does not support return_as={}".format(backend, return_as)
                )
    ...
    def print_progress(self):
        ...
        # tasks.
            ...
            if self.n_tasks is not None:
                self._print(f"Done {index:{width}d} out of {self.n_tasks:{width}d} {fmt_time}")
            else:
            ...
            if index != 0:
                ...
                if is_last_item or cursor % frequency:
                    return
            remaining_time = (elapsed_time / max(index, 1)) * (self.n_dispatched_tasks - index)
