...
def trim_zeros(filt, trim="fb", axis=None):
    ...
    sl = tuple(
        slice(start[ax], stop[ax]) if ax in axis_tuple else slice(None) for ax in range(filt_.ndim)
    )
    if len(sl) == 1:
...
def _percentile_dispatcher(
    a, q, axis=None, out=None, overwrite_input=None, method=None, keepdims=None, *, weights=None
):
...
def percentile(
    ...
    *,
    ...
    return _quantile_unchecked(a, q, axis, out, overwrite_input, method, keepdims, weights, weak_q)
