...
class build_src(build_ext.build_ext):
    ...
    user_options = [
        ...
        (
            "verbose-cfg",
            None,
            "change logging level from WARN to INFO which will show all " "compiler output",
        ),
    ]
    ...
    def swig_sources(self, sources, extension):
        ...
        for source in swig_sources:
            ...
            if self.force or newer_group(depends, target, "newer"):
                ...
                self.spawn(
                    swig_cmd + self.swig_opts + ["-o", target, "-outdir", py_target_dir, source]
                )
            else:
...
_swig_module_name_match = re.compile(
    r'\s*%module\s*(.*\(\s*package\s*=\s*"(?P<package>[\w_]+)".*\)|)\s*(?P<name>[\w_]+)', re.I
).match
