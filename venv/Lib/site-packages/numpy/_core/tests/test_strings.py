...
class TestMethods:
    ...
    def test_slice_unsupported(self, dt):
        ...
        regexp = r"Cannot cast ufunc '_slice' input .* " r"from .* to dtype\('int(64|32)'\)"
    ...
    def test_slice_int_type_promotion(self, int_dt, dt):
        ...
        assert_array_equal(np_slice(buf, int_dt(4)), np.array(["hell", "worl"], dtype=dt))
        assert_array_equal(
            np_slice(buf, np.array([4, 4], dtype=int_dt)), np.array(["hell", "worl"], dtype=dt)
        )

        assert_array_equal(np_slice(buf, int_dt(2), int_dt(4)), np.array(["ll", "rl"], dtype=dt))
        assert_array_equal(
            np_slice(buf, np.array([2, 2], dtype=int_dt), np.array([4, 4], dtype=int_dt)),
            np.array(["ll", "rl"], dtype=dt),
        )

        assert_array_equal(
            np_slice(buf, int_dt(0), int_dt(4), int_dt(2)), np.array(["hl", "wr"], dtype=dt)
        )
        assert_array_equal(
            np_slice(
                buf,
