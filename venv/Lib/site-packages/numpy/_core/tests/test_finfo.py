...
def float16_ma():
    ...
    f16 = ntypes.float16
    return MachArLike(f16, machep=-10, negep=-11, minexp=-14, maxexp=16, nmant=10, iexp=5)
...
def float32_ma():
    ...
    f32 = ntypes.float32
    return MachArLike(f32, machep=-23, negep=-24, minexp=-126, maxexp=128, nmant=23, iexp=8)
...
def float64_ma():
    ...
    f64 = ntypes.float64
    return MachArLike(f64, machep=-52, negep=-53, minexp=-1022, maxexp=1024, nmant=52, iexp=11)
...
def test_finfo_properties(dtype, ma_fixture, prop, request):
    ...
    assert actual == expected, (
        f"finfo({dtype}) property '{prop}' mismatch: " f"expected {expected}, got {actual}"
    )
