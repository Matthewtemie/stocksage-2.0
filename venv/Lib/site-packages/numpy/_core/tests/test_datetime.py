...
class TestDateTime:
    ...
    def test_datetime_divide(self):
        ...
        # NaTs
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", r".*encountered in divide", RuntimeWarning)
            nat = np.timedelta64("NaT")
            for tp in (int, float):
    ...
    def test_datetime_as_string_timezone(self):
        ...
        assert_equal(
            np.datetime_as_string(a, timezone=ZoneInfo("US/Central")), "2010-03-15T01:30-0500"
        )
        assert_equal(
            np.datetime_as_string(a, timezone=ZoneInfo("US/Eastern")), "2010-03-15T02:30-0400"
        )
        assert_equal(
            np.datetime_as_string(a, timezone=ZoneInfo("US/Pacific")), "2010-03-14T23:30-0700"
        )

        assert_equal(
            np.datetime_as_string(b, timezone=ZoneInfo("US/Central")), "2010-02-15T00:30-0600"
        )
        assert_equal(
            np.datetime_as_string(b, timezone=ZoneInfo("US/Eastern")), "2010-02-15T01:30-0500"
        )
        assert_equal(
            np.datetime_as_string(b, timezone=ZoneInfo("US/Pacific")), "2010-02-14T22:30-0800"
        )

        # Dates to strings with a timezone attached is disabled by default
        assert_raises(
            TypeError, np.datetime_as_string, a, unit="D", timezone=ZoneInfo("US/Pacific")
        )
        # Check that we can print out the date in the specified time zone
        assert_equal(
            np.datetime_as_string(a, unit="D", timezone=ZoneInfo("US/Pacific"), casting="unsafe"),
            "2010-03-14",
        )
        assert_equal(
            np.datetime_as_string(b, unit="D", timezone=ZoneInfo("US/Central"), casting="unsafe"),
            "2010-02-15",
        )
