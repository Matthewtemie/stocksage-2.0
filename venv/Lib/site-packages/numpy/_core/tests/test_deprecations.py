...
class TestDeprecatedDTypeAliases(_DeprecationTestCase):
    ...
    def _check_for_warning(self, func):
        with pytest.warns(DeprecationWarning, match="alias 'a' was deprecated in NumPy 2.0") as w:
            func()
        assert len(w) == 1
...
class TestAddNewdocUFunc(_DeprecationTestCase):
    ...
    @pytest.mark.thread_unsafe(reason="modifies and checks docstring which is global state")
    def test_deprecated(self):
...
class TestDTypeAlignBool(_VisibleDeprecationTestCase):
    ...
    def test_deprecated(self):
        ...
        self.assert_deprecated(lambda: np.dtype("f8", align=np.long(0), copy=np.long(1)))
