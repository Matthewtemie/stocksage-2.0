...
def is_final_type(tp):
    ...
    """
    if NEW_TYPING:
        return tp is Final or isinstance(tp, typingGenericAlias) and tp.__origin__ is Final
...
def is_literal_type(tp):
    if NEW_TYPING:
        return tp in LITERALS or isinstance(tp, typingGenericAlias) and tp.__origin__ in LITERALS
...
def get_parameters(tp):
    ...
    if LEGACY_TYPING:
        ...
        if (
            ...
            return ()
    elif is_generic_type(tp) or is_union_type(tp) or is_callable_type(tp) or is_tuple_type(tp):
