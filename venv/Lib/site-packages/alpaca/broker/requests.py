...
class UploadW8BenDocumentRequest(NonEmptyRequest):
    ...
    def __init__(self, **data) -> None:
        ...
        if "mime_type" not in data and "content_data" in data and data["content_data"] is not None:
    ...
    def root_validator(cls, values: dict) -> dict:
        ...
        if not content_data_is_none and values["mime_type"] != UploadDocumentMimeType.JSON:
    ...
    def root_validator(cls, values: dict) -> dict:
        ...
        if bank_code_type == IdentifierType.ABA:
            for international_param in international_parameters:
                if international_param in values and values[international_param] is not None:
                    raise ValueError(
                        f"You may only specify the {international_param} for international bank accounts."
                    )
        elif bank_code_type == IdentifierType.BIC:
            for international_param in international_parameters:
                if international_param not in values or values[international_param] is None:
                    raise ValueError(
...
class OrderRequest(BaseOrderRequest):
    ...
    @model_validator(mode="before")
    def order_type_must_be_market_for_lct(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if (
            ...
            and values.get("currency", SupportedCurrencies.USD) != SupportedCurrencies.USD
        ):
            raise ValueError(
...
class Weight(BaseModel):
    ...
    def validator(cls, values: dict) -> dict:
        ...
        if values["type"] == WeightType.ASSET.value and values.get("symbol", None) is None:
            raise ValueError
        return values
