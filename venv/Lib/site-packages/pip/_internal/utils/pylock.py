...
def _pylock_package_from_install_requirement(ireq: InstallRequirement, base_dir: Path) -> Package:
    ...
    if ireq.is_direct:
        if download_info.vcs_info:
            ...
            package_directory = PackageDirectory(
                path=(
                    Path(url_to_path(download_info.url)).resolve().relative_to(base_dir).as_posix()
                ),
                editable=(
                    download_info.dir_info.editable if download_info.dir_info.editable else None
                ),
                subdirectory=download_info.subdirectory,
            )
...
def _package_dist_url(pylock_path_or_url: str, path: str | None, url: str | None) -> str:
    ...
    if path is not None:
        if not os.path.isabs(path):
            ...
            if _is_url(pylock_path_or_url):
                ...
                return path_to_url(os.path.join(os.path.dirname(pylock_path_or_url), path))
        else:
...
def package_vcs_requirement_url(pylock_path_or_url: str, package_vcs: PackageVcs) -> str:
...
def package_archive_requirement_url(
    ...
    url = _package_dist_url(pylock_path_or_url, package_archive.path, package_archive.url)
...
def package_directory_requirement_url(
    ...
    if _is_url(pylock_path_or_url) and not pylock_path_or_url.startswith("file://"):
        raise InstallationError(
            f"Directory entries are not supported in remote pylock.toml " f"{pylock_path_or_url!r}"
        )
...
def package_sdist_requirement_url(pylock_path_or_url: str, package_sdist: PackageSdist) -> str:
    return _package_dist_url(pylock_path_or_url, package_sdist.path, package_sdist.url)


def package_wheel_requirement_url(pylock_path_or_url: str, package_wheel: PackageWheel) -> str:
...
def select_from_pylock_path_or_url(
    ...
    try:
        ...
        raise InstallationError(f"Error reading pylock file {pylock_path_or_url!r}: {exc}") from exc

    try:
        lock = Pylock.from_dict(tomllib.loads(pylock_content))
    except Exception as exc:
        raise InstallationError(f"Invalid pylock file {pylock_path_or_url!r}: {exc}") from exc
