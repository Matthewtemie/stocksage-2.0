...
def install_req_from_pylock_package(
    ...
    if isinstance(package_dist, pylock.PackageVcs):
        ...
        if isinstance(package_dist, pylock.PackageSdist) and "source" not in allowed_formats:
            raise InstallationError(
        ...
        if isinstance(package_dist, pylock.PackageWheel) and "binary" not in allowed_formats:
        ...
        if isinstance(package_dist, pylock.PackageWheel):
            ...
            requirement_url = package_wheel_requirement_url(pylock_path_or_url, package_dist)
        elif isinstance(package_dist, pylock.PackageSdist):
            if not version:
                _, version = parse_sdist_filename(package_dist.filename)
            requirement_url = package_sdist_requirement_url(pylock_path_or_url, package_dist)
        ireq = InstallRequirement(
