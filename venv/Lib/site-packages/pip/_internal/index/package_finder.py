        ...
        if self._uploaded_prior_to is not None and not link.is_file:
            if link.upload_time is None:
                ...
                return (
                    ...
                    f"Upload time {link.upload_time} not " f"prior to {self._uploaded_prior_to}",
                )
...
class CandidateEvaluator:
    ...
    def get_applicable_candidates(
        ...
        applicable_candidates = specifier.filter(
            ...
            key=lambda c: (
                str(c.version) if select_backend().NAME == "pkg_resources" else c.version
            ),
        )
    ...
    def find_requirement(
        ...
        if installed_version is None and best_candidate is None:
            ...
            if self.release_control is not None:
                allows_pre = self.release_control.allows_prereleases(canonicalize_name(name))
