...
class CliRunner:
    ...
    def __init__(
        ...
        if capture not in {"sys", "fd"}:
            raise ValueError(f"capture={capture!r} is not valid. Choose from 'sys' or 'fd'.")
        if capture == "fd" and sys.platform == "win32":
            raise ValueError(f"capture={capture!r} is not supported on Windows. Use 'sys'.")
    ...
    def isolation(
        ...
        def _patched_pdb_init(
            ...
            **kwargs: t.Any,
            ...
            old_pdb_init(self, completekey=completekey, stdin=stdin, stdout=stdout, **kwargs)
