...
class _NumberRangeBase(_NumberParamTypeBase[ParamTypeValue]):
    ...
    def convert(self, value: t.Any, param: Parameter | None, ctx: Context | None) -> ParamTypeValue:
        ...
        min = self.min
        max = self.max
        lt_min: bool = min is not None and (operator.le if self.min_open else operator.lt)(
            rv, min
        )  # type: ignore[arg-type]
        gt_max: bool = max is not None and (operator.ge if self.max_open else operator.gt)(
            rv, max
        )  # type: ignore[arg-type]
    ...
    @abc.abstractmethod
    def _clamp(self, bound: ParamTypeValue, dir: t.Literal[1, -1], open: bool) -> ParamTypeValue:
...
class UUIDParameterType(ParamType[uuid.UUID]):
    ...
    def convert(self, value: t.Any, param: Parameter | None, ctx: Context | None) -> uuid.UUID:
...
def convert_type(ty: t.Any | None = None, default: t.Any | None = None) -> ParamType[t.Any]:
