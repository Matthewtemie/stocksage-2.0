...
def test_is_hashable():
    ...
    class HashableSlice:
        ...
        def __repr__(self):
            return f"HashableSlice({self.slice.start}, {self.slice.stop}, " f"{self.slice.step})"
...
class TestInference:
    ...
    def test_maybe_convert_objects_datetime(self):
        ...
        exp = np.array([np.timedelta64("NaT", "s"), np.timedelta64(1, "s")], dtype="m8[s]")
        out = lib.maybe_convert_objects(arr, convert_non_numeric=True)
        tm.assert_numpy_array_equal(out, exp)
...
class TestTypeInference:
    ...
    @pytest.mark.parametrize(
        "arr",
        [
            np.array([np.timedelta64("NaT", "ns"), np.datetime64("2011-01-02")], dtype=object),
            np.array([np.datetime64("2011-01-02"), np.timedelta64("NaT", "ns")], dtype=object),
            ...
            np.array([np.datetime64("nat", "ns"), np.timedelta64(1, "D")], dtype=object),
            np.array([np.timedelta64(1, "D"), np.datetime64("nat", "ns")], dtype=object),
        ],
    )
    ...
    def test_infer_dtype_period_mixed(self):
        arr = np.array([Period("2011-01", freq="M"), np.datetime64("nat", "ns")], dtype=object)
        assert lib.infer_dtype(arr, skipna=False) == "mixed"

        arr = np.array([np.datetime64("nat", "ns"), Period("2011-01", freq="M")], dtype=object)
        assert lib.infer_dtype(arr, skipna=False) == "mixed"
    ...
    def test_infer_dtype_all_nan_nat_like(self):
        ...
        arr = np.array([pd.NaT, np.datetime64("nat", "ns"), np.timedelta64("NaT", "ns"), np.nan])
        assert lib.infer_dtype(arr, skipna=False) == "mixed"

        arr = np.array([np.timedelta64("NaT", "ns"), np.datetime64("nat", "ns")], dtype=object)
        assert lib.infer_dtype(arr, skipna=False) == "mixed"
    ...
    def test_is_datetimelike_array_all_nan_nat_like(self):
        ...
        arr = np.array([np.nan, pd.NaT, np.datetime64("nat", "ns"), np.timedelta64("NaT", "ns")])
        assert not lib.is_datetime_array(arr)
        assert not lib.is_datetime64_array(arr)
        assert not lib.is_timedelta_or_timedelta64_array(arr)
    ...
    def test_string_dtype(self, data, skipna, index_or_series_or_array, nullable_string_dtype):
        # StringArray
