...
class TestTimedelta64ArithmeticUnsorted:
    ...
    def test_subtraction_ops(self):
        ...
        result = dt - tdi
        expected = DatetimeIndex(["20121231", NaT, "20121230"], dtype="M8[us]", name="foo")
        tm.assert_index_equal(result, expected)
    ...
    def test_addition_ops(self):
        ...
        result = tdi + dt
        expected = DatetimeIndex(["20130102", NaT, "20130103"], dtype="M8[us]", name="foo")
        tm.assert_index_equal(result, expected)
    ...
    def test_tda_add_dt64_object_array(self, performance_warning, box_with_array, tz_naive_fixture):
...
class TestTimedeltaArraylikeAddSubOps:
    ...
    def test_operators_timedelta64(self):
        ...
        expected = Series([timedelta(days=4017 + i) for i in range(3)], name="A", dtype="m8[ns]")
        tm.assert_series_equal(result, expected)
        assert result.dtype == "m8[ns]"
    ...
    def test_td64arr_add_sub_tdi(self, box_with_array, names):
        ...
        ser = Series([Timedelta(hours=3), Timedelta(hours=4)], name=names[0], dtype="m8[ns]")
        expected = Series(
    ...
    def test_td64arr_add_sub_offset_index(self, performance_warning, names, box_with_array):
        # GH#18849, GH#19744
        box = box_with_array
        exname = get_expected_name(box, names)
    ...
    def test_td64arr_with_offset_series(self, performance_warning, names, box_with_array):
        # GH#18849
        box = box_with_array
...
class TestTimedeltaArraylikeMulDivOps:
    ...
    def test_td64arr_mod_tdscalar(self, performance_warning, box_with_array, three_days):
    ...
    def test_td64arr_div_numeric_array(self, box_with_array, klass, any_real_numpy_dtype):
        # GH#4521
        # divide/multiply by integers
...
def test_add_timestamp_to_timedelta():
    ...
    expected = DatetimeIndex(
        [
            timestamp
            + (pd.to_timedelta("0.033333s") * i + pd.to_timedelta("0.000001s") * divmod(i, 3)[0])
            for i in range(31)
        ]
    )
    tm.assert_index_equal(result, expected)
