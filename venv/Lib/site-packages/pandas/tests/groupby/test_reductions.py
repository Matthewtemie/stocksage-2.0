...
def test_masked_mixed_types(dtype1, dtype2, exp_col1, exp_col2):
    ...
    df = DataFrame({"col1": pd.array(data1, dtype=dtype1), "col2": pd.array(data2, dtype=dtype2)})
...
def test_masked_bool_aggs_skipna(all_boolean_reductions, dtype, skipna, frame_or_series):
    # GH#40585
...
def test_idxmin_idxmax_extremes_skipna(skipna, how, float_numpy_dtype):
    ...
    expected = DataFrame({"b": [1, 3, 4, 6]}, index=pd.Index(range(1, 5), name="a", dtype="intp"))
    tm.assert_frame_equal(result, expected)
