...
def _call_and_check(klass, msg, how, gb, groupby_func, args, warn_category=None, warn_msg=""):
    with tm.assert_produces_warning(warn_category, match=warn_msg, check_stacklevel=False):
        if klass is None:
            if how == "method":
...
@pytest.mark.parametrize("how", ["method", "agg", "transform"])
def test_groupby_raises_category(how, by, groupby_series, groupby_func, df_with_cat_col):
    # GH#50749
    df = df_with_cat_col
    args = get_groupby_method_args(groupby_func, df)
    gb = df.groupby(by=by)
