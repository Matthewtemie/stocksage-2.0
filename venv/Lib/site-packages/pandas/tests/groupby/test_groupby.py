...
def test_wrap_aggregated_output_multindex(multiindex_dataframe_random_data, using_infer_string):
...
def test_groupby_with_hier_columns():
    ...
    df = DataFrame(np.random.default_rng(2).standard_normal((8, 4)), index=index, columns=columns)
...
def test_groupby_multiindex_not_lexsorted(performance_warning):
    ...
    # GH 14776
    df = DataFrame({"x": ["a", "a", "b", "a"], "y": [1, 1, 2, 2], "z": [1, 2, 3, 4]}).set_index(
        ["x", "y"]
    )
...
def test_groups_sort_dropna(sort, dropna):
    ...
    for result_value, expected_value in zip(result.values(), expected.values(), strict=True):
...
def test_rolling_wrong_param_min_period():
    ...
    result_error_msg = r"^[a-zA-Z._]*\(\) got an unexpected keyword argument 'min_period'"
...
def test_groupby_sum_on_nan_should_return_nan(bug_var):
    ...
    expected_df = DataFrame([bug_var, bug_var, bug_var, None], columns=["A"], dtype=df["A"].dtype)
    tm.assert_frame_equal(result, expected_df)
...
def test_groupby_multi_index_codes():
    # GH#54347
    df = DataFrame({"A": [1, 2, 3, 4], "B": [1, float("nan"), 2, float("nan")], "C": [2, 4, 6, 8]})
