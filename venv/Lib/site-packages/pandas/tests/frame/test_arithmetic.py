...
class TestFrameArithmetic:
    ...
    def test_df_arith_2d_array_rowlike_broadcasts(self, request, all_arithmetic_operators):
        # GH#23000
    ...
    def test_df_arith_2d_array_collike_broadcasts(self, request, all_arithmetic_operators):
        # GH#23000
        opname = all_arithmetic_operators
        arr = np.arange(6).reshape(3, 2)
        df = DataFrame(arr, columns=[True, False], index=["A", "B", "C"])
    ...
    def test_binop_other(self, op, value, dtype, switch_numexpr_min_elements):
        ...
        if (op, dtype) in invalid:
            ...
            if op in [operator.add, operator.mul]:
                if expr.USE_NUMEXPR and switch_numexpr_min_elements == 0 and ne_warns_on_op:
                    warn = UserWarning
                else:
                    warn = None
                with tm.assert_produces_warning(warn, match="evaluating in Python"):
...
def test_sum_mixed_empty(any_string_dtype):
    ...
    expected = Series({"col1": "" if any_string_dtype == "string" else 0, "col2": 0}, dtype=object)
