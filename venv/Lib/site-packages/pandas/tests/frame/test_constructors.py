...
class TestDataFrameConstructors:
    ...
    def test_constructor_list_of_dict_with_str_na_key(self, missing_value, using_infer_string):
    ...
    @pytest.mark.parametrize("missing_value", [None, np.nan, pd.NA])
    def test_constructor_dict_of_dict_with_str_na_key(self, missing_value, using_infer_string):
        # https://github.com/pandas-dev/pandas/issues/63889
        dict_data = {"col": {"row1": 1, missing_value: 2, "row3": 3}}
        result = DataFrame(dict_data)
        expected = DataFrame(
            {"col": [1, 2, 3]},
            index=Index(["row1", np.nan if using_infer_string else missing_value, "row3"]),
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_construction_nan_value_timedelta64_dtype(self):
        ...
        expected = DataFrame(["NaT", "0 days 00:00:00.000000001"], dtype="timedelta64[ns]")
        tm.assert_frame_equal(result, expected)
...
class TestFromScalar:
    ...
    def test_from_out_of_bounds_ns_timedelta(self, constructor, cls, box, frame_or_series):
