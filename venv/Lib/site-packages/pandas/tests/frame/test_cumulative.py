...
class TestDataFrameCumulativeOps:
    ...
    def test_cumulative_ops_match_series_apply(self, datetime_frame, all_numeric_accumulations):
        datetime_frame.iloc[5:10, 0] = np.nan
        datetime_frame.iloc[10:15, 1] = np.nan
        datetime_frame.iloc[15:, 2] = np.nan
        ...
        expected = datetime_frame.apply(getattr(Series, all_numeric_accumulations), axis=1)
        tm.assert_frame_equal(result, expected)
