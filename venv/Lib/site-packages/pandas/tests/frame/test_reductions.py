...
class TestDataFrameAnalytics:
    ...
    def test_std_timedelta64_skipna_false(self):
        ...
        expected = Series([df["A"].std(), pd.NaT], index=["A", "B"], dtype="timedelta64[us]")
        tm.assert_series_equal(result, expected)

        result = df.std(axis=1, skipna=False)
        expected = Series([pd.Timedelta(0)] * 8 + [pd.NaT, pd.Timedelta(0)], dtype="m8[us]")
        tm.assert_series_equal(result, expected)
    ...
    def test_any_all_mixed_float(self, all_boolean_reductions, axis, bool_only, float_string_frame):
    ...
    @pytest.mark.parametrize("axis", [0, 1])
    def test_any_all_bool_with_na(self, all_boolean_reductions, axis, bool_frame_with_na):
...
def test_numeric_ea_axis_1(method, skipna, min_count, any_numeric_ea_dtype, using_nan_is_na):
    # GH 54341
    df = DataFrame(
        {
...
def test_mean_nullable_int_axis_1():
    # GH##36585
    df = DataFrame({"a": [1, 2, 3, 4], "b": Series([1, 2, 4, None], dtype=pd.Int64Dtype())})
