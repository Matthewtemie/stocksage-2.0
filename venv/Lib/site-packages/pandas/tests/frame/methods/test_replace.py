...
class TestDataFrameReplace:
    ...
    def test_regex_replace_dict_mixed(self, mix_abc):
        ...
        res2 = dfmix.copy()
        result = res2.replace({"b": r"\s*\.\s*"}, {"b": np.nan}, inplace=True, regex=True)
        ...
        res2 = dfmix.copy()
        result = res2.replace({"b": r"\s*(\.)\s*"}, {"b": r"\1ty"}, inplace=True, regex=True)
        assert result is res2
        ...
        res2 = dfmix.copy()
        result = res2.replace(regex={"b": r"\s*(\.)\s*"}, value={"b": r"\1ty"}, inplace=True)
        assert result is res2
        expec = DataFrame({"a": mix_abc["a"], "b": ["a", "b", ".ty", ".ty"], "c": mix_abc["c"]})
        tm.assert_frame_equal(res, expec)
        tm.assert_frame_equal(res2, expec)
    ...
    def test_replace_dict_category_type(self):
        ...
        result = input_df.apply(
            lambda x: x.cat.rename_categories({"a": "z", "obj1": "obj9", "cat1": "catX"})
        )
    ...
    def test_replace_na_to_nan_nullable_floats(self, dtype, using_nan_is_na):
        ...
        if using_nan_is_na:
            ...
            expected = DataFrame({0: [1, np.nan, 1], 1: Series([0, np.nan, 1], dtype=dtype)})
