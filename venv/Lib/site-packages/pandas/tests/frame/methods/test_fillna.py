...
class TestFillNA:
    ...
    def test_fillna_mixed_type(self, float_string_frame, using_infer_string):
        ...
        result = mf.ffill()
        assert (result.loc[result.index[-10:], "A"] == result.loc[result.index[-11], "A"]).all()
    ...
    def test_fillna_with_categorical_series(self):
        ...
        df = DataFrame({"cats": Categorical(["A", "B", "C"]), "ints": [1.0, 2.0, np.nan]})

        filler = Series(Categorical([10.0, 20.0, 30.0]))
        result = df.fillna({"ints": filler})

        expected = DataFrame({"cats": Categorical(["A", "B", "C"]), "ints": [1.0, 2.0, 30.0]})
        tm.assert_frame_equal(result, expected)
    ...
    def test_fillna_dtype_conversion(self):
        ...
        result = df.fillna(1)
        expected = DataFrame(1, index=["A", "B", "C"], columns=[1, 2, 3, 4, 5], dtype=object)
        tm.assert_frame_equal(result, expected)
