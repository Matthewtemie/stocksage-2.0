...
class TestLocBaseIndependent:
    ...
    def test_loc_to_fail2(self):
        ...
        s["a"] = 2
        msg = rf"\"None of \[Index\(\[-2\], dtype='{np.dtype(int)}'\)\] are " r"in the \[index\]\""
        with pytest.raises(KeyError, match=msg):
            s.loc[[-2]]
    ...
    def test_loc_to_fail3(self):
        ...
        msg = rf"\"None of \[Index\(\[3\], dtype='{np.dtype(int)}'\)\] are " r"in the \[index\]\""
        with pytest.raises(KeyError, match=msg):
            df.loc[[3], :]
    ...
    def test_loc_setitem_empty_frame(self, has_ref):
        ...
        expected = DataFrame({"A": sera, "B": serb}, columns=Index(["A", "B"])).reindex(index=index)
        tm.assert_frame_equal(df, expected)
    ...
    def test_loc_setitem_frame_mixed_labels(self):
        ...
        result = df.loc[0, [1, 2]]
        expected = Series([1, 3], index=Index([1, 2], dtype=object), dtype="int64", name=0)
        tm.assert_series_equal(result, expected)
...
class TestLocSetitemWithExpansion:
    ...
    def test_loc_setitem_datetime_keys_cast(self, conv, using_infer_string):
        ...
        expected = DataFrame(
            {"one": [100.0, 200.0]},
            index=Index([conv(dt1), conv(dt2)], dtype=None if using_infer_string else object),
            columns=Index(["one"]),
        )
        tm.assert_frame_equal(df, expected)
    ...
    def test_loc_setitem_with_expansion_preserves_nullable_int(self, any_numeric_ea_dtype):
        # GH#42099
...
class TestLocSeries:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [
                ...
                [True, True, True],
                ["A", "B", "C"],
                DataFrame([[10, 10, 10], [20, 20, 20], [30, 30, 30]], columns=list("ABC")),
            ],
            [
                DataFrame([[1, 2, 3], [4, 5, 6], [7, 8, 9]], columns=list("ABC")),
                slice(0, 4),
    ...
    def test_loc_set_series_to_multiple_columns(self, df, row_index, col_index, expected_df):
        # GH 59933
