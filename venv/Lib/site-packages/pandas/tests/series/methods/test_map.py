...
def test_map_callable(datetime_series, engine):  # noqa: F811
    ...
    tm.assert_series_equal(datetime_series.map(math.exp, engine=engine), np.exp(datetime_series))
...
def test_map_arg_as_kwarg():
    with tm.assert_produces_warning(Pandas4Warning, match="`arg` has been renamed to `func`"):
        Series([1, 2]).map(arg={})
