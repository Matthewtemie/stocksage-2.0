...
def test_constructor_nan_like(na):
    ...
    result = pd.arrays.StringArray(np.array(["a", na], dtype="object"), dtype=pd.StringDtype())
...
def test_from_sequence_no_mutate(copy, cls, dtype):
    ...
    if cls is ArrowStringArray:
        ...
        expected = cls(pa.array(na_arr, type=pa.string(), from_pandas=True), dtype=dtype)
...
def test_arrow_from_string(using_infer_string):
    ...
    if not using_infer_string:
        if pa_version_under19p0:
            ...
            expected = pd.DataFrame({"a": ["a", "b", None]}, dtype=pd.StringDtype(na_value=np.nan))
