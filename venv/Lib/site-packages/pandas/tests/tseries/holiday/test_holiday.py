...
def test_holiday_dates(holiday, start_date, end_date, expected):
    ...
    assert list(
        holiday.dates(Timestamp(start_date, tz=timezone.utc), Timestamp(end_date, tz=timezone.utc))
...
def test_holidays_within_dates(holiday, start, expected):
    ...
    # Verify that timezone info is preserved.
    assert list(
        holiday.dates(Timestamp(start, tz=timezone.utc), Timestamp(start, tz=timezone.utc))
    ) == [dt.replace(tzinfo=timezone.utc) for dt in expected]
...
def test_holiday_with_multiple_exclusions():
    ...
    yakudoshi_new_year = Holiday("Yakudoshi New Year", month=1, day=1, exclude_dates=exclude)
