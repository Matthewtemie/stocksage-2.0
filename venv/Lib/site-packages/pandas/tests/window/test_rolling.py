...
def test_iter_rolling_on_dataframe(expected, window):
    ...
    expecteds = [DataFrame(values, index=df.loc[index, "C"]) for (values, index) in expected]
...
def test_iter_rolling_datetime(expected, expected_index, window):
    ...
    expecteds = [
        Series(values, index=idx) for (values, idx) in zip(expected, expected_index, strict=True)
    ]
...
def test_rolling_var_correctness(func, values, window, ddof, expected_values):
    ...
    if result.last_valid_index():
        result = result[result.first_valid_index() : result.last_valid_index() + 1].reset_index(
            drop=True
        )
...
def test_rolling_decreasing_indices(method):
    ...
    tm.assert_almost_equal(decreasing.values[::-1][:-4], increasing.values[4:], atol=1e-12)
...
class TestMinMax:
    ...
    def test_minmax(self, is_max, has_nan, exp_list):
        ...
        r = df.data.rolling(PrescribedWindowIndexer(df.start.to_numpy(), df.end.to_numpy()))
        if is_max:
            result = r.max()
        else:
            result = r.min()
