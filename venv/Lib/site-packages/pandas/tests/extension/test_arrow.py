...
class TestArrowArray(base.ExtensionTests):
    ...
    def _get_arith_xfail_marker(self, opname, pa_dtype):
        ...
        if opname == "__rpow__" and (
            ...
            mark = pytest.mark.xfail(
                reason=(
                    f"GH#29997: 1**pandas.NA == 1 while 1**pyarrow.NA == NULL " f"for {pa_dtype}"
                )
            )
    ...
    def test_loc_setitem_with_expansion_preserves_ea_index_dtype(self, data, request):
        ...
        if pa.types.is_date(pa_dtype):
            mark = pytest.mark.xfail(reason="GH#62343 incorrectly casts to timestamp[ms][pyarrow]")
            request.applymarker(mark)
...
def test_arrow_string_addition_mixed_with_binary_raises(string_type):
    ...
    msg = f"operation 'add' not supported for dtype '{left.dtype}' " f"with dtype '{right.dtype}'"
    with pytest.raises(TypeError, match=re.escape(msg)):
        left + right

    reflected_msg = (
        f"operation 'add' not supported for dtype '{right.dtype}' " f"with dtype '{left.dtype}'"
    )
...
def test_as_unit_duration_truncation(from_unit, to_unit):
    ...
    ser_numpy = pd.Series(pd.to_timedelta([93784567890123, None], unit="ns").as_unit(from_unit))
...
def test_as_unit_timestamp_with_timezone(to_unit):
    ...
    ser_numpy = pd.Series(
        pd.to_datetime(["2024-01-15 12:30:45.123456789"]).tz_localize("US/Eastern").as_unit("ns")
    )
    ser_arrow = ser_numpy.astype("timestamp[ns, US/Eastern][pyarrow]")

    result = ser_arrow.dt.as_unit(to_unit)
    expected = ser_numpy.dt.as_unit(to_unit).astype(f"timestamp[{to_unit}, US/Eastern][pyarrow]")
    tm.assert_series_equal(result, expected)
...
def test_factorize_dictionary_with_na():
    # GH#60567
    arr = pd.array(["a1", pd.NA], dtype=ArrowDtype(pa.dictionary(pa.int32(), pa.utf8())))
...
def test_categorical_from_arrow_dictionary():
    # GH 60563
    df = pd.DataFrame({"A": ["a1", "a2"]}, dtype=ArrowDtype(pa.dictionary(pa.int32(), pa.utf8())))
...
@pytest.mark.skipif(pa_version_under19p0, reason="pa.json_ was introduced in pyarrow v19.0")
...
def test_duration_reduction_consistency(unit, method):
    ...
    assert isinstance(result, pd.Timedelta), f"{method} for {unit} returned {type(result)}"
...
def test_timestamp_reduction_consistency(unit, method):
    ...
    result = getattr(ser, method)()
    assert isinstance(result, pd.Timestamp), f"{method} for {unit} returned {type(result)}"
    assert result.unit == unit
