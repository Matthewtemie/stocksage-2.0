...
class TestDatetimeIndexShift:
    ...
    def test_shift_periods(self, unit):
        ...
        idx = date_range(start=datetime(2009, 1, 1), end=datetime(2010, 1, 1), periods=3, unit=unit)
        tm.assert_index_equal(idx.shift(periods=0), idx)
        tm.assert_index_equal(idx.shift(0), idx)

    @pytest.mark.parametrize("freq", ["B", "C"])
    def test_shift_bday(self, freq, unit):
        rng = date_range(datetime(2009, 1, 1), datetime(2010, 1, 1), freq=freq, unit=unit)
        shifted = rng.shift(5)
