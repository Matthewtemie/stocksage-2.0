...
class TestArrayToDatetimeResolutionInference:
    ...
    def test_infer_homogeoneous_dt64(self):
        ...
        result, tz = tslib.array_to_datetime(arr, creso=creso_infer)
        assert tz is None
        expected = np.array([np.datetime64("NaT", "ns"), dt64, dt64, dt64], dtype="M8[ms]")
        tm.assert_numpy_array_equal(result, expected)
    ...
    def test_infer_homogeoneous_timestamps(self):
        ...
        result, tz = tslib.array_to_datetime(arr, creso=creso_infer)
        assert tz is None
        expected = np.array([np.datetime64("NaT", "ns")] + [ts.asm8] * 3, dtype="M8[ns]")
        tm.assert_numpy_array_equal(result, expected)
    ...
    def test_infer_homogeoneous_datetimes_strings(self):
        ...
        result, tz = tslib.array_to_datetime(arr, creso=creso_infer)
        assert tz is None
        expected = np.array([np.datetime64("NaT", "ns"), item, item, item], dtype="M8[us]")
        tm.assert_numpy_array_equal(result, expected)
