...
def test_cut_nullable_integer(bins, right, include_lowest):
    ...
    b = a.astype(object)
    b[::2] = pd.NA
    result = cut(pd.array(b, dtype="Int64"), bins, right=right, include_lowest=include_lowest)
...
def test_cut_datetime_array_no_attributeerror():
    ...
    tm.assert_categorical_equal(result, expected, check_dtype=True, check_category_order=True)
