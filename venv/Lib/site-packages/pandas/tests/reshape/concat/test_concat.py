    ...
    def test_concat_copy(self):
        ...
        for blocks in result._mgr.blocks:
            ...
            if arr.dtype.kind == "f":
                ...
                assert any(np.shares_memory(arr, block.values) for block in df3._mgr.blocks)
...
def test_concat_none_with_timezone_timestamp():
    ...
    result = concat([df1, df2], ignore_index=True)
    expected = DataFrame({"A": [None, pd.Timestamp("1990-12-20 00:00:00+00:00")]}, dtype=object)
    tm.assert_frame_equal(result, expected)
