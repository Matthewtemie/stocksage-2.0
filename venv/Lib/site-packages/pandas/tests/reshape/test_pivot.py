...
class TestPivotTable:
    ...
    def test_pivot_table_with_margins_set_margin_name(self, margin_name, data):
        # see gh-3335
        msg = (
            f'Conflicting name "{margin_name}" in margins|' "margins_name argument must be a string"
        )
    ...
    def test_pivot_timegrouper_double(self):
        ...
        expected = DataFrame(
            np.array([3, np.nan, 6, np.nan, 1, np.nan, 9, np.nan, 9, np.nan, np.nan, 3]).reshape(
                6, 2
            ),
            index=idx,
            columns=["A", "B"],
    ...
    def test_categorical_margins_category(self, observed):
        ...
        table = df.pivot_table("x", "y", "z", dropna=observed, margins=True, observed=False)
        tm.assert_frame_equal(table, expected)
    ...
    def test_pivot_table_kwargs(self, kwargs):
        ...
        result = pivot_table(df, index="A", columns="B", values="X", aggfunc=f, **kwargs)
    ...
    @pytest.mark.slow
    def test_pivot_number_of_levels_larger_than_int32_warns(self, performance_warning, monkeypatch):
        # GH 20601
    ...
    def test_pivot_table_values_as_two_params(self, index, columns, e_data, e_index, e_cols):
        # GH#57876
        data = {"Category": ["A", "B", "A", "B"], "Value": [10, 20, 40, 50]}
        df = DataFrame(data)
        result = df.pivot_table(index=index, columns=columns, values="Value", aggfunc="count")
...
class TestPivot:
    ...
    def test_pivot_integer_bug(self, any_string_dtype):
        df = DataFrame(data=[("A", "1", "A1"), ("B", "2", "B2")], dtype=any_string_dtype)
    ...
    def test_pivot_empty_with_datetime(self):
        ...
        df_pivoted = df.pivot_table(index="category", columns="value", values="timestamp")
    ...
    def test_pivot_with_pyarrow_categorical(self):
        ...
        df = DataFrame({"string_column": ["A", "B", "C"], "number_column": [1, 2, 3]}).astype(
            {
    ...
    def test_pivot_empty_dataframe_period_dtype(self, freq):
        ...
        expected = DataFrame(index=expected_index, columns=expected_columns, dtype=dtype)
