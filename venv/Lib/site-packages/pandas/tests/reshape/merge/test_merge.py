...
class TestMerge:
    ...
    def test_merge_datetime64tz_values(self):
        left = DataFrame(
            {
                "key": [1, 2],
                "value": pd.date_range("20151010", periods=2, tz="US/Eastern", unit="ns"),
            }
        )
        right = DataFrame(
            {
                "key": [2, 3],
                "value": pd.date_range("20151011", periods=2, tz="US/Eastern", unit="ns"),
            }
        )
        expected = DataFrame(
            {
                "key": [1, 2, 3],
                "value_x": [
                    *list(pd.date_range("20151010", periods=2, tz="US/Eastern", unit="ns")),
                    pd.NaT,
                ],
                "value_y": [
                    pd.NaT,
                    *list(pd.date_range("20151011", periods=2, tz="US/Eastern", unit="ns")),
                ],
            }
        )
        result = merge(left, right, on="key", how="outer")
...
class TestMergeCategorical:
    ...
    def test_merge_on_int_array(self):
        ...
        expected = DataFrame({"A": Series([1, 2, pd.NA], dtype="Int64"), "B_x": 1, "B_y": 1})
        tm.assert_frame_equal(result, expected)
...
def test_merge_categorical_key_recursion():
    ...
    expected = left.astype("int64").merge(right.astype("float64"), on="key", how="outer")
    tm.assert_frame_equal(result, expected)
