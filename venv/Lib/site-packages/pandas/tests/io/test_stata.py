...
class TestStata:
    ...
    def test_read_write_reread_dta14(self, file, parsed_114, version, datapath, temp_file):
        file = datapath("io", "data", "stata", f"{file}.dta")
        parsed = self.read_dta(file)
        ...
        for col in from_frame:
            ...
            if isinstance(ser.dtype, CategoricalDtype):
                ...
                if cat.categories.dtype == object:
                    categories = pd.Index._with_infer(cat.categories._values, copy=False)
...
"""
    ...
    def test_invalid_file_not_written(self, version, temp_file):
        ...
        msg2 = (
            "'ascii' codec can't decode byte 0xef in position 14: " r"ordinal not in range\(128\)"
        )
...
the string values returned are correct."""
    ...
    def test_read_data_int_validranges(self, version, datapath):
        ...
        parsed = read_stata(datapath("io", "data", "stata", f"stata_int_validranges_{version}.dta"))
        tm.assert_frame_equal(parsed, expected)
...
def test_compression(compression, version, use_dict, infer, compression_to_extension, tmp_path):
