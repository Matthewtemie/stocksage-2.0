    ...
    if conn == "postgresql_adbc_conn" and not using_string_dtype():
        ...
        if Version(adbc_pg.__version__) < Version("1.11"):
            request.node.add_marker(
                pytest.mark.xfail(
                    reason=("postgres ADBC driver < 1.11 cannot insert index with null type"),
                )
            )
...
@pytest.mark.parametrize("conn", all_connectable)
@pytest.mark.parametrize("mode, num_row_coef", [("replace", 1), ("append", 2), ("delete_rows", 1)])
...
def test_delete_rows_is_atomic(conn_name, request):
    ...
    with pandasSQL_builder(conn) as pandasSQL:
        ...
        with pytest.raises(pd.errors.DatabaseError):
            with pandasSQL.run_transaction():
                pandasSQL.to_sql(replacing_df, table_name, if_exists="delete_rows", index=False)
...
def test_invalid_engine(conn, request, test_frame1):
    if conn == "sqlite_buildin" or "adbc" in conn:
        request.applymarker(
            pytest.mark.xfail(reason="SQLiteDatabase/ADBCDatabase does not raise for bad engine")
        )
