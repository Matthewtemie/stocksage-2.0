...
<books>
  ...
  <book>
    ...
    df_file.to_xml(temp_file, index=False, root_name="books", row_name="book", parser=parser)
    output = temp_file.read_text(encoding="utf-8").strip()
...
def test_stylesheet_wrong_path(geom_df):
    ...
    with pytest.raises(FileNotFoundError, match=r"\[Errno 2\] No such file or director"):
...
def test_s3_permission_output(parser, s3_bucket_public, geom_df):
    ...
    with tm.external_error_raised((PermissionError, FileNotFoundError)):
        ...
        geom_df.to_xml(f"s3://{s3_bucket_public.name}/geom.xml", compression="zip", parser=parser)
