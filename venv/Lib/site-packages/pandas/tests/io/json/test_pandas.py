...
class TestPandasContainer:
    ...
    @pytest.mark.parametrize("convert_axes", [True, False])
    def test_roundtrip_categorical(self, request, orient, convert_axes, using_infer_string):
    ...
    @pytest.mark.parametrize("convert_axes", [True, False])
    def test_roundtrip_timestamp(self, orient, convert_axes, dtype_backend, datetime_frame):
    ...
    def test_series_non_unique_index(self):
        ...
        tm.assert_series_equal(
            s,
            read_json(StringIO(s.to_json(orient="split")), orient="split", typ="series"),
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                DataFrame({"A": ["a", "b", "c"], "B": pd.to_timedelta(np.arange(3), unit="D")}),
                Pandas4Warning,
            ),
            (
                DataFrame({"A": pd.to_datetime(["2020-01-01", "2020-02-01", "2020-03-01"])}),
                Pandas4Warning,
            ),
        ],
    )
    ...
    def test_timedelta(self, unit):
        ...
        ser = Series([timedelta(23), timedelta(seconds=5)], index=Index([0, 1]), dtype="m8[ns]")
    ...
    def test_default_handler_indirect(self):
        ...
        expected = (
            '[9,[[1,null],["STR",null],[[["mathjs","Complex"],' '["re",4.0],["im",-5.0]],"N\\/A"]]]'
        )
    ...
    def test_datetime_tz(self):
        ...
        df = DataFrame({"A": tz_range, "B": date_range("20130101", periods=3, unit="ns")})
        ...
        expected = DataFrame.from_dict(
            {
                ...
                "Datetime": Series(["2020-01-01", None, "2020-01-03"], dtype="datetime64[ms]"),
            }
        )
        msg = (
            "The default 'epoch' date format is deprecated and will be removed "
    ...
    def test_datetime_as_label(self, date_format, key, unit):
        df = DataFrame([[1]], columns=[(Timestamp(0) + pd.Timedelta("1D")).as_unit(unit)])
        expected = f'{{"{key}":{{"0":1}}}}'
