...
def test_series(temp_h5_path):
    ...
    _check_roundtrip(ts3, tm.assert_series_equal, path=temp_h5_path, check_index_type=False)
...
def test_frame(compression, temp_h5_path):
    ...
    _check_roundtrip_table(df, tm.assert_frame_equal, path=temp_h5_path, compression=compression)
    _check_roundtrip(df, tm.assert_frame_equal, path=temp_h5_path, compression=compression)
    ...
    _check_roundtrip(tdf, tm.assert_frame_equal, path=temp_h5_path, compression=compression)
...
def test_store_hierarchical(temp_h5_path, using_infer_string, multiindex_dataframe_random_data):
