...
def test_select_with_many_inputs(temp_hdfstore):
    ...
    # small selector
    result = temp_hdfstore.select("df", "ts>=Timestamp('2012-02-01') & users=['a','b','c']")
    expected = df[(df.ts >= Timestamp("2012-02-01")) & df.users.isin(["a", "b", "c"])]
    tm.assert_frame_equal(expected, result)

    # big selector along the columns
    selector = ["a", "b", "c"] + [f"a{i:03d}" for i in range(60)]
    result = temp_hdfstore.select("df", "ts>=Timestamp('2012-02-01') and users=selector")
...
def test_select_iterator_multiple(temp_hdfstore):
    ...
    results = list(temp_hdfstore.select_as_multiple(["df1", "df2"], selector="df1", chunksize=2))
    result = concat(results)
    tm.assert_frame_equal(expected, result)
...
def test_frame_select_complex(temp_hdfstore):
    ...
    result = temp_hdfstore.select("df", '(index>df.index[3] & index<=df.index[6]) | string="bar"')
...
def test_select_as_multiple(temp_hdfstore):
    ...
    with pytest.raises(KeyError, match=msg):
        temp_hdfstore.select_as_multiple(["df1", "df3"], where=["A>0", "B>0"], selector="df1")
    ...
    with pytest.raises(KeyError, match="'No object named df4 in the file'"):
        temp_hdfstore.select_as_multiple(["df1", "df2"], where=["A>0", "B>0"], selector="df4")

    # default select
    result = temp_hdfstore.select("df1", ["A>0", "B>0"])
    expected = temp_hdfstore.select_as_multiple(["df1"], where=["A>0", "B>0"], selector="df1")
    tm.assert_frame_equal(result, expected)
    expected = temp_hdfstore.select_as_multiple("df1", where=["A>0", "B>0"], selector="df1")
    tm.assert_frame_equal(result, expected)
...
def test_select_large_integer(temp_hdfstore):
    ...
    result = temp_hdfstore.select("data", where="y==-9223372036854775801").get("y").get(0)
    expected = df["y"][0]
...
def test_select_where_datetime_index_with_non_ns_resolution(temp_hdfstore, unit):
    ...
    result = temp_hdfstore.select("df", where="index >= '2020-01-02' & index <= '2020-01-03'")
