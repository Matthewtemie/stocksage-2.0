...
def test_append_with_strings(temp_hdfstore):
    def check_col(key, name, size):
        assert getattr(temp_hdfstore.get_storer(key).table.description, name).itemsize == size
...
def test_append_with_data_columns(temp_hdfstore):
    ...
    temp_hdfstore.remove("df")
    temp_hdfstore.append("df", df_new, data_columns=["string"], min_itemsize={"string": 30})
    check_col("df", "string", 30)
    temp_hdfstore.remove("df")
    temp_hdfstore.append("df", df_new, data_columns=["string"], min_itemsize=30)
    check_col("df", "string", 30)
    temp_hdfstore.remove("df")
    temp_hdfstore.append("df", df_new, data_columns=["string"], min_itemsize={"values": 30})
    check_col("df", "string", 30)
    ...
    result = temp_hdfstore.select("df", "string='foo' and string2='foo' and A>0 and B<0")
    expected = df_new[
    ...
    temp_hdfstore.append("df_dc", df_dc, data_columns=["B", "C", "string", "string2", "datetime"])
...
because its data contents are not [string] but [mixed] object dtype"""
    ...
    # series directly
    msg = re.escape(
        "cannot properly create the storer for: " "[group->df,value-><class 'pandas.Series'>]"
    )
    with pytest.raises(TypeError, match=msg):
...
def test_append_to_multiple(temp_hdfstore):
    ...
    with pytest.raises(ValueError, match=msg):
        temp_hdfstore.append_to_multiple({"df1": ["A", "B"], "df2": None}, df, selector="df3")
    ...
    # regular operation
    temp_hdfstore.append_to_multiple({"df1": ["A", "B"], "df2": None}, df, selector="df1")
    result = temp_hdfstore.select_as_multiple(["df1", "df2"], where=["A>0", "B>0"], selector="df1")
...
def test_append_to_multiple_dropna(temp_hdfstore):
    ...
    expected = df.dropna()
    tm.assert_frame_equal(result, expected, check_index_type=True)
    tm.assert_index_equal(temp_hdfstore.select("df1").index, temp_hdfstore.select("df2").index)
...
def test_append_to_multiple_dropna_false(temp_hdfstore):
    ...
    with pd.option_context("io.hdf.dropna_table", True):
        ...
        assert not temp_hdfstore.select("df1a").index.equals(temp_hdfstore.select("df2a").index)
