...
def test_start_stop_multiple(temp_hdfstore):
    ...
    temp_hdfstore.append_to_multiple({"selector": ["foo"], "data": None}, df, selector="selector")
...
def test_preserve_timedeltaindex_type(temp_hdfstore, unit):
    ...
    df.index = timedelta_range(start="0s", periods=10, freq="1s", name="example", unit=unit)
...
def test_select_categorical_string_columns(temp_hdfstore, model):
    ...
    df = DataFrame({"modelId": ["name", "longname", "longname"], "value": [1, 2, 3]}).astype(
        {"modelId": models, "value": int}
    )
