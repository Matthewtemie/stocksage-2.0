...
class TestS3:
    ...
    def test_parse_private_s3_bucket(self, s3_bucket_private_with_data, s3so, tips_df):
        ...
        pytest.importorskip("s3fs")
        df = read_csv(f"s3://{s3_bucket_private_with_data.name}/tips.csv", storage_options=s3so)
        tm.assert_frame_equal(df, tips_df)

    @pytest.mark.parametrize("scheme", ["s3n", "s3a"])
    def test_parse_public_bucket_s3n_s3a(self, s3_bucket_public_with_data, s3so, tips_df, scheme):
        nrows = 10
        df = read_csv(
    ...
    def test_infer_s3_compression(self, s3_bucket_public_with_data, s3so, tips_df, suffix):
        df = read_csv(
            f"s3://{s3_bucket_public_with_data.name}/tips.csv{suffix}",
            engine="python",
            compression="infer",
    ...
    @pytest.mark.single_cpu
    def test_read_csv_handles_boto_s3_object(self, s3_bucket_public_with_data, tips_file):
        # see gh-16135
    ...
    def test_read_feather_s3_file_path(self, s3_bucket_public_with_data, s3so, feather_file):
        # GH 29055
