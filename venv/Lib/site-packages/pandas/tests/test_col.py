...
def test_col_simple(expr: Expression, expected_values: list[object], expected_str: str) -> None:
...
def test_inplace_ops(op: Callable, expected_values: list[object], expected_str: str) -> None:
    # https://github.com/pandas-dev/pandas/pull/64267
...
def test_namespaces(expr: Expression, expected_values: list[object], expected_str: str) -> None:
    df = pd.DataFrame({"a": [datetime(2020, 1, 1)], "b": ["foo"]})
    result = df.assign(c=expr)
    expected = pd.DataFrame({"a": [datetime(2020, 1, 1)], "b": ["foo"], "c": expected_values})
...
def test_col_logical_ops(expr: Expression, expected_values: list[bool], expected_str: str) -> None:
...
def test_contains():
    ...
    with pytest.raises(TypeError, match="argument of type 'Expression' is not .*iterable"):
        1 in pd.col("a")
