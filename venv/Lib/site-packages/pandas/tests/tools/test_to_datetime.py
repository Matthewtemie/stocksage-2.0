...
class TestTimeConversionFormats:
    ...
    def test_to_datetime_format_YYYYMMDD_with_nat(self, cache):
        ...
        with pytest.raises(
            ValueError,
            match=('unconverted data remains when parsing with format "%Y%m%d": ".0". '),
        ):
...
class TestToDatetime:
    ...
    @pytest.mark.filterwarnings("ignore:Timestamp.utcnow is deprecated:DeprecationWarning")
    @pytest.mark.skipif(WASM, reason="tzset is not available on WASM")
    @pytest.mark.parametrize(
        "format, expected_ds",
        [
    ...
    def test_datetime_bool_arrays_mixed(self, cache):
        ...
        with pytest.raises(
            ValueError,
            match=(r'^time data "True" doesn\'t match format "%Y%m%d". ' f"{PARSING_ERR_MSG}$"),
        ):
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                (None, Timestamp("2012-07-26").as_unit("s")) + (NaT,) * start_caching_at,
    ...
    def test_convert_object_to_datetime_with_cache(self, datetimelikes, expected_values, exp_unit):
        # GH#39882
        ser = Series(
            datetimelikes,
            dtype="object",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [
                ...
                datetime(2020, 1, 1, 18).astimezone(zoneinfo.ZoneInfo("Australia/Melbourne")),
            ],
        ],
    )
...
class TestToDatetimeDataFrame:
    ...
    def test_dataframe_coerce(self, cache):
        ...
        msg = r'^cannot assemble the datetimes: time data ".+" doesn\'t ' r'match format "%Y%m%d"\.'
        with pytest.raises(ValueError, match=msg):
...
class TestToDatetimeMisc:
    ...
    def test_to_datetime_with_space_in_series(self, cache):
        ...
        msg = r'^time data " " doesn\'t match format "%m/%d/%Y". ' rf"{PARSING_ERR_MSG}$"
        with pytest.raises(ValueError, match=msg):
    ...
    def test_to_datetime_float_with_nans_floating_point_error(self):
        ...
        expected = Series([NaT] * 1000 + [Timestamp("2024-04-04 08:23:53")], dtype="datetime64[s]")
        tm.assert_series_equal(result, expected)
    ...
    def test_dayfirst_warnings_valid_input(self):
        ...
        expected = DatetimeIndex(["2014-12-31", "2011-03-10"], dtype="datetime64[us]", freq=None)
    ...
    def test_dayfirst_warnings_invalid_input(self):
        ...
        with pytest.raises(
            ValueError,
            match=(
                r'^time data "03/30/2011" doesn\'t match format ' rf'"%d/%m/%Y". {PARSING_ERR_MSG}$'
            ),
        ):
...
class TestToDatetimeInferFormat:
    ...
    def test_to_datetime_consistent_format(self, cache):
        ...
        ser = Series(np.array(data))
        result = to_datetime(ser, cache=cache)
        expected = Series(["2011-01-01", "2011-02-01", "2011-03-01"], dtype="datetime64[us]")
        tm.assert_series_equal(result, expected)
    ...
    def test_to_datetime_series_with_nans(self, cache):
        ...
        result = to_datetime(ser, cache=cache)
        expected = Series(["2011-01-01", NaT, "2011-01-03", NaT], dtype="datetime64[us]")
        tm.assert_series_equal(result, expected)
