...
def test_apply_category_equalness(val):
    ...
    expected = Series([False if pd.isnull(x) else x == val for x in df_values], name="a")
...
def test_dictlike_datetime_index_unsorted():
    ...
    result = df.apply({"a": lambda x: x, "b": lambda x: x.shift(freq="D")}, by_row=False)
    expected = DataFrame(
        {
            "a": [1.0, 2.0, np.nan],
            "b": [4.0, np.nan, 3.0],
...
def test_result_type_broadcast(int_frame_const_col, request, engine):
    ...
    if engine is MockEngineDecorator:
        with pytest.raises(
            ...
            df.apply(lambda x: [1, 2, 3], axis=1, result_type="broadcast", engine=engine)
    else:
        # broadcast result
