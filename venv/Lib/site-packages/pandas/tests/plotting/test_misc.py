...
class TestDataFramePlots:
    ...
    def test_andrews_curves_cmap(self, request, df):
        ...
        ax = _check_plot_works(plotting.andrews_curves, frame=df, class_column="Name", color=cmaps)
    ...
    def test_parallel_coordinates_line_diff(self, iris):
        ...
        ax = _check_plot_works(plotting.parallel_coordinates, frame=df, class_column="Name")
...
def _df_bar_subplot_checker(df_bar_data, df_bar_df, subplot_data_df, subplot_columns):
    subplot_sliced_by_source = [
        subplot_data_df.iloc[len(df_bar_data) * i : len(df_bar_data) * (i + 1)].reset_index()
...
def test_bar_1_subplot_1_double_stacked(df_bar_data, df_bar_df, columns_used):
    ...
    subplot_data_df_list = _df_bar_xyheight_from_ax_helper(df_bar_data, ax, subplot_division)
...
@pytest.mark.parametrize("columns_used", [["A", "B", "C"], ["A", "C", "B"], ["D", "A", "C"]])
...
def test_plot_bar_label_count_default():
    df = DataFrame([(30, 10, 10, 10), (20, 20, 20, 20), (10, 30, 30, 10)], columns=list("ABCD"))
    df.plot(subplots=True, kind="bar", title=["A", "B", "C", "D"])


def test_plot_bar_label_count_expected_fail():
    df = DataFrame([(30, 10, 10, 10), (20, 20, 20, 20), (10, 30, 30, 10)], columns=list("ABCD"))
    error_regex = re.escape("The number of titles (4) must equal the number of subplots (3).")
