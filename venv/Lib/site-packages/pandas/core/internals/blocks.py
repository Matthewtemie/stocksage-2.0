...
class Block(PandasObject, libinternals.Block):
    ...
    def convert(self) -> list[Block]:
        ...
        if res_values is values or (
            isinstance(res_values, NumpyExtensionArray) and res_values._ndarray is values
        ):
            refs = self.refs
    ...
    def convert_dtypes(
        ...
        for blk in blks:
            ...
            sub_blks = [blk] if blk.ndim == 1 or blk.shape[0] == 1 else list(blk._split())
            dtypes = [
                convert_dtypes(
                    b.values,
                    convert_string,
        ...
        for i, ((src, dest), mask) in enumerate(zip(pairs, masks, strict=True)):
            ...
            for blk_num, blk in enumerate(rb):
                ...
                if i != src_len:
                    ...
                    self_blk_ids = {id(b()): i for i, b in enumerate(self.refs.referenced_blocks)}
                    for b in result:
    ...
    def shift(self, periods: int, fill_value: Any = None) -> list[Block]:
        ...
        try:
            ...
            if self.dtype.kind not in "iub" or not is_valid_na_for_dtype(fill_value, self.dtype):
                # GH#53802
                warnings.warn(
