...
class BaseGrouper:
    ...
    def indices(self) -> dict[Hashable, npt.NDArray[np.intp]]:
        ...
        if not self.dropna:
            ...
            if not has_mi and self.result_index.hasnans:
                result = {np.nan if isna(key) else key: value for key, value in result.items()}
            elif has_mi:
    ...
    def result_index_and_ids(self) -> tuple[Index, npt.NDArray[np.intp]]:
        levels = [Index._with_infer(ping.uniques, copy=False) for ping in self.groupings]
        ...
        if len(self.groupings) == 1:
            ...
            if any(sorts):
                ...
                n_levels = len(sorts)
                drop_levels = [
                    n_levels - idx for idx, sort in enumerate(reversed(sorts), 1) if not sort
                ]
