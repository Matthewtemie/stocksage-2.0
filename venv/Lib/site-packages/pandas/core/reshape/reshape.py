    ...
    def _repeater(self) -> np.ndarray:
        ...
        if len(self.removed_level_full) != len(self.removed_level):
            ...
            if self.has_nan and not self.sort:
                assert self.unique_nan_index > -1, "`unique_nan_index` not properly initialized"
...
def stack(frame: DataFrame, level=-1, dropna: bool = True, sort: bool = True) -> Series | DataFrame:
    ...
    if isinstance(frame.columns, MultiIndex):
        ...
        levels, (ilab, clab) = zip(*map(stack_factorize, (frame.index, frame.columns)), strict=True)
...
def stack_v3(frame: DataFrame, level: list[int]) -> Series | DataFrame:
    ...
    if not isinstance(frame.columns, MultiIndex):
        ...
        if len(frame.columns) > 0 and frame._is_homogeneous_type:
            ...
            dtype = None
        result = frame._constructor_sliced(frame._values.reshape(-1, order="F"), dtype=dtype)
    else:
    ...
    for idx in stack_cols.unique():
        if len(frame.columns) == 1:
            ...
            column_indexer = tuple(
                next(gen) if k in set_levels else slice(None) for k in range(frame.columns.nlevels)
            )
