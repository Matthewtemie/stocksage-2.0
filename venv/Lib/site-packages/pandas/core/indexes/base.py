...
class Index(IndexOpsMixin, PandasObject):
    ...
    def _wrap_join_result(
        ...
        if lidx is None:
            ...
            join_index = self._constructor._with_infer(joined, dtype=self.dtype, copy=False)
    ...
    def _get_join_target(self) -> np.ndarray:
        ...
        elif isinstance(self._values, ArrowExtensionArray) and self.dtype.kind not in "mM":
    ...
    def get_slice_bound(self, label, side: Literal["left", "right"]) -> int:
        ...
        if isinstance(slc, np.ndarray):
            ...
            if isinstance(slc, np.ndarray):
                raise KeyError(
                    f"Cannot get {side} slice bound for non-unique " f"label: {original_label!r}"
                )
