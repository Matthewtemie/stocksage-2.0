...
class MultiIndex(Index):
    ...
    def levels(self) -> FrozenList:
        ...
        result = [x._rename(name=name) for x, name in zip(self._levels, self._names, strict=True)]
        for level in result:
    ...
    def _get_values_for_csv(self, *, na_rep: str = "nan", **kwargs) -> npt.NDArray[np.object_]:
        ...
        if len(new_levels) == 1:
            ...
            return Index(new_levels[0].take(new_codes[0]), copy=False)._get_values_for_csv()
        else:
    ...
    def argsort(self, *args, na_position: NaPosition = "last", **kwargs) -> npt.NDArray[np.intp]:
        """
    ...
    def _recode_for_new_levels(self, new_levels, *, copy: bool) -> Generator[np.ndarray]:
