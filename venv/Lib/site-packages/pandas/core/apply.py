...
def frame_apply(
    ...
    if axis == 0:
        ...
        if columns:
            raise NotImplementedError(f"Named aggregation is not supported when {axis=}.")
...
class Apply(metaclass=abc.ABCMeta):
    ...
    def wrap_results_dict_like(
        ...
        if all(is_ndframe):
            ...
            keys_to_use = [k for k, v in zip(result_index, result_data, strict=True) if not v.empty]
...
class FrameRowApply(FrameApply):
    ...
    def apply_with_numba(self) -> dict[int, Any]:
        ...
        args, kwargs = prepare_function_arguments(func, self.args, self.kwargs, num_required_args=1)
...
def reconstruct_func(
    ...
    **kwargs: dict, kwargs used in is_multi_agg_with_relabel and
    ...
    relabeling = func is None and (
        is_multi_agg_with_relabel(**kwargs) or any(isinstance(v, NamedAgg) for v in kwargs.values())
    )
        ...
        for key, val in kwargs.items():
            if isinstance(val, NamedAgg):
                ...
                if val.args or val.kwargs:
                    aggfunc = lambda x, func=aggfunc, a=val.args, kw=val.kwargs: func(x, *a, **kw)
