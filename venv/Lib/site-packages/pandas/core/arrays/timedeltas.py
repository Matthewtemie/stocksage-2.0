...
class TimedeltaArray(dtl.TimelikeOps):
    ...
    @classmethod
    def _generate_range(cls, start, end, periods, freq, closed=None, *, unit: TimeUnit) -> Self:
    ...
    def __mul__(self, other) -> Self:
        ...
        if other.dtype.kind == "b":
            # GH#58054
            raise TypeError(
                f"Cannot multiply '{self.dtype}' by bool, explicitly cast to " "integers instead"
            )
    ...
    def __truediv__(self, other):
        ...
        if is_object_dtype(other.dtype):
            ...
            if self.ndim > 1:
                res_cols = [left / right for left, right in zip(self, other, strict=True)]
...
def sequence_to_td64ns(
    ...
    if data.dtype == object or is_string_dtype(data.dtype):
        ...
        if unit is not None and unit != "ns":
            ...
            if all_round:
                result, _ = sequence_to_td64ns(int_data, copy=False, unit=unit, errors=errors)
                result[mask] = iNaT
