...
class ArrowExtensionArray(
        ...
        if isinstance(value, cls):
            ...
            if pa_type is not None and pa.types.is_timestamp(pa_type):
                ...
                if isinstance(value, DatetimeArray):
                    ...
                    dta = DatetimeArray._from_sequence(value, copy=copy, dtype=pass_dtype)
                dta_mask = dta.isna()
            ...
            if is_nan_na():
                try:
                    ...
                    mask = isna(arr_value)
            elif getattr(value, "dtype", None) is None or value.dtype.kind not in "iumMf":
    ...
    def _evaluate_op_method(self, other, op, arrow_funcs) -> Self:
        ...
        if (
            ...
            if op in [operator.add, roperator.radd]:
                ...
                elif pa.types.is_large_string(pa_type) and pa.types.is_string(other.type):
    ...
    def _arith_method(self, other, op) -> Self | npt.NDArray[np.object_]:
        if (
            ...
            # GH#61940
            return np.array(
                [op(x, other) if isinstance(x, str) else self.dtype.na_value for x in self],
                dtype=object,
            )
    ...
    def _pad_or_backfill(
        ...
        *,
        ...
        if limit is None and limit_area is None:
            ...
            try:
                if method == "pad":
                    return self._from_pyarrow_array(pc.fill_null_forward(self._pa_array))
                elif method == "backfill":
                    return self._from_pyarrow_array(pc.fill_null_backward(self._pa_array))
            except pa.ArrowNotImplementedError:
    ...
    def fillna(
        ...
        try:
            return self._from_pyarrow_array(_safe_fill_null(self._pa_array, fill_value=fill_value))
        except pa.ArrowNotImplementedError:
            # ArrowNotImplementedError: Function 'coalesce' has no kernel
            #   matching input types (duration[ns], duration[ns])
            # TODO: remove try/except wrapper if/when pyarrow implements
    ...
    def factorize(
        ...
        if encoded.length() == 0:
            ...
            uniques = self._from_pyarrow_array(pa.chunked_array([], type=encoded.type.value_type))
        else:
            # GH 54844
    ...
    def to_numpy(
        ...
        if pa.types.is_timestamp(pa_type) or pa.types.is_duration(pa_type):
            ...
            and (
                na_value is np.nan
                or (original_na_value is lib.no_default and is_float_dtype(dtype) and is_nan_na())
            )
        ):
