...
class StringDtype(StorageExtensionDtype):
    ...
    def __init__(
        ...
        if storage == "pyarrow" and not HAS_PYARROW:
            raise ImportError(
                f"pyarrow>={PYARROW_MIN_VERSION} is required for PyArrow " "backed StringArray."
            )
...
class BaseStringArray(ExtensionArray):
    ...
    def _str_map_nan_semantics(self, f, na_value=lib.no_default, dtype: Dtype | None = None):
        if dtype is None:
            dtype = self.dtype
        if na_value is lib.no_default:
...
class StringArray(BaseStringArray, NumpyExtensionArray):  # type: ignore[misc]
    ...
    def __init__(self, values, *, dtype: StringDtype | None = None, copy: bool = False) -> None:
        if dtype is None:
            dtype = StringDtype()
