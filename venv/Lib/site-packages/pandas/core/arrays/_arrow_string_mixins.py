...
class ArrowStringArrayMixin:
    ...
    def _has_unsupported_regex(pat: str | re.Pattern) -> bool:
        ...
        def has_unsupported_code(tokens):
            ...
            for op_code, argument in tokens:
                if (
                    (op_code == _parser.SUBPATTERN and has_unsupported_code(argument[3]))
                    or (
                        op_code == _parser.BRANCH
                        and any(has_unsupported_code(tokens) for tokens in argument[1])
                    )
                    or (op_code in [_parser.ASSERT_NOT, _parser.ASSERT, _parser.GROUPREF])
                ):
                    return True
            return False
    ...
    def _str_slice_replace(
        ...
        return self._from_pyarrow_array(pc.utf8_replace_slice(self._pa_array, start, stop, repl))
