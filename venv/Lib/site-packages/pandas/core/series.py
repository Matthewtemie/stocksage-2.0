...
class Series(base.IndexOpsMixin, NDFrame):  # type: ignore[misc]
    ...
    @deprecate_nonkeyword_arguments(Pandas4Warning, allowed_args=["self", "buf"], name="to_string")
    def to_string(
        self,
        ...
        if not isinstance(result, str):
            raise AssertionError(
                "result must be of type str, type " f"of result is {type(result).__name__!r}"
            )
    ...
    def cov(
        ...
        this_values = this.to_numpy(dtype=float, na_value=np.nan, copy=False)
        other_values = other.to_numpy(dtype=float, na_value=np.nan, copy=False)
        result = nanops.nancov(this_values, other_values, min_periods=min_periods, ddof=ddof)
        result = maybe_unbox_numpy_scalar(result)
        return result
    ...
    def diff(self, periods: int = 1) -> Series:
        ...
        return self._constructor(result, index=self.index.view(), copy=False).__finalize__(
            self, method="diff"
        )
