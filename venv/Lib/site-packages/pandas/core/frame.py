...
class DataFrame(NDFrame, OpsMixin):
    ...
    @classmethod
    def from_arrow(cls, data: ArrowArrayExportable | ArrowStreamExportable) -> DataFrame:
        """
    ...
    def select_dtypes(self, include=None, exclude=None) -> DataFrame:
        ...
        * To select Pandas datetimetz dtypes, use ``'datetimetz'``
        ...
        if (
            ...
            and any(
                isinstance(dtype, StringDtype) and dtype.na_value is np.nan for dtype in blk_dtypes
            )
        ):
            # GH#61916
            warnings.warn(
    ...
    def _should_reindex_frame_op(self, right, op, axis: int, fill_value, level) -> bool:
        ...
        if (
            (isinstance(self.columns, MultiIndex) or isinstance(right.columns, MultiIndex))
    ...
    def update(
        ...
        """
        if not CHAINED_WARNING_DISABLED:
            if sys.getrefcount(self) <= REF_COUNT_METHOD and not com.is_local_in_caller_frame(self):
                warnings.warn(
                    _chained_assignment_method_update_msg,
                    ChainedAssignmentError,
                    stacklevel=2,
        ...
        **kwargs
        ...
        if engine is None or isinstance(engine, str):
            ...
            if result_type is not None:
                raise NotImplementedError(f"{result_type=} only implemented for the default engine")
    ...
    def corrwith(
        ...
        if not drop:
            ...
            if len(idx_diff) > 0:
                correl = correl._append_internal(Series([np.nan] * len(idx_diff), index=idx_diff))
    ...
    @deprecate_nonkeyword_arguments(Pandas4Warning, allowed_args=["self"], name="median")
    def median(
        self,
        axis: Axis | None = 0,
        skipna: bool = True,
...
def _from_nested_dict(
    ...
    new_data: collections.defaultdict[HashableT2, dict[HashableT, T]] = collections.defaultdict(
        dict
    )
