...
class _TableBuilderVerboseMixin(_TableBuilderAbstract):
    ...
    def _get_gross_column_widths(self) -> Sequence[int]:
        ...
        return [
            max(*widths)
            for widths in zip(self.header_column_widths, body_column_widths, strict=False)
        ]
    ...
    def add_header_line(self) -> None:
        header_line = self.SPACING.join(
            [
                ...
                for header, col_width in zip(self.headers, self.gross_column_widths, strict=True)
            ]
        )
        ...
        for row in self.strrows:
            body_line = self.SPACING.join(
                [
                    ...
                    for col, gross_colwidth in zip(row, self.gross_column_widths, strict=True)
                ]
            )
