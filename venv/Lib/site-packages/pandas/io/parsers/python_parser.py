...
class PythonParser(ParserBase):
    ...
    def read(self, rows: int | None = None) -> tuple[
        ...
        indexnamerow = None
        if self.has_index_names and sum(int(v == "" or v is None) for v in content[0]) == len(
            columns
        ):
    ...
    def _alert_malformed(self, msg: str, row_num: int) -> None:
        ...
        if self.on_bad_lines == self.BadLineHandleMethod.WARN or callable(self.on_bad_lines):
            warnings.warn(
    ...
    def _rows_to_cols(self, content: list[list[Scalar]]) -> list[np.ndarray]:
        ...
        if max_len > col_len and self.index_col is not False and self.usecols is None:
            ...
            for row_num, actual_len, source in bad_lines:
                ...
                elif self.delimiter and len(self.delimiter) > 1 and self.quoting != csv.QUOTE_NONE:
                    # see gh-13374
                    reason = (
