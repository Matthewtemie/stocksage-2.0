...
def is_type_ignore_comment(leaf: Leaf, mode: Mode) -> bool:
    ...
    t = leaf.type
    v = leaf.value
    return t in {token.COMMENT, STANDALONE_COMMENT} and is_type_ignore_comment_string(v, mode)


def is_type_ignore_comment_string(value: str, mode: Mode) -> bool:
    """Return True if the given string match with type comment with
    ignore annotation."""
    return is_type_comment_string(value, mode) and value.split(":", 1)[1].lstrip().startswith(
        "ignore"
    )
