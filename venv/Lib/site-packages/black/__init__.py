...
@click.option(
    "--no-cache",
    is_flag=True,
    help=("Skip reading and writing the cache, forcing Black to reformat all" " included files."),
)
@click.pass_context
def main(
    ctx: click.Context,
...
def get_features_used(node: Node, *, future_imports: set[str] | None = None) -> set[Feature]:
...
def is_unpacking_comprehension(node: LN) -> bool:
    ...
    if not any(child.type in {syms.comp_for, syms.old_comp_for} for child in node.children):
        return False
