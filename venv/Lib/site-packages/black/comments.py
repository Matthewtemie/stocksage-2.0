...
def generate_comments(leaf: LN, mode: Mode) -> Iterator[Leaf]:
    ...
    """
    total_consumed = 0
    for pc in list_comments(leaf.prefix, is_endmarker=leaf.type == token.ENDMARKER, mode=mode):
...
def _should_process_fmt_comment(comment: ProtoComment, leaf: Leaf) -> tuple[bool, bool, bool]:
...
def _handle_comment_only_fmt_block(
    ...
    standalone_comment_prefix = original_prefix[:pre_fmt_off_consumed] + "\n" * comment.newlines
...
def convert_one_fmt_off_pair(node: Node, mode: Mode, lines: Collection[tuple[int, int]]) -> bool:
    ...
    for leaf in node.leaves():
        ...
        if leaf.type == STANDALONE_COMMENT and hasattr(leaf, "fmt_pass_converted_first_leaf"):
            continue

        previous_consumed = 0
        for comment in list_comments(leaf.prefix, is_endmarker=False, mode=mode):
            should_process, is_fmt_off, is_fmt_skip = _should_process_fmt_comment(comment, leaf)
            if not should_process:
                previous_consumed = comment.consumed
                continue

            if not _is_valid_standalone_fmt_comment(comment, leaf, is_fmt_off, is_fmt_skip):
                previous_consumed = comment.consumed
                continue
            ...
            if not ignored_nodes and is_fmt_off:
                if _handle_comment_only_fmt_block(leaf, comment, previous_consumed, mode):
                    return True
                continue
...
def _handle_regular_fmt_block(
    ...
    leaf_is_ignored = any(
        ignored is leaf
        or (isinstance(ignored, Node) and any(child is leaf for child in ignored.leaves()))
        ...
        if children_contains_fmt_on(container, mode=mode):
            for index, child in enumerate(container.children):
                ...
                if (
                    ...
                    and children_contains_fmt_on(container.children[index + 1], mode=mode)
                ):
...
def _should_keep_compound_statement_inline(body_node: Node, simple_stmt_parent: Node) -> bool:
    ...
    for leaf in body_node.leaves():
        if leaf.type == token.SEMI:
            ...
            if body_node.type == syms.suite:
                ...
                simple_stmts = [
                    child for child in body_node.children if child.type == syms.simple_stmt
                ]
    ...
    if parent is not None and parent.type == syms.suite and leaf.type == token.NEWLINE:
        ...
        while "\n" not in current_node.prefix and current_node.prev_sibling is not None:
            ...
            if current_node.type in (token.NEWLINE, token.INDENT):
                if not list_comments(current_node.prefix, is_endmarker=False, mode=mode):
                    current_node.prefix = ""
                break
