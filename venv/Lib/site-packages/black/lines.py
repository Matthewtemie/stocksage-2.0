...
class EmptyLineTracker:
    ...
    @staticmethod
    def _find_adjacent_decorated(node: Node | Leaf, *, reverse: bool = False) -> Node | None:
    ...
    @staticmethod
    def _if_stmt_branch_has_func_named(if_stmt: Node, exclude_suite: Node, name: str) -> bool:
    ...
    def _block_is_part_of_overload_group(line: Line) -> bool:
        ...
        return EmptyLineTracker._if_stmt_branch_has_func_named(if_stmt, suite, func_name)
    ...
    def _is_decorator_in_conditional_overload(line: Line) -> bool:
        ...
        for reverse in (True, False):
            adjacent = EmptyLineTracker._find_adjacent_decorated(if_stmt, reverse=reverse)
            if adjacent is not None and EmptyLineTracker._decorated_node_has_func_named(
                adjacent, name
            ):
                return True
    ...
    def maybe_empty_lines(self, current_line: Line) -> LinesBlock:
        ...
        overload_groups = self.mode.is_pyi and Preview.pyi_overload_group_blank_lines in self.mode
        if overload_groups:
            if current_line.is_def and self._is_line_decorated(current_line):
                ...
                if name is not None:
                    prev = self._pyi_previous_decorated_func
                    is_multi = (
                        prev is not None and prev.name == name and prev.depth == current_line.depth
                    )
                ...
                and (
                    ...
                    or (
                        ...
                        and not (
                            current_line.leaves and current_line.leaves[0].value in ("else", "elif")
                        )
                    )
                )
            ):
    ...
    if rhs.head.leaves:
        ...
        if head_comments:
            has_type_ignore_in_head = any(
                is_type_ignore_comment(comment, mode=rhs.head.mode) for comment in head_comments
            )
            has_other_comment_in_head = any(
                not is_type_ignore_comment(comment, mode=rhs.head.mode) for comment in head_comments
            )
