        ...
        def _wrapper(*args, **kwargs):
            ...
            if isinstance(obj_getter, int):
                ...
                if obj_getter in kwargs:
                    ...
                    pos_args[obj_getter] = schema.validate(pos_args[obj_getter], *validate_args)
                ...
                try:
                    ...
                    if obj_arg_name in kwargs:
                        ...
                        kwargs[obj_arg_name] = schema.validate(obj, *validate_args)
                    elif obj_arg_name in pos_args:
                        obj = args[arg_idx]
                        args[arg_idx] = schema.validate(obj, *validate_args)
                    else:
                        raise ValueError(f"argument {obj_arg_name} not found in args or kwargs")
                except errors.SchemaError as e:
                    _handle_schema_error("check_input", wrapped, schema, obj, e)
            else:
...
def check_output(
    ...
    if callable(obj_getter) and (
        ...
        raise ValueError("Cannot use callable obj_getter when the schema uses coercion.")

    def validate(out: Any, fn: Callable) -> None:
        def _try_validate(obj: Any):
            try:
                return schema.validate(obj, head, tail, sample, random_state, lazy, inplace)
            except errors.SchemaError as e:
    ...
    *,
    ...
    class _AnnotationInfoWithModelTree:
        def __init__(
            ...
            if children and schema_model:
                raise ValueError("At most one of children or schema_model should be set")
        ...
        def child_at_index(self, index: int) -> Union["_AnnotationInfoWithModelTree", None]:
            """
        ...
        def from_annotation(
            ...
            if annotation_info.is_generic_model:
                ...
                return _AnnotationInfoWithModelTree(annotation_info=annotation_info, children=None)
    ...
    def _get_annotated_schema_models(
        ...
        return {
            ...
            for arg_name, annotation in get_type_hints(wrapped, include_extras=True).items()
        }
    ...
    def _check_arg_value_against_model(
        ...
        if schema_model is None or (annotation_info.optional and arg_value is None):
            return arg_value
        ...
        if isinstance(arg_value, (int, str, bool, float, dict, list, tuple, set)):
            return arg_value
    ...
    def _check_arg_value_against_union(
        ...
        if schema_errors:
            raise errors.SchemaErrors(
                schema=child.dataframe_model.to_schema() if child.dataframe_model else None,
    ...
    def _check_arg_value_against_tuple(
        ...
        for child_arg_value, child_annotation_model_tree in zip(arg_value, tuple_child_nodes):
    ...
    def _check_arg_value(
        ...
        if annotation_model_tree.annotation_info.origin == Union:
            return _check_arg_value_against_union(arg_value, annotation_model_tree.children or [])
        ...
        #       This is important because we want to match both list and List, for example.
            ...
            return _check_arg_value_against_tuple(arg_value, annotation_model_tree.children or [])
        elif annotation_model_tree.annotation_info.origin in [list, "List"]:
            return _check_arg_value_against_list(arg_value, annotation_model_tree.child_at_index(0))
        elif annotation_model_tree.annotation_info.origin in [dict, "Dict"]:
            return _check_arg_value_against_dict(arg_value, annotation_model_tree.child_at_index(1))
        else:
            return _check_arg_value_against_model(
                arg_value,
    ...
    def validate_args(named_arguments: dict[str, Any], arguments: tuple[Any, ...]) -> list[Any]:
        ...
        if len(arguments) > len(named_arguments):
            ...
            star_args_tuple = (
                _check_arg(star_args_name, arg_value) for arg_value in star_args_values
            )

            explicit_args_tuple = (
                _check_arg(arg_name, arg_value) for arg_name, arg_value in named_arguments.items()
            )
