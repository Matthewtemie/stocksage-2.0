...
class _PhysicalNumber(_Number):
    ...
    _base_name: str | None = dataclasses.field(default=None, init=False, repr=False)
...
class Float(_PhysicalNumber):  # type: ignore
    ...
    def check(
        ...
        return isinstance(pandera_dtype, Float) and self.bit_width == pandera_dtype.bit_width
...
class Complex(_PhysicalNumber):  # type: ignore
    ...
    def check(
        ...
        return isinstance(pandera_dtype, Complex) and self.bit_width == pandera_dtype.bit_width
...
class Decimal(_Number):
    ...
    rounding: str | None = dataclasses.field(default_factory=lambda: decimal.getcontext().rounding)
    """
    ...
    def __init__(
        ...
        if scale is not None and scale > precision:
            raise ValueError(f"Decimal scale {scale} must be between 0 and {precision}.")
...
class Category(DataType):  # type: ignore
    ...
    def __init__(self, categories: Iterable[Any] | None = None, ordered: bool = False):
