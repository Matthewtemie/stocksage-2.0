...
def get_backend_types(check_cls_fqn: str) -> BackendTypes:
    ...
    if mod_name != "pandera":
        if cls_name not in PANDAS_LIKE_CLS_NAMES:
            raise BackendNotFoundError(f"cls_name {cls_name} not in {PANDAS_LIKE_CLS_NAMES}")
...
def is_table(obj):
    ...
    return backend_types is not None and isinstance(obj, backend_types.dataframe_datatypes)
...
def is_field(obj):
    ...
    """
    backend_types = get_backend_types_from_mro(type(obj))
    return backend_types is not None and isinstance(obj, backend_types.series_datatypes)


def is_index(obj):
    """Verifies whether an object is a table index."""
    backend_types = get_backend_types_from_mro(type(obj))
    return backend_types is not None and isinstance(obj, backend_types.index_datatypes)


def is_multiindex(obj):
    """Verifies whether an object is a multi-level table index."""
    backend_types = get_backend_types_from_mro(type(obj))
    return backend_types is not None and isinstance(obj, backend_types.multiindex_datatypes)
