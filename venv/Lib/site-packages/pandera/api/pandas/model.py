    ...
    *new in 0.5.0*
    ...
    def _build_columns_index(
        ...
        **multiindex_kwargs: Any,
        ...
        index_count = sum(
            annotation.origin in get_index_types() for annotation, _ in fields.values()
        )
        ...
        for field_name, (annotation, field) in fields.items():
            ...
            if (
                ...
                dtype = _get_nullable_coercion_dtype(
                    ...
                    coerce=field.coerce or bool(getattr(cls.__config__, "coerce", False)),
                )
                ...
                raise SchemaInitError(
                    f"Invalid annotation '{field_name}: " f"{annotation.raw_annotation}'"
                )
    ...
    def empty(cls: type[Self], *_args) -> DataFrame[Self]:
        ...
        if isinstance(schema.index, MultiIndex):
            ...
            index = pd.Index(
                [],
                dtype=schema.index.dtype.type if schema.index.dtype is not None else None,
...
def _build_schema_index(indices: list[Index], **multiindex_kwargs: Any) -> SchemaIndex | None:
