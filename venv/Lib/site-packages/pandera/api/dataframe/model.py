...
class _ChecksDescriptor(_ClassDescriptor):
    def __get__(self, obj, cls) -> TChecks:
        if self.cache.get(cls) is None:
            check_infos = typing.cast(list[FieldCheckInfo], cls._collect_check_infos(CHECK_KEY))
...
class _ParsersDescriptor(_ClassDescriptor):
    def __get__(self, obj, cls) -> TParsers:
        if self.cache.get(cls) is None:
            parser_infos = typing.cast(list[FieldParserInfo], cls._collect_parser_infos(PARSER_KEY))
...
class DataFrameModel(Generic[TDataFrame, TSchema], BaseModel):
    ...
    def __class_getitem__(
        ...
        parameterized_name = f"{cls.__name__}[{', '.join(p.__name__ for p in item)}]"
    ...
    def validate(
        ...
        return cast(
            DataFrameBase[Self],
            cls.to_schema().validate(check_obj, head, tail, sample, random_state, lazy, inplace),
        )
        ...
        for field_name, annotation in annotations.items():
            ...
            if not isinstance(field, FieldInfo):
                raise SchemaInitError(
                    f"'{field_name}' can only be assigned a 'Field', " + f"not a '{type(field)}.'"
                )
    ...
    def _collect_config_and_extras(
        ...
        for model in models:
            ...
            base_options, base_extras = cls._extract_config_options_and_extras(config)
    ...
    def _extract_checks(
        ...
        for check_info in check_infos:
            check_info_fields = {
                field.name if isinstance(field, FieldInfo) else field for field in check_info.fields
            }
            if check_info.regex:
