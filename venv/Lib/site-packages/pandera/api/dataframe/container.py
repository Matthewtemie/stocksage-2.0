...
class DataFrameSchema(Generic[TDataObject], BaseSchema):
    ...
    def get_dtypes(self, check_obj: TDataObject) -> dict[str, DataType]:
        ...
        for _, column in self.columns.items():
            ...
            if column.regex:
                regex_dtype.update(
                    {c: column.dtype for c in backend.get_regex_columns(column, check_obj)}
                )
        return {
    ...
    def __str__(self) -> str:
        ...
        if len(index_) == 1:
            ...
            index = "\n".join(x if i == 0 else f"{indent}{x}" for i, x in enumerate(index_))
    ...
    def remove_columns(self, cols_to_remove: list[str]) -> Self:
        ...
        not_in_cols: list[str] = [x for x in cols_to_remove if x not in schema_copy.columns.keys()]
        if not_in_cols:
    ...
    def update_column(self, column_name: str, **kwargs) -> Self:
        ...
        column_copy = copy.deepcopy(schema.columns[column_name]).set_name(column_name)
    ...
    def update_columns(
        ...
        for col in new_schema.columns:
            ...
            if update_dict.get(col):
                ...
                new_columns[col] = new_schema.columns[col].__class__(**original_properties)
    ...
    def rename_columns(self, rename_dict: dict[str, str]) -> Self:
        ...
        new_columns = {
            (rename_dict[col_name] if col_name in rename_dict else col_name): (
                col_attrs.set_name(rename_dict[col_name]) if col_name in rename_dict else col_attrs
            )
    ...
    def update_index(self, index_name: str, **kwargs) -> Self:
        ...
        if index_name not in schema.index.names:
            raise errors.SchemaInitError(f"index '{index_name}' not in {schema}")
    ...
    def update_indexes(self, update_dict: dict[str, dict[str, Any]]) -> Self:
        ...
        not_in_cols: list[str] = [x for x in update_dict.keys() if x not in schema.index.names]
        if not_in_cols:
            raise errors.SchemaInitError(f"Keys {not_in_cols} not found in schema indexes!")
    ...
    def rename_indexes(self, rename_dict: dict[str, str]) -> Self:
        ...
        # ensure all specified keys are present in the index
        not_in_cols: list[str] = [x for x in rename_dict.keys() if x not in new_schema.index.names]
        if not_in_cols:
            raise errors.SchemaInitError(f"Keys {not_in_cols} not found in schema indexes!")
    ...
    def select_columns(self, columns: list[Any]) -> Self:
        ...
        *New in version 0.4.5*
        ...
        new_schema = copy.deepcopy(self)
        # ensure all specified keys are present in the columns
        not_in_cols: list[str] = [x for x in columns if x not in new_schema.columns.keys()]
        if not_in_cols:
            raise errors.SchemaInitError(f"Keys {not_in_cols} not found in schema columns!")

        new_columns = {col_name: new_schema.columns[col_name] for col_name in columns}

        new_schema.columns = new_columns
        return cast(Self, new_schema)

    def set_index(self, keys: list[str], drop: bool = True, append: bool = False) -> Self:
        """
        ...
        # ensure all specified keys are present in the columns
        not_in_cols: list[str] = [x for x in keys_temp if x not in new_schema.columns.keys()]
        if not_in_cols:
            raise errors.SchemaInitError(f"Keys {not_in_cols} not found in schema columns!")
    ...
    def reset_index(self, level: list[str] | None = None, drop: bool = False) -> Self:
        ...
        if new_schema.index is None:
            raise errors.SchemaInitError("There is currently no index set for this schema.")
        ...
        level_not_in_index: Union[list[Any], list[str], None] = (
            ...
            if isinstance(new_schema.index, MultiIndex) and level_temp
                ...
                []
                if isinstance(new_schema.index, Index) and (level_temp == [new_schema.index.name])
                else level_temp
            )
        )
        if level_not_in_index:
            raise errors.SchemaInitError(f"Keys {level_not_in_index} not found in schema columns!")
    ...
    for column_name, column in column_dict.items():
        for check in column.checks:
            if check.groupby is None or callable(check.groupby):
                continue
            nonexistent_groupby_columns = [c for c in check.groupby if c not in column_dict]
...
def _columns_renamed(
    ...
    return {column_name: renamed(column, column_name) for column_name, column in columns.items()}
