...
class DataArraySchema(_BaseDataArraySchema):
    ...
    def __init__(
        ...
        if sizes is not None and shape is not None:
            raise SchemaDefinitionError("Pass only one of `sizes` and `shape` on DataArraySchema.")
    ...
    def to_json(
        ...
        **kwargs,
        ...
        return xarray_io.to_json(self, target=target, minimal=minimal, **kwargs)
        ...
        for _key, spec in data_vars.items():
            ...
            if not isinstance(spec, (_DataVar, DataArraySchema)):
                raise SchemaDefinitionError(
                    "DatasetSchema data_vars values must be DataVar, " "DataArraySchema, or None."
                )
