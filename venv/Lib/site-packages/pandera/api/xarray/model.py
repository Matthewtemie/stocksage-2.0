...
class _ChecksDescriptor(_ClassDescriptor):
    def __get__(self, obj: Any, cls: type[_XarrayModelBase]) -> dict[str, list[Check]]:
        if self.cache.get(cls) is None:
            infos = typing.cast(
                list[FieldCheckInfo],
                cls._collect_check_infos(CHECK_KEY),
            )
            self.cache[cls] = cls._extract_checks(infos, field_names=list(cls.__fields__.keys()))
        return self.cache[cls]
...
class _RootCheckDescriptor(_ClassDescriptor):
    def __get__(self, obj: Any, cls: type[_XarrayModelBase]) -> list[Check]:
        if self.cache.get(cls) is None:
            ...
            reg = _convert_extras_to_checks({} if cls.__extras__ is None else cls.__extras__)
            self.cache[cls] = custom + reg
        return self.cache[cls]


class _ParsersDescriptor(_ClassDescriptor):
    def __get__(self, obj: Any, cls: type[_XarrayModelBase]) -> dict[str, list[Parser]]:
        if self.cache.get(cls) is None:
            infos = typing.cast(
                list[FieldParserInfo],
                cls._collect_parser_infos(PARSER_KEY),
            )
            self.cache[cls] = cls._extract_parsers(infos, field_names=list(cls.__fields__.keys()))
        return self.cache[cls]
...
class _XarrayModelBase(BaseModel):
    ...
    Config: type[DataArrayConfig] | type[DatasetConfig] | type[DataTreeConfig] = DataArrayConfig
    ...
    __root_parsers__: list[Parser] = cast(list[Parser], _RootParsersDescriptor())
    ...
    def _get_model_attrs(cls) -> dict[str, Any]:
        ...
        for base in reversed(bases):
            if issubclass(base, _XarrayModelBase) and base is not _XarrayModelBase:
        ...
        for field_name, annotation in annotations.items():
            ...
            if not isinstance(field, XarrayFieldInfo):
                raise SchemaInitError(
                    f"'{field_name}' must use pandera.xarray.Field, " f"not {type(field)}."
                )
    ...
    def _collect_config_and_extras(
        ...
        bases = inspect.getmro(cls)[:-1]
        bases = tuple(
            b for b in bases if issubclass(b, _XarrayModelBase) and b is not _XarrayModelBase
        )
        root_model, *models = reversed(bases)
        options, extras = cls._extract_config_options_and_extras(root_model.Config)
        for model in models:
        ...
        for base in bases:
            for attr_name, attr_value in vars(base).items():
                ...
                if key == CHECK_KEY:
                    ...
                    if not isinstance(info, CheckInfo) or isinstance(info, FieldCheckInfo):
                        continue
                else:
                    continue
        ...
        for base in bases:
            for attr_name, attr_value in vars(base).items():
                ...
                if key == PARSER_KEY:
                    ...
                    if not isinstance(info, ParserInfo) or isinstance(info, FieldParserInfo):
                        continue
                else:
                    continue
                if attr_name in names:
    ...
    def _extract_checks(
        ...
        for check_info in check_infos:
            fields_set = {f.name if isinstance(f, FieldInfo) else f for f in check_info.fields}
            matched = cls._regex_filter(field_names, fields_set) if check_info.regex else fields_set
            check_ = check_info.to_check(cls)
            for field in matched:
                if field not in field_names:
                    raise SchemaInitError(f"Check {check_.name} targets unknown field '{field}'.")
    ...
    def _extract_parsers(
        ...
        for pi in parser_infos:
            fields_set = {f.name if isinstance(f, FieldInfo) else f for f in pi.fields}
            matched = cls._regex_filter(field_names, fields_set) if pi.regex else fields_set
            parser_ = pi.to_parser(cls)
            for field in matched:
                if field not in field_names:
                    raise SchemaInitError(f"Parser {parser_.name} targets unknown field '{field}'.")
                parsers.setdefault(field, []).append(parser_)
        return parsers

    @classmethod
    def _extract_df_parsers(cls, parser_infos: list[ParserInfo]) -> list[Parser]:
    ...
    def to_json(cls, target=None, *, minimal: bool = True, **kwargs):
        ...
        return cls.to_schema().to_json(target=target, minimal=minimal, **kwargs)
...
class DataArrayModel(_XarrayModelBase):
    ...
    def build_schema_(cls) -> DataArraySchema:
        ...
        if dtype in (None, typing.Any):
            raise SchemaInitError("Specify dtype on the 'data' field or Config.dtype.")
        ...
        for fname, (ann, fi) in fields.items():
            ...
            if not _is_coordinate_annotation(ann.raw_annotation):
                raise SchemaInitError(f"Unexpected field {fname!r} on DataArrayModel.")
        ...
        for fname, (ann, fi) in fields.items():
            ...
            if fi.nested_data_array_model is not None:
                ...
                if hasattr(nested_cls, "to_schema"):
                    ...
                    raise SchemaInitError(
                        f"nested_data_array_model {nested_cls!r} " "must have a to_schema() method."
                    )
                continue

            dtype = ann.arg
            if dtype in (None, typing.Any):
                raise SchemaInitError(f"Field {fname!r} needs a dtype or nested DataArrayModel.")
            dv_kwargs = fi.to_data_var_kwargs(dtype, optional=ann.optional)
            dv_kwargs["checks"] = dv_kwargs.get("checks", []) + cls.__checks__.get(fname, [])
            dv_kwargs["parsers"] = list(fi.parses) + cls.__parsers__.get(fname, [])
