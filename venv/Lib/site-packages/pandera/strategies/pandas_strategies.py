...
def _mask(val: Union[pd.Series, pd.Index], null_mask: list[bool]) -> Union[pd.Series, pd.Index]:
...
def _datetime_strategy(dtype: Union[np.dtype, pd.DatetimeTZDtype], strategy) -> SearchStrategy:
...
def numpy_time_dtypes(dtype: Union[np.dtype, pd.DatetimeTZDtype], min_value=None, max_value=None):
...
def to_numpy_dtype(pandera_dtype: DataType):
    ...
    try:
        ...
        raise TypeError(
            f"Data generation for the '{pandera_dtype}' data type is " "currently unsupported."
        ) from err
...
def in_range_strategy(
    ...
    *,
    ...
    return strategy.filter(partial(min_op, min_value)).filter(partial(max_op, max_value))
...
def str_matches_strategy(
    ...
    *,
    ...
    """
    if strategy is None:
        return st.from_regex(pattern, fullmatch=True).map(to_numpy_dtype(pandera_dtype).type)
...
def str_contains_strategy(
    ...
    *,
    ...
    """
    if strategy is None:
        return st.from_regex(pattern, fullmatch=False).map(to_numpy_dtype(pandera_dtype).type)
...
def str_length_strategy(
    ...
    *,
    ...
    if min_value is None and max_value is None:
        raise ValueError("At least one of min_value/max_value or exact_value must be set")
...
def field_element_strategy(
    ...
    *,
    ...
    def undefined_check_strategy(elements, check):
        ...
        return (pandas_dtype_strategy(pandera_dtype) if elements is None else elements).filter(
            check._check_fn
        )
    ...
    *,
        ...
        for check in checks:
            if check.strategy is not None:
                ...
                strategy = undefined_check_strategy(
                    strategy=(pandas_dtype_strategy(col.dtype) if strategy is None else strategy),
                    check=check,
                )
        if strategy is None:
    ...
    def _dataframe_strategy(draw):
        ...
        strategy = pdst.data_frames(
            columns=[column.strategy_component() for column in expanded_columns.values()],
            rows=row_strategy,
            index=pdst.range_indexes(min_size=0 if size is None else size, max_size=size),
        )
    ...
    *,
    ...
    for name, dtype in index_dtypes.items():
        if dtype in {"object", "str"} or dtype.startswith("string"):
            strategy = strategy.map(
                lambda df, name=name: df.assign(**{name: df[name].map(_str_no_surrogates)})
            )
