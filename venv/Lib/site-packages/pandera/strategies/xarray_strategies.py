...
def _numpy_array_strategy(
    ...
    arr = draw(npst.arrays(dtype=np_dtype, shape=shape, elements=element_strategy))
    return arr
...
def data_array_strategy(
    ...
    data = draw(npst.arrays(dtype=np_dtype, shape=resolved_shape, elements=element_strat))
        ...
        if isinstance(schema.coords, list):
            ...
            for name, coord in schema.coords.items():
                if isinstance(coord, Coordinate):
                    coord_specs[name] = {"dtype": str(coord.dtype) if coord.dtype else "float64"}
                else:
                    coord_specs[name] = {"dtype": "float64"}
