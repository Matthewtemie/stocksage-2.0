...
if TYPE_CHECKING:  # pragma: no cover
    ...
    class Dispatch:
        ...
        @staticmethod
        def register(data_type: Any, func: Callable[[Any], DataType]) -> Callable[[Any], DataType]:
...
class Engine(ABCMeta):
    ...
    def _check_source_dtype(cls, data_type: Any) -> None:
        if isinstance(data_type, cls._base_pandera_dtypes) or (
            inspect.isclass(data_type) and issubclass(data_type, cls._base_pandera_dtypes)
        ):
            base_names = [
                f"{base.__module__}.{base.__qualname__}" for base in cls._base_pandera_dtypes
            ]
            raise ValueError(
                f"Subclasses of {base_names} cannot be registered" f" with {cls.__name__}."
            )
    ...
    def _register_from_parametrized_dtype(
        ...
        if not isinstance(method, classmethod):
            raise ValueError(
                f"{pandera_dtype_cls.__name__}.from_parametrized_dtype " + "must be a classmethod."
            )
    ...
    def _register_equivalents(cls, pandera_dtype_cls: type[DataType], *source_dtypes: Any) -> None:
    ...
    def dtype(cls: "Engine", data_type: Any) -> DataType:
        ...
        try:
            ...
            raise TypeError(f"Data type '{data_type}' not understood by {cls.__name__}.") from None
