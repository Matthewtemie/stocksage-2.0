...
class DataType(dtypes.DataType):
    ...
    def __post_init__(self):
        ...
        object.__setattr__(self, "type", pd.api.types.pandas_dtype(self.type))  # pragma: no cover
    ...
    def try_coerce(self, data_container: PandasObject) -> PandasObject:
        try:
            ...
            raise errors.ParserError(
                ...
                failure_cases=utils.numpy_pandas_coerce_failure_cases(data_container, self),
            ) from exc
...
def _register_numpy_numbers(builtin_name: str, pandera_name: str, sizes: list[int]) -> None:
...
class Decimal(DataType, dtypes.Decimal):
    ...
    rounding: str = dataclasses.field(default_factory=lambda: decimal.getcontext().rounding)
    """
    ...
    def check(  # type: ignore
        ...
        if data_container is None:
            return True
        return _check_decimal(data_container, precision=self.precision, scale=self.scale)
...
class Category(DataType, dtypes.Category):
    ...
    def __init__(self, categories: Iterable[Any] | None = None, ordered: bool = False) -> None:
...
class NpString(numpy_engine.String):
    ...
    def check(
        ...
        if data_container is None:
            ...
            if PANDAS_3_0_0_PLUS:
                return isinstance(pandera_dtype, (numpy_engine.Object, type(self), STRING))
...
class DateTime(_BaseDateTime, dtypes.Timestamp):
    ...
    def _coerce(self, data_container: PandasObject, pandas_dtype: Any) -> PandasObject:
        ...
        def _to_datetime(col: PandasObject) -> PandasObject:
            ...
            if pdtype_tz is not None or coltype_tz is not None:
                ...
                elif hasattr(col, "tz") and col.tz != pdtype_tz and hasattr(col, "tz_localize"):
                    if col.tz is None:
    ...
    def coerce(self, data_container: PandasObject) -> PandasObject:
        if self.time_zone_agnostic:
            data_container = self._prepare_coerce_time_zone_agnostic(data_container=data_container)
        return self._coerce(data_container, pandas_dtype=self.type)

    def _prepare_coerce_time_zone_agnostic(self, data_container: PandasObject) -> PandasObject:
        if not self.tz:
            raise errors.ParserError(
        ...
        if isinstance(data_container.dtype, pd.DatetimeTZDtype):
            tz = self.tz
            unit = self.unit if self.unit else getattr(data_container.dtype, "unit", "ns")
    ...
    def _prepare_check_time_zone_agnostic(
        ...
        # If the data has a mix of timezones, pandas defines the dtype as 'object`
            ...
            raise errors.ParserError(
                ...
                failure_cases=(
                    utils.numpy_pandas_coerce_failure_cases(data_container, self)
...
class PydanticModel(DataType):
    ...
    def _check_column_names(
        ...
        absent_columns = [col for col in column_names if col not in data_container.columns]

        if absent_columns:
            raise errors.ParserError(
                f"Missing columns in {type(data_container)} " f"data_container: {absent_columns}",
    ...
    def coerce(self, data_container: PandasObject) -> PandasObject:
        ...
        def _coerce_row(row):
            try:
                ...
                row["failure_cases"] = {k: row[k] for k in (x["loc"][0] for x in exc.errors())}
        ...
        if coerced_df["failure_cases"].any():
            failure_cases = coerced_df["failure_cases"][coerced_df["failure_cases"].notna()].astype(
                str
            )
            raise errors.ParserError(
                f"Could not coerce {type(data_container)} data_container " f"into type {self.type}",
...
class PythonGenericType(DataType):
    ...
    def _check_type(self, element: Any) -> bool:
        ...
        try:
            _type = getattr(self, "generic_type") or getattr(self, "special_type")
            if (
...
if PYARROW_INSTALLED:
    ...
    @Engine.register_dtype(equivalents=[pyarrow.decimal128, pyarrow.Decimal128Type])
    @immutable(init=True)
    ...
    @Engine.register_dtype(equivalents=[pyarrow.dictionary, pyarrow.DictionaryType])
    @immutable(init=True)
    ...
    class ArrowList(ArrowDataType):
        ...
        type: pd.ArrowDtype | None = dataclasses.field(default=None, init=False)
        value_type: Union[pyarrow.DataType, pyarrow.Field] | None = pyarrow.string()
    ...
    class ArrowTime32(ArrowDataType):
        ...
        def coerce(self, data_container: PandasObject) -> PandasObject:
            if data_container.dtype == self.type:
                ...
                return data_container.astype(pd.ArrowDtype(pyarrow.int32())).astype(
                    self.type
    ...
    class ArrowTime64(ArrowDataType):
        ...
        def coerce(self, data_container: PandasObject) -> PandasObject:
            if data_container.dtype == self.type:
                ...
                return data_container.astype(pd.ArrowDtype(pyarrow.int64())).astype(
                    self.type
                )  # type: ignore[arg-type]
