...
def polars_object_coercible(data_container: PolarsData, type_: PolarsDataType) -> pl.LazyFrame:
    ...
    coercible = data_container.lazyframe.cast({key: type_}, strict=strict).select(
...
def polars_failure_cases_from_coercible(
    ...
    return (
        pl.concat(items=[data_container.lazyframe, is_coercible], how="horizontal")
...
def polars_coerce_failure_cases(
    ...
    try:
        failure_cases = polars_failure_cases_from_coercible(data_container, is_coercible)
        is_coercible_df = is_coercible.collect()
    except COERCION_ERRORS:
        # If coercion fails, all of the relevant rows are failure cases
        failure_cases = data_container.lazyframe.select(data_container.key or "*").collect()

        is_coercible_df = (
            data_container.lazyframe.with_columns(**{CHECK_OUTPUT_KEY: pl.lit(False)}).select(
                CHECK_OUTPUT_KEY
            )
        ).collect()
...
class DataType(dtypes.DataType):
    ...
    def __init__(self, dtype: Any | None = None):
        ...
        try:
            object.__setattr__(self, "type", convert_py_dtype_to_polars_dtype(dtype))
        except ValueError:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.type, DataTypeClass):
            try:
                object.__setattr__(self, "type", convert_py_dtype_to_polars_dtype(self.type))
            except ValueError:
                object.__setattr__(self, "type", pl.Object)
    ...
    def try_coerce(self, data_container: PolarsDataContainer) -> pl.LazyFrame:
        ...
        try:
            ...
            _key = "" if data_container.key == "*" else f"'{data_container.key}' in"
...
@Engine.register_dtype(equivalents=["int16", pl.Int16, dtypes.Int16, dtypes.Int16()])
@immutable
...
@Engine.register_dtype(equivalents=["int32", pl.Int32, dtypes.Int32, dtypes.Int32()])
@immutable
...
@Engine.register_dtype(equivalents=["int64", int, pl.Int64, dtypes.Int64, dtypes.Int64()])
@immutable
...
@Engine.register_dtype(equivalents=["uint8", pl.UInt8, dtypes.UInt8, dtypes.UInt8()])
@immutable
...
@Engine.register_dtype(equivalents=["uint16", pl.UInt16, dtypes.UInt16, dtypes.UInt16()])
@immutable
...
@Engine.register_dtype(equivalents=["uint32", pl.UInt32, dtypes.UInt32, dtypes.UInt32()])
@immutable
...
@Engine.register_dtype(equivalents=["uint64", pl.UInt64, dtypes.UInt64, dtypes.UInt64()])
@immutable
...
@Engine.register_dtype(equivalents=["float32", pl.Float32, dtypes.Float32, dtypes.Float32()])
@immutable
...
class Decimal(DataType, dtypes.Decimal):
    ...
    def __init__(
        ...
        object.__setattr__(self, "type", pl.Decimal(precision=precision, scale=scale))
...
class DateTime(DataType, dtypes.DateTime):
    ...
    def from_parametrized_dtype(cls, polars_dtype: pl.Datetime):
        ...
        return cls(time_zone=polars_dtype.time_zone, time_unit=polars_dtype.time_unit)
...
@Engine.register_dtype(equivalents=["bool", bool, pl.Boolean, dtypes.Bool, dtypes.Bool()])
@immutable
...
@Engine.register_dtype(equivalents=["binary", bytes, pl.Binary, dtypes.Binary, dtypes.Binary()])
@immutable
...
@Engine.register_dtype(equivalents=["string", str, pl.Utf8, dtypes.String, dtypes.String()])
@immutable
...
@Engine.register_dtype(equivalents=["category", dtypes.Category, dtypes.Category()])
@immutable(init=True)
...
class PydanticModel(DataType):
    ...
    def coerce(self, data_container: PolarsDataContainer) -> pl.LazyFrame:
        ...
        if not get_config_context().is_warning_silenced(SILENCE_WARNING_PYDANTIC_MODEL):
            logger.warning(
        ...
        for row in df.iter_rows(named=True):
            try:
                ...
                failed_fields = {
                    k: row.get(k, "<missing>") for k in (x["loc"][0] for x in exc.errors())
                }
        ...
        if failure_cases:
            ...
            fc_df = pl.DataFrame(coerced_rows).filter(pl.lit(pl.Series(row_passed)).not_())
            raise errors.ParserError(
