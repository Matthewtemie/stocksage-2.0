...
def _silenced_warnings_from_env() -> list[str]:
    ...
    return [name for name in all_warning_names if os.environ.get(name, "false") in _TRUTHY]


def _config_from_env_vars():
    validation_enabled = os.environ.get("PANDERA_VALIDATION_ENABLED", "True") in _TRUTHY
    ...
    cache_dataframe = os.environ.get("PANDERA_CACHE_DATAFRAME", "False") in _TRUTHY
    keep_cached_dataframe = os.environ.get("PANDERA_KEEP_CACHED_DATAFRAME", "False") in _TRUTHY
