    ...
    *new in 0.5.0*
        ...
        if config.from_format is None:
            if not isinstance(obj, pd.DataFrame):
                try:
                    ...
                    raise ValueError(f"Expected pd.DataFrame, found {type(obj)}") from exc
            return obj
    ...
    def to_format(cls, data: pd.DataFrame, config) -> Any:
        ...
        if callable(config.to_format):
            ...
            if callable(config.to_format_buffer):
                ...
                raise TypeError(
                    "to_format_buffer must be Callable or None, found " f"{config.to_format_buffer}"
                )
        else:
    ...
    def _get_schema_model(cls, field):
        if not field.sub_fields:
            raise TypeError("Expected a typed pandera.typing.DataFrame," " e.g. DataFrame[Schema]")
    ...
    def from_records(  # type: ignore
        ...
        **kwargs,
        ...
        schema_index = dataframe_schema.index.names if dataframe_schema.index is not None else None
        ...
        return DataFrame[schema_model](  # type: ignore
            ...
            data_df[[c for c in dataframe_schema.columns if c in data_df.columns]]
        )
