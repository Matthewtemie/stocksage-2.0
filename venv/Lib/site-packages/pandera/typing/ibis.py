        ...
        *new in 0.25.0*
            ...
            if config.from_format is None:
                if not isinstance(obj, ibis.Table):
                    try:
                        ...
                        raise ValueError(f"Expected ibis.Table, found {type(obj)}") from exc
                return obj
        ...
        def to_format(cls, data: ibis.Table, config) -> Any:
            ...
            if callable(config.to_format):
                ...
                try:
                    ...
                    raise ValueError(
                        f"Unsupported format: {config.to_format}. " f"Ibis natively supports: dict."
                    ) from exc
        ...
        def _get_schema_model(cls, field):
            if not field.sub_fields:
                raise TypeError("Expected a typed pandera.typing.ibis.Table," " e.g. Table[Schema]")
