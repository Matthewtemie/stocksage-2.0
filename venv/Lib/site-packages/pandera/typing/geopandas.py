...
if GEOPANDAS_INSTALLED:
    ...
    class GeoDataFrame(DataFrameBase, gpd.GeoDataFrame, Generic[T]):
        ...
        @classmethod
        def _coerce_geometry(cls, obj: Union[pd.DataFrame, gpd.GeoDataFrame]) -> gpd.GeoDataFrame:
            if not isinstance(obj, gpd.GeoDataFrame):
                ...
                if "geometry" in obj.columns:
                    ...
                    obj["geometry"] = Geometry()._coerce_values(obj["geometry"])
        ...
        def from_format(cls, obj: Any, config) -> gpd.GeoDataFrame:
            ...
            if config.from_format is None:
                if not isinstance(obj, gpd.GeoDataFrame):
                    try:
                        ...
                        raise ValueError(f"Expected gpd.GeoDataFrame, found {type(obj)}") from exc
                return obj
            ...
            if callable(config.from_format):
                ...
                reader = {
                    ...
                    Formats.json: lambda x: gpd.GeoDataFrame.from_features(json.loads(x)),
        ...
        def _get_schema_model(cls, field):
            if not field.sub_fields:
                raise TypeError(
                    "Expected a typed pandera.typing.GeoDataFrame," " e.g. GeoDataFrame[Schema]"
                )
