...
class PanderaPlugin(Plugin):
    ...
    def get_base_class_hook(self, fullname: str) -> "Callable[[ClassDefContext], None] | None":
        ...
        if sym and isinstance(sym.node, TypeInfo):  # pragma: no branch
            if any(get_fullname(base) in DATAFRAMEMODEL_FULLNAMES for base in sym.node.mro):
                return self._pandera_model_class_maker_callback
        return None
    ...
    def get_attribute_hook(self, fullname: str) -> "Callable[[AttributeContext], Instance] | None":
        if self._is_dataframe_model_field_attribute(fullname):
            return self._dataframe_model_class_attr_callback
        return None
    ...
    def _is_dataframe_model_field_attribute(self, fullname: str) -> bool:
        ...
        class_info = sym.node
        if not any(get_fullname(base) in DATAFRAMEMODEL_FULLNAMES for base in class_info.mro):
            return False
    ...
    def erase_field_type_arg(self):
        ...
        for def_, var in self._get_field_assignments():
            ...
            if (
                ...
                not hasattr(type_, "module_name")
                or not hasattr(type_, "name")
            ):
                continue
