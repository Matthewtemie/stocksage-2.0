...
def _serialize_check_stats(check_stats, dtype=None):
    ...
    if isinstance(check_stats, dict) and len(check_stats) == 1:
        value = _handle_stat_dtype_serialize(list(check_stats.values())[0], dtype)
        if check_options:
    ...
    if isinstance(check_stats, dict):
        ...
        for arg, stat in check_stats.items():
            serialized_check_stats[arg] = _handle_stat_dtype_serialize(stat, dtype)
        if check_options:
    ...
    if component_stats.get("checks") is not None:
        ...
        for check_stats in component_stats["checks"]:
            serialized_checks.append(
                _serialize_check_stats(check_stats, component_stats.get("dtype"))
            )
...
def serialize_data_array_schema(data_array_schema, *, minimal: bool = True) -> dict[str, Any]:
...
def serialize_dataset_schema(dataset_schema, *, minimal: bool = True) -> dict[str, Any]:
...
def serialize_schema(schema, *, minimal: bool = True) -> dict[str, Any]:
    ...
    if isinstance(schema, DataArraySchema):
        ...
        raise TypeError(f"Expected DataArraySchema or DatasetSchema, got {type(schema)}")
...
def _deserialize_check_stats(check, serialized_check_stats, dtype=None):
    ...
    if isinstance(serialized_check_stats, dict):
        ...
        check_instance = check(_handle_stat_dtype_deserialize(serialized_check_stats, dtype))
...
def _deserialize_checks(serialized_checks, dtype=None):
    ...
    for check_entry in serialized_checks:
        ...
        if check_name is None:
            warnings.warn("Check entry missing 'check_name' in options, skipping.")
            continue
...
def _deserialize_component_stats(serialized_component_stats):
    ...
    checks = _deserialize_checks(serialized_component_stats.get("checks"), dtype)
    result = {
...
def deserialize_schema(serialized_schema):
    ...
    if schema_type == "data_array":
        ...
        raise ValueError(
            f"Unknown schema_type '{schema_type}'. " "Expected 'data_array' or 'dataset'."
        )
