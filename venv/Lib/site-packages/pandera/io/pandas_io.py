...
def _serialize_check_stats(check_stats, dtype=None):
    ...
    def handle_stat_dtype(stat):
        ...
        if pandas_engine.Engine.dtype(dtypes.DateTime).check(dtype) and hasattr(stat, "strftime"):
...
_DATAFRAME_LIBRARY_CHOICES = frozenset(("pandas", "modin", "dask", "pyspark.pandas"))
...
def serialize_schema(
    ...
    *,
    ...
    if lib not in _DATAFRAME_LIBRARY_CHOICES:
        raise ValueError(
            "dataframe_library must be one of " f"{sorted(_DATAFRAME_LIBRARY_CHOICES)}, got {lib!r}"
        )
    ...
    if statistics["index"] is not None:
        index = [_serialize_component_stats(index_stats) for index_stats in statistics["index"]]
...
def deserialize_schema(serialized_schema):
    ...
    if columns is not None:
        if isinstance(columns, list):
            ...
            for column_stats in columns:
                ...
                component_stats = {
                    key: value for key, value in column_stats.items() if key != "name"
                }
    ...
    if index is not None:
        index = [_deserialize_component_stats(index_component) for index_component in index]
    ...
    if index is None:
        ...
        index = MultiIndex(indexes=[Index(**index_properties) for index_properties in index])
    ...
    return DataFrameSchema(
        ...
        add_missing_columns=serialized_schema.get("add_missing_columns", False),
...
def _format_checks(checks_list):
    ...
    for check_kwargs in checks_list:
        if check_kwargs is None:
            warnings.warn("Check cannot be serialized. This check will be ignored")
            continue

        # Handle options separately
        options = check_kwargs.pop("options", {}) if isinstance(check_kwargs, dict) else {}
        ...
        if options:
            if args:
                args += ", "
            args += ", ".join(f"{k}={v.__repr__()}" for k, v in options.items())
...
def _format_index(index_statistics):
    ...
    for properties in index_statistics:
        ...
        index_code = INDEX_TEMPLATE.format(
            ...
            checks=(
                "None" if properties["checks"] is None else _format_checks(properties["checks"])
            ),
            nullable=properties["nullable"],
            coerce=properties["coerce"],
            name=("None" if properties["name"] is None else f'"{properties["name"]}"'),
...
def _to_script_minimal(dataframe_schema):
    ...
    columns = {
        name: _format_column_minimal(column) for name, column in dataframe_schema.columns.items()
    }
    ...
    if getattr(dataframe_schema, "drop_invalid_rows", False):
        parts.append(f"drop_invalid_rows={dataframe_schema.drop_invalid_rows!r}")
    inner = ",\n    ".join(parts)
...
class FrictionlessFieldParser:
    ...
    def __init__(self, field, primary_keys) -> None:
        ...
        self.description = None if field.description == "" else field.description
...
def from_frictionless_schema(
    ...
    assembled_schema = {
        "columns": {
            field.name: FrictionlessFieldParser(field, schema.primary_key).to_pandera_column()
