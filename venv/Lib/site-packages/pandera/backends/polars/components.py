...
class ColumnBackend(PolarsSchemaBackend):
    ...
    def validate(
        ...
        *,
        ...
        if schema.name is None:
            raise SchemaDefinitionError("Column schema must have a name specified.")
        ...
        if getattr(schema, "regex", False):
            ...
            # (e.g. "var_.+").
            column_keys_to_check = list(self.get_regex_columns(schema, check_obj))
    ...
    def coerce_dtype(
        ...
        try:
            ...
            raise SchemaError(
                ...
                data=check_obj,
                message=(
                    f"Error while coercing '{schema.selector}' to type " f"{schema.dtype}: {exc}"
                ),
        ...
        for column in get_lazyframe_column_names(isna):
            ...
            results.append(
                CoreCheckResult(
                    ...
                    check_output=isna.collect().rename({column: CHECK_OUTPUT_KEY}),
                    check="not_nullable",
                    reason_code=SchemaErrorReason.SERIES_CONTAINS_NULLS,
                    message=(f"non-nullable column '{schema.selector}' contains " f"null values"),
    ...
    def check_unique(
        ...
        results = []
        duplicates = check_obj.select(schema.selector).collect().select(pl.col("*").is_duplicated())
            ...
            if duplicates.select(pl.col(column).any()).item():
                failure_cases = (
                    pl.concat(
                        items=[
                            check_obj,
                            duplicates.select(pl.col(column).alias("_duplicated")).lazy(),
                        ],
                        how="horizontal",
                    )
                ...
                results.append(
                    CoreCheckResult(
                        ...
                        reason_code=SchemaErrorReason.SERIES_CONTAINS_DUPLICATES,
                        message=(f"column '{schema.selector}' " f"not unique:\n{failure_cases}"),
                        failure_cases=failure_cases,
                    )
                )
