...
def notin(data: PolarsData, forbidden_values: Collection) -> pl.LazyFrame:
    ...
    :param forbidden_values: The set of values which should not occur. May
        be any iterable.
    """
    return data.lazyframe.select(pl.col(data.key).is_in(forbidden_values).not_())
...
def str_matches(
    ...
    if not pattern.startswith("^"):
        pattern = f"^{pattern}"
    return data.lazyframe.select(pl.col(data.key).str.contains(pattern=pattern))
...
def str_contains(
    ...
    pattern = pattern.pattern if isinstance(pattern, re.Pattern) else pattern
    return data.lazyframe.select(pl.col(data.key).str.contains(pattern=pattern, literal=False))
...
def unique_values_eq(data: PolarsData, values: Iterable) -> bool:
    ...
    return set(data.lazyframe.collect().get_column(data.key).unique()) == values
