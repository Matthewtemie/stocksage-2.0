...
class DataFrameSchemaBackend(PolarsSchemaBackend):
    ...
    def collect_schema_components(
        ...
        try:
            is_pydantic = issubclass(polars_engine.Engine.dtype(schema.dtype).type, BaseModel)
        except TypeError:
            is_pydantic = False
    ...
    def add_missing_columns(
        ...
        cols_not_in_schema = [
            col for col in check_obj.collect_schema().names() if col not in schema.columns
        ]
        ...
        for column in column_info.destuttered_column_names:
            ...
            if schema.strict is True and not is_schema_col:
                raise SchemaError(
                    ...
                    data=check_obj,
                    message=(
                        f"column '{column}' not in {schema.__class__.__name__}" f" {schema.columns}"
                    ),
        ...
        try:
            if schema.dtype is not None:
                ...
                for col_schema in schema.columns.values():
                    if not col_schema.required and col_schema.name not in lf_columns:
                        continue
            ...
            error_handler.collect_error(
                ...
                SchemaError(
                    ...
                    data=obj,
                    message=(
                        f"Error while coercing '{schema.name}' to type " f"{schema.dtype}: {exc}"
                    ),
                    check=f"coerce_dtype('{schema.dtypes}')",
                    reason_code=SchemaErrorReason.DATATYPE_COERCION,
                ),
    ...
    def set_default(self, check_obj: pl.LazyFrame, schema) -> pl.LazyFrame:
        ...
        for col_schema in [
            s for s in schema.columns.values() if hasattr(s, "default") and s.default is not None
        ]:
    ...
    def check_column_names_are_unique(
        ...
        raise NotImplementedError("polars does not support duplicate column names")
    ...
    def check_column_presence(
        ...
        if column_info.absent_column_names and not schema.add_missing_columns:
            for colname in column_info.absent_column_names:
                if is_regex(colname) and check_obj.select(pl.col(colname)).columns:
                ...
                results.append(
                    CoreCheckResult(
                        ...
                        message=(f"column '{colname}' not in dataframe" f"\n{check_obj.head()}"),
    ...
    def check_column_values_are_unique(
        ...
        for lst in temp_unique:
            subset = [x for x in lst if x in get_lazyframe_column_names(check_obj)]
            duplicates = check_obj.select(subset).collect().is_duplicated()

            check_output = check_obj.with_columns(duplicates.not_().alias("check_output")).collect()
