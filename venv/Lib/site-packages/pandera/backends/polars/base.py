...
def is_float_dtype(check_obj: pl.LazyFrame, selector):
    ...
    return all(
        ...
        for dtype in get_lazyframe_column_dtypes(check_obj.select(pl.col(selector)))
    )
...
class PolarsSchemaBackend(BaseSchemaBackend):
    ...
    def subsample(
        ...
        return check_obj if not obj_subsample else pl.concat(obj_subsample).unique()
        ...
        *args,
        ...
        if not passed:
            if check_result.failure_cases is None:
                ...
                message = (
                    f"{schema.__class__.__name__} '{schema.name}' failed " f"{check_index}: {check}"
                )
            else:
                # use check_result
    ...
    def failure_cases_metadata(
        ...
        for err in schema_errors:
            ...
            if isinstance(err.failure_cases, pl.DataFrame):
                ...
                index = _index_lf.filter(pl.col(CHECK_OUTPUT_KEY).eq(False))["index"]
                if len(err.failure_cases.columns) > 1:
                    ...
                    failure_cases_df = err.failure_cases.with_columns(
                        failure_case=pl.Series(err.failure_cases.rows(named=True))
                ...
                scalar_failure_cases["schema_context"].append(err.schema.__class__.__name__)
    ...
    def drop_invalid_rows(
        ...
        check_outputs = pl.DataFrame({str(i): err.check_output for i, err in enumerate(errors)})
