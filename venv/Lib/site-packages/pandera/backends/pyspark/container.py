...
class DataFrameSchemaBackend(PysparkSchemaBackend):
    ...
    def validate(
        ...
        *,
        ...
        if sample is not None:
            check_obj_sample = self.subsample(check_obj, sample=sample, random_state=random_state)
        else:
            check_obj_sample = check_obj

        # collect schema components
        schema_components = self.collect_schema_components(check_obj, schema, column_info)
    ...
    def collect_column_info(
        ...
        for col_name, col_schema in schema.columns.items():
            if not col_schema.regex and col_name not in check_obj.columns and col_schema.required:
    ...
    def _coerce_dtype(
        ...
        def _try_coercion(obj, colname, col_schema):
            try:
                ...
                error_handler.collect_error(ErrorCategory.DTYPE_COERCION, exc.reason_code, exc)
                return obj
    ...
    def check_column_values_are_unique(
        ...
        unique_columns = [schema.unique] if isinstance(schema.unique, str) else schema.unique
        ...
        duplicates_count = (
            ...
            .filter(col("pandera_duplicate_counts") > 1)  # long name to avoid collisions
            .count()
        )
        ...
        if duplicates_count > 0:
            passed = False
            message = (
                f"Duplicated rows [{duplicates_count}] were found " f"for columns {unique_columns}"
            )
    ...
    def check_column_names_are_unique(
        ...
        if failed:
            passed = False
            message = f"dataframe contains multiple columns with label(s): {failed}"
            ...
            for colname in column_info.absent_column_names:
                results.append(
                    CoreCheckResult(
                        ...
                        message=(f"column '{colname}' not in dataframe" f" {check_obj.head()}"),
