...
def in_range(
    ...
    """
    compare_min = (
        col(data.column_name) >= min_value if include_min else col(data.column_name) > min_value
    )
    compare_max = (
        col(data.column_name) <= max_value if include_max else col(data.column_name) < max_value
    )
...
@register_input_datatypes(
    acceptable_datatypes=convert_to_list(ALL_NUMERIC_TYPE, ALL_DATE_TYPE, STRING_TYPE, BINARY_TYPE)
)
def isin(data: PysparkDataframeColumnObject, allowed_values: Iterable) -> bool:
    ...
    """
    return (
        data.dataframe.filter(~col(data.column_name).isin(list(allowed_values))).limit(1).count()
        == 0
    )
...
def notin(data: PysparkDataframeColumnObject, forbidden_values: Iterable) -> bool:
    ...
    """
    return (
        data.dataframe.filter(col(data.column_name).isin(list(forbidden_values))).limit(1).count()
        == 0
    )
...
def str_matches(
    ...
    if not pattern.startswith("^"):
        pattern = f"^{pattern}"
    return data.dataframe.filter(~col(data.column_name).rlike(pattern)).limit(1).count() == 0
