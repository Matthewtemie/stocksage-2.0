    ...
    def validate(
        ...
        *,
        ...
        column_keys_to_check = (
            self.get_regex_columns(schema, check_obj) if schema.regex else [schema.name]
        )
    ...
    def get_regex_columns(self, schema, check_obj) -> Iterable:
        ...
        if isinstance(schema.name, tuple):
            ...
            for i, name in enumerate(schema.name):
                matched = pd.Index(columns.get_level_values(i).astype(str).str.match(name)).fillna(
                    False
                )
...
class MultiIndexBackend(PandasSchemaBackend):
    ...
    def validate(
        ...
        *,
        ...
        if not is_multiindex(check_obj.index):
            ...
            if len(schema.indexes) == 1 and (is_index(check_obj.index) or is_pyspark_index):
        ...
        if schema.strict:
            self._check_strict(check_obj.index, schema, level_mapping, error_handler)
        ...
        for level_pos, index_schema in level_mapping:
            ...
            can_optimize = validate_full_df and self._can_optimize_level(index_schema)
            ...
            try:
                ...
                self._collect_or_raise(error_handler, exc, schema, index_schema=index_schema)
    ...
    def _can_optimize_level(self, index_schema) -> bool:
        ...
        return all(self._check_determined_by_unique(check) for check in index_schema.checks)
    ...
    def _validate_level_optimized(
        ...
        try:
            ...
            transformed_errors = [
                self._expand_error_to_full_multiindex(err, multiindex, level_pos)
            ...
            transformed_error = self._expand_error_to_full_multiindex(exc, multiindex, level_pos)
    ...
    def _expand_error_to_full_multiindex(
        ...
        failing_codes = level_values.get_indexer(failing_values[~failing_null_mask])
        ...
        lookup_df = pd.DataFrame(
            {
                "level_value": pd.Series(level_values[codes[mask]], dtype=dtype),
    ...
    def _validate_level_with_full_materialization(
        ...
        level_series = pd.Series(full_values, index=multiindex, name=index_schema.name)
    ...
    def _check_strict(
        ...
        if unmapped_level_positions:
            unmapped_level_names = [
                check_obj.names[pos] for pos in sorted(unmapped_level_positions)
            ]
    ...
    def _check_unique(
        ...
        if unique_levels is None:
            ...
            for level_name in unique_levels:
                if isinstance(level_name, str):
                    ...
                    if level_name in check_obj.names:
                        level_positions.append(check_obj.names.index(level_name))
            ...
            if level_positions:
                ...
                level_values = [check_obj.get_level_values(pos) for pos in level_positions]
                ...
                if duplicated_mask is not None and duplicated_mask.any():
                    ...
                    for pos in level_positions:
                        level_name = (
                            check_obj.names[pos] if check_obj.names[pos] is not None else pos
                        )
                        valid_level_names.append(level_name)
                        duplicate_level_values[level_name] = check_obj.get_level_values(pos)[
                            duplicated_mask
                        ]
    ...
    def _collect_or_raise(
        ...
        def _update_schema_error(schema_error: SchemaError):
            ...
            if is_table(failure_cases) and index_schema is not None:
                ...
                schema_error.failure_cases = failure_cases.assign(column=index_schema.name)
            ...
            for pos, expected_name in enumerate(expected):
                ...
                if expected_name is None:
                    ...
                    if (
                        ...
                        and actual_name not in mapped_names[:pos]  # not a continuation
                    ):
                        self._collect_or_raise(
                            error_handler,
                            SchemaError(
    ...
    def _map_ordered_levels(  # helper for ordered=True
        ...
        for idx_schema in schema.indexes:
            ...
            while current_level_pos < n_levels and mi_names[current_level_pos] != idx_name:
