...
class PandasCheckBackend(BaseCheckBackend):
    ...
    def preprocess(
        self, check_obj, key
    ) -> pd.Series | pd.DataFrame | dict[str, pd.Series] | dict[str, pd.DataFrame]:
    ...
    def _get_series_failure_cases(
        ...
        if not failure_cases.empty and self.check.n_failure_cases is not None:
            ...
            if type(failure_cases).__module__.startswith(("pyspark.pandas", "modin.pandas")):
                ...
                failure_cases = failure_cases.groupby(check_output).head(self.check.n_failure_cases)
    ...
    def postprocess_table(
        ...
        for col in select_failure_cases.columns:
            ...
            failure_cases_list.append(
                cases.to_frame().assign(column=col).rename_axis("index").reset_index()
            )
        ...
        if failure_cases_list:
            ...
            failure_cases = (
                failure_cases.set_index("column").groupby("index").agg(lambda df: df.to_dict())
            )
        else:
