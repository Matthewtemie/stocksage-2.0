...
class ArraySchemaBackend(PandasSchemaBackend):
    ...
    def _try_convert_dtype_after_fillna(series, schema_dtype, default_value):
        ...
        if not ArraySchemaBackend._is_default_type_compatible(default_value, schema_dtype):
            return series
        ...
        *,
        ...
        try:
            if is_field(check_obj) and schema.coerce:
                ...
                check_obj[schema.name] = self.coerce_dtype(check_obj[schema.name], schema=schema)
    ...
    def check_nullable(self, check_obj: pd.Series, schema) -> CoreCheckResult:
        ...
        failure_cases = (
            reshape_failure_cases(check_obj[isna], ignore_na=False) if not passed else None
        )
        return CoreCheckResult(
    ...
    def check_unique(self, check_obj: pd.Series, schema) -> CoreCheckResult:
        ...
        if schema.unique:
            ...
            if duplicates is not None and failed is not None and duplicates.any():
                passed = False
                failure_cases = reshape_failure_cases(failed)
                message = f"series '{check_obj.name}' contains duplicate " f"values:\n{failed}"
                        ...
                        try:
                            ...
                            if str(expected_dtype) in (
                                ...
                                non_string = check_obj[
                                    ~check_obj.map(lambda x: (isinstance(x, str) or pd.isna(x)))
                                ]
    ...
    def set_default(self, check_obj, schema):
        ...
        if is_field(check_obj) and not isinstance(check_obj.dtype, pd.SparseDtype):
            ...
            check_obj[schema.name] = check_obj[schema.name].fillna(schema.default)
