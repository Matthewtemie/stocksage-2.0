...
class PandasHypothesisBackend(PandasCheckBackend):
    ...
    def _relationships(self, relationship: Union[str, Callable]):
        ...
        if isinstance(relationship, str):
            ...
            raise ValueError(
                "expected relationship to be str or callable, found " f"{type(relationship)}"
            )
        return relationship
    ...
    def _hypothesis_check(self, check_obj):
        ...
        if is_field(check_obj):
            return self.relationship(*self.check._check_fn(check_obj, **self.check._check_kwargs))

        _check_obj = [check_obj.get(s) for s in self.check.samples]
        return self.relationship(*self.check._check_fn(*_check_obj, **self.check._check_kwargs))
