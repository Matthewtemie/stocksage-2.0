...
def format_vectorized_error_message(
    ...
    if type(reshaped_failure_cases.failure_case).__module__.startswith("pyspark.pandas"):
...
def reshape_failure_cases(
    ...
    if not (is_table(failure_cases) or is_field(failure_cases)):
        raise TypeError(
            "Expected failure_cases to be a DataFrame or Series, found " f"{type(failure_cases)}"
        )
    ...
    if (
        ...
        reshaped_failure_cases = (
            ...
            .assign(index=lambda df: (df.index.to_frame().apply(tuple, axis=1).astype(str)))
        ...
        reshaped_failure_cases = (
            ...
            .to_frame()
            .assign(index=lambda df: (_multiindex_to_frame(df).apply(tuple, axis=1).astype(str)))[
                ["failure_case", "index"]
            ]
        ...
        raise TypeError("type of failure_cases argument not understood: " f"{type(failure_cases)}")
...
def consolidate_failure_cases(schema_errors: list[SchemaError]):
        ...
        check_identifier = (
            ...
            if err.check is None
                ...
                if isinstance(err.check, str)
                    ...
                    else (err.check.name if err.check.name is not None else str(err.check))
                )
            )
        )
    ...
    if scalar_check_failure_cases:
        check_failure_cases.append(pd.DataFrame.from_records(scalar_check_failure_cases))

    # NOTE: this is a hack to support pyspark.pandas and modin
    concat_fn = pd.concat  # type: ignore
    if any(type(x).__module__.startswith("pyspark.pandas") for x in check_failure_cases):
        import pyspark.pandas as ps

        concat_fn = ps.concat  # type: ignore
        check_failure_cases = [
            x if isinstance(x, ps.DataFrame) else ps.DataFrame(x) for x in check_failure_cases
        ]
        return (
    ...
    elif any(type(x).__module__.startswith("modin.pandas") for x in check_failure_cases):
        import modin.pandas as mpd

        concat_fn = mpd.concat  # type: ignore
        check_failure_cases = [
            x if isinstance(x, mpd.DataFrame) else mpd.DataFrame(x) for x in check_failure_cases
        ]
