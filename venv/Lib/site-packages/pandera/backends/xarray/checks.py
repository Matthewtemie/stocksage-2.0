...
class XarrayCheckBackend(BaseCheckBackend):
    ...
    def preprocess(self, check_obj, key: str | None):
        ...
        raise TypeError(f"cannot select key {key!r} on {type(check_obj).__name__}")
    ...
    def _apply_elementwise(self, check_obj):
        ...
        if not isinstance(check_obj, xr.DataArray):
            raise NotImplementedError("element_wise checks on non-DataArray xarray objects.")
    ...
    def postprocess(self, check_obj, check_output) -> CheckResult:
        ...
        raise TypeError(f"check_fn returned unsupported type: {type(check_output)!r}")
