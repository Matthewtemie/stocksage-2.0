        ...
        for err in exc.errors():
            ...
            results.append(
                CoreCheckResult(
                    ...
                    message=(f"{attr_label}: {err['msg']} [type={err['type']}]"),
...
class DataArraySchemaBackend(XarraySchemaBackend):
    ...
    def check_name(self, check_obj, schema: DataArraySchema) -> CoreCheckResult:
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_dims(self, check_obj, schema: DataArraySchema) -> list[CoreCheckResult]:
                    ...
                    if e is not None and e != g:
                        results.append(
                            CoreCheckResult(
                                ...
                                check="dims",
                                reason_code=(SchemaErrorReason.MISMATCH_INDEX),
                                message=(f"dim position {i}: " f"expected {e!r}, " f"got {g!r}"),
                                failure_cases=str(got),
                            )
                        )
                        break
            ...
            if d not in check_obj.sizes or check_obj.sizes[d] != sz:
                results.append(
                    CoreCheckResult(
                        ...
                        check="sizes",
                        reason_code=(SchemaErrorReason.MISMATCH_INDEX),
                        message=(f"expected size {d}={sz}, " f"got {check_obj.sizes.get(d)}"),
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_shape(self, check_obj, schema: DataArraySchema) -> list[CoreCheckResult]:
            ...
            if i >= len(check_obj.shape) or check_obj.shape[i] != sh:
                results.append(
                    CoreCheckResult(
                        ...
                        check="shape",
                        reason_code=(SchemaErrorReason.MISMATCH_INDEX),
                        message=(f"expected shape[{i}]={sh}, " f"got shape " f"{check_obj.shape}"),
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_dtype(self, check_obj, schema: DataArraySchema) -> CoreCheckResult:
        """Check the data type."""
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_chunked(self, check_obj, schema: DataArraySchema) -> CoreCheckResult:
    ...
    def check_array_type(self, check_obj, schema: DataArraySchema) -> CoreCheckResult:
        ...
        if schema.array_type is None or isinstance(check_obj.data, schema.array_type):
        ...
        return CoreCheckResult(
            ...
            reason_code=SchemaErrorReason.INVALID_TYPE,
            message=(
                f"expected array type " f"{schema.array_type}, " f"got {type(check_obj.data)}"
            ),
            failure_cases=type(check_obj.data).__name__,
        )

    @validate_scope(scope=ValidationScope.DATA)
    def check_nullable(self, check_obj, schema: DataArraySchema) -> CoreCheckResult:
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_attrs(self, check_obj, schema: DataArraySchema) -> list[CoreCheckResult]:
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_strict_attrs(self, check_obj, schema: DataArraySchema) -> list[CoreCheckResult]:
        """Check for unexpected attributes."""
        results: list[CoreCheckResult] = []
        if not (schema.strict_attrs and schema.attrs is not None):
            return results
        if _is_pydantic_model_class(schema.attrs):
            allowed = set(schema.attrs.model_fields.keys())  # type: ignore[union-attr]
        else:
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_encoding(self, check_obj, schema: DataArraySchema) -> list[CoreCheckResult]:
            ...
            if ek not in actual_enc:
                results.append(
                    CoreCheckResult(
                        ...
                        check="encoding",
                        reason_code=(SchemaErrorReason.SCHEMA_COMPONENT_CHECK),
                        message=(f"missing encoding key {ek!r}: expected {ev!r}"),
                        failure_cases="<missing>",
                    )
                )
                ...
                results.append(
                    CoreCheckResult(
                        ...
                        reason_code=(SchemaErrorReason.SCHEMA_COMPONENT_CHECK),
                        message=(
                            f"encoding {ek!r}: " f"expected {ev!r}, " f"got {actual_enc[ek]!r}"
                        ),
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_coords(self, check_obj, schema: DataArraySchema) -> list[CoreCheckResult]:
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_strict_coords(self, check_obj, schema: DataArraySchema) -> list[CoreCheckResult]:
            ...
            if c.indexed is True and coord_name not in parent.xindexes:
                results.append(
                    CoreCheckResult(
                        ...
                        check="indexed",
                        reason_code=(SchemaErrorReason.SCHEMA_COMPONENT_CHECK),
                        message=(f"coordinate {coord_name!r} " f"expected indexed on " f"parent"),
                        failure_cases=coord_name,
                    )
                )
            if c.indexed is False and coord_name in parent.xindexes:
                results.append(
                    CoreCheckResult(
                        ...
                        reason_code=(SchemaErrorReason.SCHEMA_COMPONENT_CHECK),
                        message=(
                            f"coordinate {coord_name!r} " f"expected non-indexed " f"on parent"
                        ),
                        failure_cases=coord_name,
                    )
                )
...
class DatasetSchemaBackend(XarraySchemaBackend):
            ...
            if exp_tuple != got_tuple:
                results.append(
                    CoreCheckResult(
                        ...
                        check="dims",
                        reason_code=(SchemaErrorReason.MISMATCH_INDEX),
                        message=(f"expected dims {exp_tuple!r}, got {got_tuple!r}"),
            ...
            if exp_set != got_set:
                results.append(
                    CoreCheckResult(
                        ...
                        check="dims",
                        reason_code=(SchemaErrorReason.MISMATCH_INDEX),
                        message=(f"expected dims {exp_set!r}, got {got_set!r}"),
            ...
            if d not in ds.sizes or ds.sizes[d] != sz:
                results.append(
                    CoreCheckResult(
                        ...
                        reason_code=(SchemaErrorReason.MISMATCH_INDEX),
                        message=(
                            f"dataset size {d!r} " f"expected {sz}, " f"got {ds.sizes.get(d)}"
                        ),
            ...
            if ak not in ds.attrs:
                results.append(
                    CoreCheckResult(
                        ...
                        check="attrs",
                        reason_code=(SchemaErrorReason.SCHEMA_COMPONENT_CHECK),
                        message=(f"missing dataset attribute " f"{ak!r}: expected {av!r}"),
                        failure_cases="<missing>",
                    )
                )
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_strict_attrs(self, ds, schema: DatasetSchema) -> list[CoreCheckResult]:
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_encoding(self, ds, schema: DatasetSchema) -> list[CoreCheckResult]:
        """Check encoding values (equality, regex, callable, pydantic)."""
        results: list[CoreCheckResult] = []
        if not schema.encoding:
            return results
    ...
    @validate_scope(scope=ValidationScope.SCHEMA)
    def check_strict_coords(self, ds, schema: DatasetSchema) -> list[CoreCheckResult]:
        """Check for unexpected coordinates."""
        results: list[CoreCheckResult] = []
                ...
                if other not in ds.data_vars:
                    results.append(
                        CoreCheckResult(
                            ...
                            reason_code=(SchemaErrorReason.SCHEMA_COMPONENT_CHECK),
                            message=(
                                f"aligned_with: peer " f"{other_log!r} " f"({other!r}) missing"
                            ),
                        )
                    )
                    continue
                if not _aligned_dims_sizes(da_self, ds[other]):
                    results.append(
                        CoreCheckResult(
                            ...
                            check="aligned_with",
                            reason_code=(SchemaErrorReason.MISMATCH_INDEX),
                            message=(f"{actual!r} not aligned" f" with {other!r} " f"(dims/sizes)"),
                        )
                    )
                ...
                if other not in ds.data_vars:
                    results.append(
                        CoreCheckResult(
                            ...
                            reason_code=(SchemaErrorReason.SCHEMA_COMPONENT_CHECK),
                            message=(f"broadcastable_with: " f"peer {other_log!r} " f"missing"),
                        )
                    )
                    continue
                if not _broadcast_compatible(da_self, ds[other]):
                    results.append(
                        CoreCheckResult(
                            ...
                            check="broadcastable_with",
                            reason_code=(SchemaErrorReason.MISMATCH_INDEX),
                            message=(f"{actual!r} not " f"broadcast-compatible " f"with {other!r}"),
                        )
                    )
        return results
    ...
    def _apply_default(
        ...
        if spec.dims is None:
            raise SchemaError(
                dataset_schema,
                data=ds,
                message=("inserting scalar default requires " f"DataVar.dims for {actual_name!r}"),
                reason_code=(SchemaErrorReason.SCHEMA_COMPONENT_CHECK),
            )
        shape = []
        coords = {}
    ...
    def validate(
        ...
        *,
        ...
        dupes = {a: lgs for a, lgs in actual_to_logicals.items() if len(lgs) > 1}
        if dupes:
            ...
            raise SchemaError(
                ...
                data=ds,
                message=(
                    "multiple data_vars resolve to the " "same actual variable name: " f"{detail}"
                ),
        ...
        ds = self._fill_data_var_defaults(ds, schema, logical_to_actual, error_handler)
