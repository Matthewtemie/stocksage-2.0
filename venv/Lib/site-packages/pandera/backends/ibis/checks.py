...
class IbisCheckBackend(BaseCheckBackend):
    ...
    def __init__(self, check: Check):
        ...
        self.check = check
        self.check_fn = (
            partial(ibis.udf.scalar.python(check._check_fn), **check._check_kwargs)
    ...
    def preprocess(self, check_obj: Union[ibis.Column, ibis.Table], key: str | None):
    ...
    def apply(self, check_obj: IbisData):
        ...
        if self.check.element_wise:
            selector = select_column(check_obj.key) if check_obj.key is not None else s.all()
            ...
            if isinstance(out, dict):
                ...
                if check_obj.table.get_backend().name in POSITIONAL_JOIN_BACKENDS:
                    ...
                    index_col = "__idx__"
                    out = (
                        check_obj.table.mutate(**{index_col: ibis.row_number().over()})
                        .join(
                            out.mutate(**{index_col: ibis.row_number().over()}),
                            index_col,
                        )
                        .drop(index_col)
                    )
