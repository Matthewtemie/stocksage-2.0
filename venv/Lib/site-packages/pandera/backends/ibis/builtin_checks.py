...
def _across(table: ibis.Table, selection: str | None, func) -> ibis.Table:
    return table.select(s.across(s.all() if selection is None else select_column(selection), func))
...
def in_range(
    ...
    use_time_cast = isinstance(min_value, datetime.time) or isinstance(max_value, datetime.time)
    if use_time_cast:
...
def isin(data: IbisData, allowed_values: Iterable) -> ibis.Table:
    ...
    if allowed_values and any(isinstance(v, datetime.time) for v in allowed_values):
        return _across(
            data.table,
            data.key,
...
def notin(data: IbisData, forbidden_values: Iterable) -> ibis.Table:
    ...
    :param forbidden_values: The set of values which should not occur. May
        be any iterable.
    """
    forbidden_values = [_infer_interval_with_mixed_units(value) for value in forbidden_values]
    if forbidden_values and any(isinstance(v, datetime.time) for v in forbidden_values):
        return _across(
            data.table,
            data.key,
...
def unique_values_eq(data: IbisData, values: Iterable) -> ibis.Table:
    ...
    """
    return set(data.table.select(data.key).distinct()[data.key].to_pyarrow().to_pylist()) == values
