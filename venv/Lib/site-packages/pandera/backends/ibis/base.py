...
class IbisSchemaBackend(BaseSchemaBackend):
        ...
        *args,
        ...
        if not passed:
            if check_result.failure_cases is None:
                ...
                elif is_table(check_output) and CHECK_OUTPUT_KEY in check_output.columns:
                    failed_index = check_output.index[~check_output[CHECK_OUTPUT_KEY]]
                ...
                failure_cases = reshape_failure_cases(check_failure_cases, check.ignore_na)
                message = format_vectorized_error_message(schema, check, check_index, failure_cases)
    ...
    def drop_invalid_rows(self, check_obj: ibis.Table, error_handler: ErrorHandler) -> ibis.Table:
        """Remove invalid elements in a check obj according to failures caught by the error handler."""
        import ibis.expr.types as ir

        if positional_join := check_obj.get_backend().name in POSITIONAL_JOIN_BACKENDS:
            out = check_obj
        ...
        for i, error in enumerate(error_handler.schema_errors):
            ...
            if isinstance(check_output, ir.BooleanColumn):
                check_output = (~check_output).name(CHECK_OUTPUT_KEY).as_table()

            out = join(
                out,
                check_output.rename({f"{i}{CHECK_OUTPUT_SUFFIX}": CHECK_OUTPUT_KEY}),
            )
