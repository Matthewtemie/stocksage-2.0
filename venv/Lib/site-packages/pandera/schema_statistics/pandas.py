...
def infer_index_statistics(index: Union[pd.Index, pd.MultiIndex]):
    ...
    if isinstance(index, pd.MultiIndex):
        index_statistics = [_index_stats(index.get_level_values(i)) for i in range(index.nlevels)]
    ...
    for check_name, stats in check_stats.items():
        ...
        try:
            ...
            if isinstance(stats, dict):
                options = stats.pop("options", {}) if "options" in stats else {}
...
def get_index_schema_statistics(index_schema_component):
    ...
    return [
        _get_series_base_schema_statistics(index_component) for index_component in index_components
    ]
...
def parse_checks(checks) -> Union[list[dict[str, Any]], None]:
    ...
    def _has_custom_error(check: Check) -> bool:
        ...
        try:
            default_check = getattr(Check, check.name)(**(check.statistics or {}))
    ...
    for check in checks:
        ...
        check_options = {k: v for k, v in check_options.items() if v is not None}
...
def _get_array_check_statistics(x, data_type: dtypes.DataType) -> Union[dict[str, Any], None]:
