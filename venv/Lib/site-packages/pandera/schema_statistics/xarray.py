...
def get_data_array_schema_statistics(
    ...
    if data_array_schema.coords:
        if isinstance(data_array_schema.coords, list):
            ...
            for name, coord in data_array_schema.coords.items():
                coord_stats[name] = {
                    "dtype": (str(coord.dtype) if coord.dtype is not None else None),
    ...
    return {
        "dtype": (str(data_array_schema.dtype) if data_array_schema.dtype is not None else None),
    ...
    if dataset_schema.data_vars:
        ...
        for key, spec in dataset_schema.data_vars.items():
            ...
            if isinstance(spec, DataVar):
                data_var_stats[key] = {
                    "dtype": (str(spec.dtype) if spec.dtype is not None else None),
                    "dims": spec.dims,
...
def _get_array_check_statistics(
    ...
    elif np.issubdtype(dtype, np.number) and not np.issubdtype(dtype, np.bool_):
        check_stats = {
