...
class _dia_base(_data_matrix):
    ...
    def _getnnz(self, axis=None):
        ...
        return int(
            np.maximum(np.minimum(M + self.offsets, L) - np.maximum(self.offsets, 0), 0).sum()
        )
    ...
    def _add_sparse(self, other, sub=False):
        ...
        if np.array_equal(self.offsets, other.offsets):
            return self._with_data(self.data - other.data if sub else self.data + other.data)
    ...
    def _matmul_multivector(self, other):
        res = np.zeros((self.shape[0], other.shape[1]), dtype=np.result_type(self.data, other))
        dia_matvecs(
            *self.shape, *self.data.shape, self.offsets, self.data, other.shape[1], other, res
        )
        return res
    ...
    def tocsr(self, copy=False):
        ...
        out = self._csr_container((csr_data, indices, indptr), shape=self.shape, dtype=self.dtype)
