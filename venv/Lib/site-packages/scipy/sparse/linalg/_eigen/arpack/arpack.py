...
WHICH_DICT = {"LM": 0, "SM": 1, "LR": 2, "SR": 3, "LI": 4, "SI": 5, "LA": 6, "SA": 7, "BE": 8}
...
class _ArpackParams:
    def __init__(
        self, n, k, tp, rng, mode=1, sigma=None, ncv=None, v0=None, maxiter=None, which="LM", tol=0
    ):
        if k <= 0:
    ...
    def _raise_no_convergence(self):
        ...
        raise ArpackNoConvergence(
            f"No convergence ({num_iter} iterations, " f"{k_ok}/{self.k} eigenvectors converged)",
            ev,
            vec,
        )
...
class _SymmetricArpackParams(_ArpackParams):
    def __init__(
        ...
        _ArpackParams.__init__(self, n, k, tp, self.rng, mode, sigma, ncv, v0, maxiter, which, tol)
    ...
    def iterate(self):
        ...
        if self.arpack_dict["ido"] == 1:
            ...
            self.resid[:] = self.rng.uniform(low=-1.0, high=1.0, size=[self.n]).astype(self.tp)
            ...
            if self.arpack_dict["info"] == 0:
                ...
                raise ArpackError(self.arpack_dict["info"], infodict=self.iterate_infodict)
...
class _UnsymmetricArpackParams(_ArpackParams):
    def __init__(
        ...
        if which not in _NEUPD_WHICH:
            raise ValueError("Parameter which must be one of" f" {' '.join(_NEUPD_WHICH)}")
        if k >= n - 1:
            raise ValueError(f"k must be less than ndim(A)-1, k={k}")

        self.rng = np.random.default_rng(rng)
        _ArpackParams.__init__(self, n, k, tp, rng, mode, sigma, ncv, v0, maxiter, which, tol)
    ...
    def iterate(self):
        if self.tp in "fd":
            ...
            self._arpack_solver(
                self.arpack_dict, self.resid, self.v, self.ipntr, self.workd, self.workl, self.rwork
            )
        ...
        if self.arpack_dict["ido"] == 1:
            ...
            if self.arpack_dict["info"] == 0:
                ...
                raise ArpackError(info=self.arpack_dict["info"], infodict=self.iterate_infodict)
