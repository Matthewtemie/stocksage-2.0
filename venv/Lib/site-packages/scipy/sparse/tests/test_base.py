...
class _TestCommon:
    ...
    def test_mean(self):
        ...
        def check(dtype):
            ...
            assert_array_almost_equal(dat.mean(axis=-1, keepdims=keep), datsp.mean(axis=-1))
            assert_equal(dat.mean(axis=-1).dtype, datsp.mean(axis=-1).dtype)
            assert_array_almost_equal(dat.mean(axis=(0, 1), keepdims=keep), datsp.mean(axis=(0, 1)))
...
class _TestSolve:
    def test_solve(self):
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings(
                "ignore", "splu converted its input to CSC format", SparseEfficiencyWarning
            )
            x = splu(A).solve(r)
...
class TestCOO(
    BaseTestCOO,
    sparse_test_class(
        getset=True, slicing=True, slicing_assign=True, fancy_indexing=True, fancy_assign=True
    ),
):
    spcreator = coo_array
...
class TestDIA(
    ...
    def ill_cases(self):
        ...
        return [
            ...
            (d1, [1], 0, [[0, 0], [0, 0]], "shorter diagonal above (but within if full)"),
