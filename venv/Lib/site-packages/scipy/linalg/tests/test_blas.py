...
from numpy import arange, triu, tril, zeros, tril_indices, ones, diag, append, eye, nonzero
...
class TestFBLAS2Simple:
    ...
    def test_spmv_hpmv(self, fname, dtype):
        ...
        y1 = func(
            n=n - 1, alpha=alpha, beta=beta, x=xlong, y=ylong, ap=Ap, incx=2, incy=2, offx=n, offy=n
        )
...
class TestTRMM:
    ...
    def test_side(self, dtype):
        ...
        k = self.b2.shape[1]
        assert_allclose(res, self.b2 @ self.a2[:k, :k], rtol=0.0, atol=100 * np.finfo(dtype).eps)
