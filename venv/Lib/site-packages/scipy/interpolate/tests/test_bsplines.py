...
class TestBSpline:
    ...
    def test_integral(self, xp):
        ...
        assert math.isclose(
            b.integrate(1, -1, extrapolate=False), _impl.splint(1, -1, b.tck), abs_tol=1e-14
        )
        ...
        xp_assert_close(b.integrate(1.5 + 12, 3 + 12), xp.asarray(i(1) - i(0) + i(2) - i(1.5)))
        xp_assert_close(
            b.integrate(1.5, 3 + 12), xp.asarray(i(1) - i(0) + i(2) - i(1.5) + 6 * period_int)
        )
...
class TestInterp:
    ...
    def test_cubic_deriv(self, xp):
        ...
        xp_assert_close(b(xx), yy, atol=1e-14, rtol=1e-14)
        xp_assert_close(
            b(xx[0], 1), xp.asarray(der_l[0][1], dtype=xp.float64), atol=1e-14, rtol=1e-14
        )
        xp_assert_close(
            b(xx[-1], 1), xp.asarray(der_r[0][1], dtype=xp.float64), atol=1e-14, rtol=1e-14
        )
    ...
    def test_knots_not_data_sites(self, xp):
        ...
        t = concat_1d(xp, xp.ones(k + 1) * xx[0], (xx[1:] + xx[:-1]) / 2.0, xp.ones(k + 1) * xx[-1])
...
class TestGivensQR:
    ...
    def test_evaluate_all_bspl(self):
        ...
        for xval in x:
            xp_assert_equal(_dierckx.evaluate_all_bspl(t, k, xval, n, k + 2), zero_array)
            xp_assert_equal(_dierckx.evaluate_all_bspl(t, k, xval, n, 2 * k), zero_array)
...
class TestNdBSpline:
    ...
    def test_2D_separable(self, xp):
        ...
        xp_assert_close(
            xp.asarray(b2), xp.asarray(target, dtype=xp.float64), check_shape=False, atol=1e-14
        )
    ...
    def test_2D_derivative(self, xp):
        ...
        for nu, expected_fn in test_cases.items():
            expected_vals = xp.asarray([expected_fn(x, y) for x, y in xi], dtype=xp.float64)
...
class TestGenerateKnots:
    ...
    def test_nest(self, xp):
        ...
        xp_assert_close(
            knots[-1],
            xp.asarray([0.0, 0.0, 0.0, 0.0, 2.0, 4.0, 7.0, 7.0, 7.0, 7.0], dtype=xp.float64),
            atol=1e-15,
        )
