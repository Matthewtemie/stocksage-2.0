...
class TestAkima1DInterpolator:
    def test_eval(self, xp):
        ...
        y = xp.asarray([0.0, 2.0, 1.0, 3.0, 2.0, 6.0, 5.5, 5.5, 2.7, 5.1, 3.0], dtype=xp.float64)
        ak = Akima1DInterpolator(x, y)
        xi = xp.asarray(
            [0.0, 0.5, 1.0, 1.5, 2.5, 3.5, 4.5, 5.1, 6.5, 7.2, 8.6, 9.9, 10.0], dtype=xp.float64
        )
        yi = xp.asarray(
            [
                0.0,
...
class TestPPoly:
    ...
    def test_periodic(self, xp):
        ...
        xp_assert_close(
            p(-0.3), xp.asarray(4 * (0.7 - 0.5) ** 2 + 5 * (0.7 - 0.5) + 6, dtype=xp.float64)
        )
...
class TestBPoly:
    ...
    def test_simple3(self, xp):
        ...
        xp_assert_close(
            bp(0.2), xp.asarray(3 * 0.8 * 0.8 + 1 * 2 * 0.2 * 0.8 + 4 * 0.2 * 0.2, dtype=xp.float64)
        )
    ...
    def test_simple4(self, xp):
        ...
        xp_assert_close(
            bp(0.3),
            xp.asarray(0.7**3 + 3 * 0.7**2 * 0.3 + 3 * 0.7 * 0.3**2 + 2 * 0.3**3, dtype=xp.float64),
        )
...
class TestBPolyCalculus:
    def test_derivative(self, xp):
        ...
        xp_assert_close(
            xp.stack([bp(1.7, nu) for nu in [1, 2, 3]]), xp.asarray([0.7, 1.0, 0], dtype=xp.float64)
        )
    ...
    def test_integrate_periodic(self, xp):
        ...
        xp_assert_close(P.integrate(3.5 + 12, 5 + 12), xp.asarray(I(2) - I(1) + I(4) - I(3.5)))
        xp_assert_close(
            P.integrate(3.5, 5 + 12), xp.asarray(I(2) - I(1) + I(4) - I(3.5) + 4 * period_int)
        )
