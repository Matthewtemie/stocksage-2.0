    ...
    **Interpolate and extrapolate a 2D dataset**
    ...
    def _construct_spline(self, method, solver=None, **solver_args):
        if solver is None:
            solver = ssl.gcrotmk
        spl = make_ndbspl(
            self._grid, self._values, self._SPLINE_DEGREE_MAP[method], solver=solver, **solver_args
        )
        return spl
    ...
    def __call__(self, xi, method=None, *, nu=None):
        ...
        if method == "linear":
            ...
            if (
                ...
                result = evaluate_linear_2d(self._values, indices, norm_distances, self._grid, out)
            else:
