...
class _PPolyBase:
    ...
    def extend(self, c, x):
        ...
        if c.shape[2:] != self._c.shape[2:] or c.ndim != self._c.ndim:
            raise ValueError(f"Shapes of c {c.shape} and self._c {self._c.shape} are incompatible")
        ...
        c2 = np.zeros((k2, self._c.shape[1] + c.shape[1]) + self._c.shape[2:], dtype=dtype)
...
@xp_capabilities(
    cpu_only=True,
    jax_jit=False,
    skip_backends=[("dask.array", "https://github.com/data-apis/array-api-extra/issues/488")],
)
class PPoly(_PPolyBase):
    ...
    def antiderivative(self, nu=1):
        ...
        c = np.zeros(
            (self._c.shape[0] + nu, self._c.shape[1]) + self._c.shape[2:], dtype=self._c.dtype
        )
        c[:-nu] = self._c
