...
def _validate_inputs(x, y, w, k, s, xb, xe, parametric, periodic=False):
    ...
    if periodic and not np.allclose(y[0], y[-1], atol=1e-15):
        raise ValueError(
            "First and last points does not match which is required " "for `bc_type='periodic'`."
        )
...
class Fperiodic:
    ...
    def __init__(self, x, y, t, k, s, w=None, *, R=None, Y=None, A1=None, A2=None, Z=None):
        ...
        if (A1 is None or A2 is None or Z is None) or (R is None and Y is None):
            ...
            R, A1, A2, Z, _, _, _, _ = _lsq_solve_qr_for_root_rati_periodic(x, y, t, k, w)
    ...
    def __call__(self, p):
        ...
        _dierckx.qr_reduce_augmented_matrices(G1, G2, H1, H2, c, self.offset_, len(self.t), self.k)
        ...
        c, _, fp = _dierckx.fpbacp(
            G1, G2, c, self.k, self.k + 1, self.x[:-1], self.y[:-1, :], self.t, self.w[:-1]
        )
...
@xp_capabilities(cpu_only=True, jax_jit=False, allow_dask_compute=True)
def make_splprep(x, *, w=None, u=None, ub=None, ue=None, k=3, s=0, t=None, nest=None, bc_type=None):
    r"""
