...
def from_matrix(matrix: Array, assume_valid: bool = False) -> Array:
    ...
    if not assume_valid:
        ...
        is_orthogonal = xp.all(xpx.isclose(gramians, eye, atol=1e-12, xp=xp), axis=(-2, -1))
...
def from_euler(seq: str, angles: Array, degrees: bool = False) -> Array:
    ...
    if not (intrinsic or extrinsic):
        raise ValueError(
            "Expected axes from `seq` to be from ['x', 'y', " f"'z'] or ['X', 'Y', 'Z'], got {seq}"
        )
...
def from_davenport(axes: Array, order: str, angles: Array | float, degrees: bool = False) -> Array:
    ...
    if not broadcastable(axes.shape[:-1], angles.shape) or axes.shape[-2] != angles.shape[-1]:
        raise ValueError(
...
def as_quat(quat: Array, canonical: bool = False, *, scalar_first: bool = False) -> Array:
    xp = array_namespace(quat)
    if canonical:
...
def as_euler(
    ...
    if not (intrinsic or extrinsic):
        raise ValueError(
            "Expected axes from `seq` to be from ['x', 'y', 'z'] or ['X', 'Y', 'Z'], " f"got {seq}"
        )
    ...
    angles = _get_angles(extrinsic, symmetric, sign, xp.pi / 2, a, b, c, d, suppress_warnings)
...
def mean(
    ...
    if axis != () and (min(axis) < -(quat.ndim - 1) or max(axis) > (quat.ndim - 2)):
        raise ValueError(f"axis {axis} is out of bounds for rotation with shape {quat.shape[:-1]}.")
...
def apply(quat: Array, points: Array, inverse: bool = False) -> Array:
    ...
    if not broadcastable(mat.shape, points.shape):
        raise ValueError(
            f"Cannot broadcast {quat.shape[:-1]} rotations to {points.shape[:-1]} " "vectors."
        )
    if inverse:
...
def align_vectors(
    ...
    if a.shape[-1] != 3:
        raise ValueError(f"Expected input `a` to have shape (3,) or (N, 3), got {a_original.shape}")
    if b.shape[-1] != 3:
        raise ValueError(f"Expected input `b` to have shape (3,) or (N, 3), got {b_original.shape}")
    if a.shape != b.shape:
        raise ValueError(
    ...
    if not is_lazy_array(n_inf):
        ...
        if n_inf == 1 and return_sensitivity:
            raise ValueError(
                "Cannot return sensitivity matrix with an " "infinite weight or one vector pair"
            )
...
def _align_vectors(a: Array, b: Array, weights: Array) -> tuple[Array, Array, Array]:
    ...
    # (s[1] + s[2] < 1e-16 * s[0])
    ssd = xp.sum(weights * xp.sum(b**2 + a**2, axis=-1), axis=-1) - 2 * xp.sum(s, axis=-1)
...
def _align_vectors_fixed(a: Array, b: Array, weights: Array) -> tuple[Array, Array, Array]:
    xp = array_namespace(a)
    device = xp_device(a)
    N = a.shape[0]
...
def _compute_davenport_from_quat(
    ...
    quat_lamb = xp.concat((xp.sin(lamb / 2)[..., None] * n2, xp.cos(lamb / 2)[..., None]), axis=-1)
    ...
    angles = xpx.at(angles)[..., 1].set(xp.where(correct_set, -angles[..., 1], angles[..., 1]))
