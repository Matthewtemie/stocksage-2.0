...
def from_rotation(quat: Array) -> Array:
    ...
    matrix = xp.zeros((*rotmat.shape[:-2], 4, 4), dtype=quat.dtype, device=xp_device(quat))
    matrix = xpx.at(matrix)[..., :3, :3].set(rotmat)
...
def from_translation(translation: Array) -> Array:
    ...
    if translation.shape[-1] != 3:
        raise ValueError(f"Expected `translation` to have shape (..., 3), got {translation.shape}.")
    ...
    matrix = xpx.at(matrix)[..., :3, 3].set(xp_promote(translation, force_floating=True, xp=xp))
    return matrix
...
def from_dual_quat(dual_quat: Array, *, scalar_first: bool = False) -> Array:
    ...
    if dual_quat.shape[-1] != 8:
        raise ValueError(f"Expected `dual_quat` to have shape (..., 8), got {dual_quat.shape}.")
...
def as_dual_quat(matrix: Array, *, scalar_first: bool = False) -> Array:
    ...
    pure_translation_quats = xpx.at(pure_translation_quats)[..., :3].set(matrix[..., :3, 3])
...
def inv(matrix: Array) -> Array:
    ...
    matrix = xp.zeros((*matrix.shape[:-2], 4, 4), dtype=matrix.dtype, device=xp_device(matrix))
...
def apply(matrix: Array, vector: Array, inverse: bool = False) -> Array:
    ...
    vec = xp.empty((*vector.shape[:-1], 4), dtype=vector.dtype, device=xp_device(vector))
...
def mean(
    matrix: Array, weights: ArrayLike | None = None, axis: None | int | tuple[int, ...] = None
) -> Array:
    xp = array_namespace(matrix)
    if matrix.shape[0] == 0:
    ...
    if axis != () and (min(axis) < -(matrix.ndim - 2) or max(axis) > (matrix.ndim - 3)):
        raise ValueError(
            f"axis {axis} is out of bounds for transform with shape " f"{matrix.shape[:-2]}."
        )
...
def _create_transformation_matrix(translations: Array, rotation_matrices: Array) -> Array:
    if not translations.shape[:-1] == rotation_matrices.shape[:-2]:
        raise ValueError("The number of rotation matrices and translations must be the same.")
...
def _normalize_dual_quaternion(real_part: Array, dual_part: Array) -> tuple[Array, Array]:
    ...
    unit_quat = xp.asarray([0.0, 0.0, 0.0, 1.0], dtype=real_part.dtype, device=xp_device(real_part))
