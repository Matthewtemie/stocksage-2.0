...
class TestCobyla:
    ...
    def test_simple(self):
        ...
        x = fmin_cobyla(
            self.fun, self.x0, [self.con1, self.con2], rhobeg=1, rhoend=1e-5, maxfun=100, disp=1
        )
    ...
    def test_minimize_simple(self):
        ...
        cons = (NonlinearConstraint(self.con1, 0, np.inf), {"type": "ineq", "fun": self.con2})
        sol = minimize(
            self.fun,
            self.x0,
            method="cobyla",
        ...
        assert_array_almost_equal(
            ...
            callback.last_x,
            decimal=5,
            err_msg="Last design vector sent to the callback is not equal to" " returned value.",
        )
    ...
    def test_minimize_constraint_violation(self):
        ...
        cons = ({"type": "ineq", "fun": lambda x: 4 - x}, {"type": "ineq", "fun": lambda x: x - 5})
        sol = minimize(lambda x: x, [0], method="cobyla", constraints=cons, options={"catol": 0.6})
        assert sol.maxcv > 0.1
        assert sol.success
        sol = minimize(lambda x: x, [0], method="cobyla", constraints=cons, options={"catol": 0.4})
        assert sol.maxcv > 0.1
        assert not sol.success

    def test_f_target(self):
        f_target = 250
        sol = minimize(lambda x: x**2, [500], method="cobyla", options={"f_target": f_target})
        assert sol.status == 1
        assert sol.success
