...
def least_squares(
    ...
    if J0.shape != (m, n):
        raise ValueError(
            f"The return value of `jac` has wrong shape: expected {(m, n)}, " f"actual {J0.shape}."
        )
    ...
    if method == "lm":
        if callback is not None:
            warn("Callback function specified, but not supported with `lm` method.", stacklevel=2)
        result = call_minpack(
            vector_fun.fun, x0, vector_fun.jac, ftol, xtol, gtol, max_nfev, x_scale, jac_method=jac
        )
