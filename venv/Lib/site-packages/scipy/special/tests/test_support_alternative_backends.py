...
def _skip_or_tweak_alternative_backends(xp, nfo, dtypes, int_only):
    ...
    if (
        ...
        or (
            is_jax(xp)
            and f_name
            in {"stdtr", "ndtr", "ndtri", "log_ndtr", "hyp1f1", "hyp2f1", "spence", "kl_div"}
        )
    ):
    ...
    if any("float" in dtype for dtype in dtypes) and (
        (is_torch(xp) and f_name in ("rel_entr", "xlogy", "polygamma", "zeta", "xlog1py"))
        or (
            is_jax(xp)
            and f_name
            in (
...
@pytest.mark.parametrize("func,nfo", [make_xp_pytest_param(i.wrapper, i) for i in _special_funcs])
def test_support_alternative_backends(xp, func, nfo, base_dtype, shapes):
    ...
    positive_only, dtypes = _skip_or_tweak_alternative_backends(xp, nfo, dtypes, int_only)
    ...
    no_shape = [cond1 or cond2 for cond1, cond2 in zip(python_int_only, scalar_or_0d_only)]

    shapes = [shape if not cond else None for shape, cond in zip(shapes, no_shape)]

    for dtype, dtype_np, shape, needs_python_int in zip(dtypes, dtypes_np, shapes, python_int_only):
    ...
    args_np = [np.abs(arg) if cond else arg for arg, cond in zip(args_np, positive_only)]
    ...
    if is_torch(xp) and func.__name__ == "j1":
        ...
        atol = 1e-7
        rtol = 1e-5
    xp_assert_close(res, xp.asarray(ref), rtol=rtol, atol=atol, check_0d=nfo.produces_0d)


@pytest.mark.parametrize(
    "func, nfo", [make_xp_pytest_param(i.wrapper, i) for i in _special_funcs if i.n_args >= 2]
)
@pytest.mark.filterwarnings("ignore:invalid value encountered:RuntimeWarning:dask")
def test_support_alternative_backends_mismatched_dtypes(xp, func, nfo):
    ...
    args_xp = [xp.asarray(arg, dtype=dtype_xp) for arg, dtype_xp in zip(args_np, dtypes_xp)]
    args_np = [np.asarray(arg, dtype=dtype_np) for arg, dtype_np in zip(args_np, dtypes_np)]
...
@pytest.mark.filterwarnings("ignore:overflow encountered:RuntimeWarning:array_api_strict")
def test_support_alternative_backends_hypothesis(xp, func, nfo, data):
    ...
    args_np = [data.draw(npst.arrays(dtype_np, shape, elements=elements)) for shape in shapes]
...
def test_ufunc_kwargs(func, n_args, int_only, is_ufunc):
    ...
    # out=
    args = [
        np.asarray([0.1, 0.2]) if not needs_int else np.asarray([1, 2]) for needs_int in int_only
    ]
    out = np.empty(2)
    y = func(*args, out=out)
