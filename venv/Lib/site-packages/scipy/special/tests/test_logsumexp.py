...
class TestLogSumExp:
    ...
    def test_dtypes_ab(self, dtype_a, dtype_b, xp):
        ...
        if is_array_api_strict(xp):
            ...
            xp_float_dtypes = [
                dtype for dtype in [xp_dtype_a, xp_dtype_b] if not xp.isdtype(dtype, "integral")
            ]
    ...
    @pytest.mark.parametrize("a, b, sign_ref", [([np.inf], None, 1.0), ([np.inf], [-1.0], -1.0)])
...
class TestSoftmax:
    def test_softmax_fixtures(self, xp):
        ...
        xp_assert_close(softmax(xp.asarray([1.0, 1.0])), xp.asarray([0.5, 0.5]), rtol=1e-13)
        xp_assert_close(
            softmax(xp.asarray([0.0, 1.0])), xp.asarray([1.0, np.e]) / (1 + np.e), rtol=1e-13
        )
        ...
        # the shape.
        xp_assert_close(softmax(xp.reshape(x, (2, 2))), xp.reshape(expected, (2, 2)), rtol=1e-13)
    ...
    def test_softmax_multi_axes(self, xp):
        ...
        xp_assert_close(softmax(x3d, axis=(1, 2)), xp.reshape(expected, (2, 2, 2)), rtol=1e-13)

    @pytest.mark.xfail_xp_backends("array_api_strict", reason="int->float promotion")
    def test_softmax_int_array(self, xp):
        xp_assert_close(
            softmax(xp.asarray([1000, 0, 0, 0])), xp.asarray([1.0, 0.0, 0.0, 0.0]), rtol=1e-13
        )
    ...
    def test_softmax_array_like(self):
        xp_assert_close(softmax([1000, 0, 0, 0]), np.asarray([1.0, 0.0, 0.0, 0.0]), rtol=1e-13)
...
class TestLogSoftmax:
    def test_log_softmax_basic(self, xp):
        xp_assert_close(
            log_softmax(xp.asarray([1000.0, 1.0])), xp.asarray([0.0, -999.0]), rtol=1e-13
        )

    @pytest.mark.xfail_xp_backends("array_api_strict", reason="int->float promotion")
    def test_log_softmax_int_array(self, xp):
        xp_assert_close(log_softmax(xp.asarray([1000, 1])), xp.asarray([0.0, -999.0]), rtol=1e-13)
    ...
    def test_axes(self, axis_2d, expected_2d, xp):
        ...
        xp_assert_close(
            log_softmax(x, axis=axis_2d), xp.asarray(expected_2d, dtype=x.dtype), rtol=1e-13
        )
