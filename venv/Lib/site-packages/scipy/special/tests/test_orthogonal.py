...
class TestGegenbauer:
    ...
    def test_gegenbauer(self):
        ...
        assert_allclose(
            Ca3.c, array([4 * sc.poch(a, 3), 0, -6 * a * (a + 1), 0]) / 3.0, atol=1.5e-11, rtol=0
        )
        assert_allclose(
            Ca4.c,
            array([4 * sc.poch(a, 4), 0, -12 * sc.poch(a, 3), 0, 3 * a * (a + 1)]) / 6.0,
            atol=1.5e-11,
            rtol=0,
        )
        assert_allclose(
    ...
    def test_call(self):
        ...
        with np.errstate(all="ignore"):
            for pstr in poly:
                p = eval(pstr)
                assert_allclose(
                    p(0.315), np.poly1d(p.coef)(0.315), atol=1.5e-7, rtol=0, err_msg=pstr
                )
