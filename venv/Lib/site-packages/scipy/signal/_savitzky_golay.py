...
def savgol_coeffs(
    window_length, polyorder, deriv=0, delta=1.0, pos=None, use="conv", *, xp=None, device=None
):
    ...
    order = xp.reshape(xp.arange(polyorder + 1, dtype=xp.float64, device=device), (-1, 1))
    A = x**order
...
def _polyder(p, m, *, xp):
    ...
    if m == 0:
        ...
        if n <= m:
            ...
            for k in range(m):
                rng = xp.arange(n - k - 1, m - k - 1, -1, dtype=p.dtype, device=xp_device(p))
...
def _fit_edges_polyfit(x, window_length, polyorder, deriv, delta, axis, y):
    ...
    n = x.shape[axis]
    y = _fit_edge(x, n - window_length, n, n - halflen, n, axis, polyorder, deriv, delta, y)
