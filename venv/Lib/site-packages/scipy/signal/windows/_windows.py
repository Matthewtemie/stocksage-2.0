...
def parzen(M, sym=True, *, xp=None, device=None):
    ...
    n = xp.arange(-(M - 1) / 2.0, (M - 1) / 2.0 + 0.5, 1.0, dtype=xp.float64, device=device)
    w = xp.where(
        abs(n) <= (M - 1) / 4.0,
...
def nuttall(M, sym=True, *, xp=None, device=None):
    ...
    """
    xp = _namespace(xp)
    a = xp.asarray([0.3635819, 0.4891775, 0.1365995, 0.0106411], dtype=xp.float64, device=device)
    device = xp_device(a)
    return _general_cosine_impl(M, a, xp, device, sym=sym)
...
def blackmanharris(M, sym=True, *, xp=None, device=None):
    ...
    """
    xp = _namespace(xp)
    a = xp.asarray([0.35875, 0.48829, 0.14128, 0.01168], dtype=xp.float64, device=device)
    device = xp_device(a)
    return _general_cosine_impl(M, a, xp, device, sym=sym)
...
@xp_capabilities(
    skip_backends=(("jax.numpy", "item assignment"), ("dask.array", "data-dependent output shapes"))
)
...
def get_window(window, Nx, fftbins=True, *, xp=None, device=None):
    ...
    if func is general_cosine:
        if not (xp is None and device is None):
            raise ValueError(
                "'general_cosine' does not accept the parameters xp " + "and device not being None!"
            )
