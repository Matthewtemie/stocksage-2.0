...
class TestBartlett:
    ...
    def test_basic(self, xp):
        xp_assert_close(
            windows.bartlett(6, xp=xp), xp.asarray([0, 0.4, 0.8, 0.8, 0.4, 0], dtype=xp.float64)
        )
        xp_assert_close(
...
class TestBlackmanHarris:
    ...
    def test_basic(self, xp):
        xp_assert_close(
            ...
            xp.asarray([6.0e-05, 0.055645, 0.520575, 1.0, 0.520575, 0.055645], dtype=xp.float64),
        )
        xp_assert_close(
        ...
        xp_assert_close(
            ...
            xp.asarray(
                [6.0e-05, 0.055645, 0.520575, 1.0, 0.520575, 0.055645, 6.0e-05], dtype=xp.float64
            ),
        )
...
class TestTaylor:
    ...
    def test_normalized(self, xp):
        ...
        normalization.
        """
        xp_assert_close(windows.taylor(1, 2, 15, xp=xp), xp.asarray([1.0], dtype=xp.float64))
        xp_assert_close(
            windows.taylor(5, 2, 15, xp=xp),
            xp.asarray([0.75803341, 0.90757699, 1.0, 0.90757699, 0.75803341], dtype=xp.float64),
        )
        xp_assert_close(
    ...
    def test_non_normalized(self, xp):
        ...
        xp_assert_close(
            ...
            xp.asarray(
                [0.87508054, 1.04771499, 1.15440894, 1.04771499, 0.87508054], dtype=xp.float64
            ),
        )
        xp_assert_close(
            windows.taylor(6, 2, 15, norm=False, xp=xp),
            xp.asarray(
                [0.86627793, 1.0, 1.13372207, 1.13372207, 1.0, 0.86627793], dtype=xp.float64
            ),
        )
...
class TestBoxcar:
    ...
    def test_basic(self, xp):
        ...
        xp_assert_close(
            windows.boxcar(7, xp=xp), xp.asarray([1.0, 1, 1, 1, 1, 1, 1], dtype=xp.float64)
        )
        xp_assert_close(
            windows.boxcar(6, False, xp=xp), xp.asarray([1.0, 1, 1, 1, 1, 1], dtype=xp.float64)
        )
...
class TestChebWin:
    ...
    def test_cheb_odd_low_attenuation(self, xp):
        cheb_odd_low_at_true = xp.asarray(
            [1.000000, 0.519052, 0.586405, 0.610151, 0.586405, 0.519052, 1.000000], dtype=xp.float64
        )
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", "This window is not suitable", UserWarning)
...
class TestGeneralCosine:
    ...
    def test_basic(self, xp):
        ...
        xp_assert_close(
            windows.general_cosine(5, a), xp.asarray([0.4, 0.3, 1, 0.3, 0.4], dtype=xp.float64)
        )
...
class TestGeneralHamming:
    ...
    def test_basic(self, xp):
        ...
        xp_assert_close(
            ...
            xp.asarray(
                [0.5, 0.6727457514, 0.9522542486, 0.9522542486, 0.6727457514], dtype=xp.float64
            ),
        )
        xp_assert_close(
            windows.general_hamming(6, 0.75, sym=True, xp=xp),
            xp.asarray(
                [0.5, 0.6727457514, 0.9522542486, 0.9522542486, 0.6727457514, 0.5], dtype=xp.float64
            ),
        )
...
class TestKaiserBesselDerived:
    ...
    def test_basic(self, xp):
        ...
        xp_assert_close(
            ...
            xp.asarray([0.436168993154, 0.707106781187, 0.899864772847], dtype=xp.float64),
        )
...
class TestNuttall:
    ...
    def test_basic(self, xp):
        xp_assert_close(
            ...
            xp.asarray(
                [0.0003628, 0.0613345, 0.5292298, 1.0, 0.5292298, 0.0613345], dtype=xp.float64
            ),
        )
        xp_assert_close(
    ...
    def test_unity(self, xp):
        ...
        for M in range(1, 21):
            ...
            if M > 2:
                ...
                xp_assert_equal(np.isclose(win_sub, 1.0).sum(), expected, err_msg=f"{win_sub}")
...
class TestLanczos:
    ...
    def test_basic(self, xp):
        ...
        xp_assert_close(
            ...
            xp.asarray(
                [0.0, 0.413496672, 0.826993343, 1.0, 0.826993343, 0.413496672], dtype=xp.float64
            ),
            atol=1e-9,
        )
        xp_assert_close(
            ...
            xp.asarray(
                [0.0, 0.504551152, 0.935489284, 0.935489284, 0.504551152, 0.0], dtype=xp.float64
            ),
            atol=1e-9,
        )
        xp_assert_close(
...
class TestGetWindow:
    ...
    def test_cheb_odd(self, xp):
        ...
        assert_array_almost_equal(w, xp.asarray(cheb_odd_true, dtype=xp.float64), decimal=4)
    ...
    def test_general_cosine(self, xp):
        ...
        xp_assert_close(
            get_window(("general_cosine", xp.asarray([0.5, 0.3, 0.2])), 4, fftbins=False),
    ...
    def test_general_hamming(self, xp):
        xp_assert_close(
            ...
            xp.asarray([0.4, 0.6072949, 0.9427051, 0.9427051, 0.6072949], dtype=xp.float64),
        )
        xp_assert_close(
...
def test_windowfunc_basics(window, window_name, params, xp):
    ...
    with warnings.catch_warnings():
        ...
        xp_assert_equal(window(0, *params, sym=True, xp=xp), xp.asarray([], dtype=xp.float64))
        xp_assert_equal(window(0, *params, sym=False, xp=xp), xp.asarray([], dtype=xp.float64))
        xp_assert_equal(window(1, *params, sym=True, xp=xp), xp.asarray([1.0], dtype=xp.float64))
        xp_assert_equal(window(1, *params, sym=False, xp=xp), xp.asarray([1.0], dtype=xp.float64))
...
@pytest.mark.parametrize(
    "window,winstr", [make_xp_pytest_param(_WIN_FUNCS[winstr][0], winstr) for winstr in _winstr]
)
@make_xp_test_case(get_window)
