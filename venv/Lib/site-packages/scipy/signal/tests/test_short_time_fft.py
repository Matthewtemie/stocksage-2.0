...
def test_closest_STFT_dual_window_STFT_roundtrip_complex(scaled):
    ...
    SFT = ShortTimeFFT(
        win, hop=hop, dual_win=dual, fs=1, fft_mode="twosided", scale_to=None, phase_shift=None
    )
...
def test_closest_STFT_dual_window_cola(win_name, nperseg, noverlap, scale_fac):
    ...
    xp_assert_close(s, scale_fac, atol=res * 10, err_msg=f"Scale factor off by {s/scale_fac}")
    xp_assert_close(
        d_s,
...
def test_from_win_equals_dual_unitary():
    ...
    assert sum(x * y) == 0
    xp_assert_close(
        np.sum(Sxx), np.sum(x**2, dtype=Sxx.dtype), atol=atol, err_msg="Energies do not match!"
    )
    xp_assert_close(np.sum(Sxy), 0.0j, atol=atol, err_msg="STFT scalar product of Sx and Sy not 0!")
    xp_assert_close(
        SFT.dual_win, SFT.win * SFT.m_num, atol=atol, err_msg="Wrong factor for dual_win/win!"
    )
