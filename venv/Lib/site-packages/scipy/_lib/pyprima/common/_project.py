...
def _project(x0, lb, ub, constraints):
    ...
    if len(lb_c.shape) != 1 or lb_c.size != lenx0:
        raise ValueError(
            "{}: UNEXPECTED ERROR: the size of lb is inconsistent with x0.".format(invoker)
        )
    ...
    if len(ub_c.shape) != 1 or ub_c.size != lenx0:
        raise ValueError(
            "{}: UNEXPECTED ERROR: the size of ub is inconsistent with x0.".format(invoker)
        )
    ...
    if (
        ...
        or not (
            isinstance(constraints["linear"], LinearConstraint) or constraints["linear"] is None
        )
    ):
    ...
    if constraints["linear"] is None:
        ...
        all(np.less_equal(np.abs(constraints["linear"].ub - constraints["linear"].lb), eps))
