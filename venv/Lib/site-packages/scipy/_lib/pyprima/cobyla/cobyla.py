...
def cobyla(
    ...
    if DEBUGGING:
        ...
        assert present(Aeq) == present(beq), f"{srname} Aeq and Beq are both present or both absent"
        if present(Aeq):
    ...
    maxhist = (
        maxhist if present(maxhist) else max(maxfun, num_vars + 2, MAXFUN_DIM_DEFAULT * num_vars)
    )
