...
def is_named_function_like_object(obj):
    return not isinstance(obj, ModuleType | type) and callable(obj) and hasattr(obj, "__name__")
...
def calculate_table_statistics(
    ...
    return {
        key: (dict(value), key not in missing_xp_capabilities) for key, value in counter.items()
    }
