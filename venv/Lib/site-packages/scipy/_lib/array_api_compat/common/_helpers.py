...
def _cls_to_namespace(
    ...
    if _issubclass_fast(cls_, "numpy", "ndarray") or _issubclass_fast(cls_, "numpy", "generic"):
...
def array_namespace(
    *xs: Array | complex | None,
    ...
    for x in xs:
        ...
        if info is _ClsToXPInfo.MAYBE_JAX_ZERO_GRADIENT and _is_jax_zero_gradient_array(x):
    ...
    try:
        ...
        if not namespaces:
            raise TypeError("array_namespace requires at least one non-scalar array input")
