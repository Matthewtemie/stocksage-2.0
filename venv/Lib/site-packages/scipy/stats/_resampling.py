...
def _percentile_of_score(a, score, axis, xp):
    ...
    """
    B = a.shape[axis]
    nonzeros = xp.count_nonzero(a < score, axis=axis) + xp.count_nonzero(a <= score, axis=axis)
...
def _bca_interval(data, statistic, axis, alpha, theta_hat_b, batch, xp):
    ...
    percentile = xp.asarray(percentile, dtype=theta_hat.dtype, device=xp_device(theta_hat))
    ...
    theta_hat_j_dot = [xp.mean(theta_hat_i, axis=-1, keepdims=True) for theta_hat_i in theta_hat_ji]
...
def bootstrap(
    ...
    *,
    ...
    for k in range(0, n_resamples, batch_nominal):
        ...
        resampled_data = []
        for sample in data:
            resample = _bootstrap_resample(sample, n_resamples=batch_actual, rng=rng, xp=xp)
    ...
    if method == "bca":
        interval = _bca_interval(
            data, statistic, axis=-1, alpha=alpha, theta_hat_b=theta_hat_b, batch=batch, xp=xp
        )[:2]
    else:
        alpha = xp.asarray(alpha, dtype=theta_hat_b.dtype, device=xp_device(theta_hat_b))
...
def power(
    ...
    *alternative* hypothesis and calculate the power.
    ...
    (rvs, test, nobs, significance, vectorized, n_resamples, batch, args, kwds, shape, xp) = tmp
...
def _calculate_null_samples(data, statistic, n_permutations, batch, rng=None, *, xp):
    ...
    data = [data[i, ...] for i in range(data.shape[0])]
    return _calculate_null_pairings(data, statistic_wrapped, n_permutations, batch, rng, xp=xp)
...
def permutation_test(
    ...
    *difference* between two paired elements. Accordingly, if ``data`` contains
    ...
    null_distribution, n_resamples, exact_test = calculate_null(*null_calculator_args, xp=xp)
    ...
    # theoretically equal values in the null distribution
    eps = (
        0 if not xp.isdtype(observed.dtype, "real floating") else xp.finfo(observed.dtype).eps * 100
    )
    gamma = xp.abs(eps * observed)
