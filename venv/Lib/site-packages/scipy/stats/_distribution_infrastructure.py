...
def _set_invalid_nan(f):
    ...
    def filtered(self, x, *args, **kwargs):
        ...
        keep_low_endpoint = discrete and method_name in {"_cdf1", "_logcdf1", "_ccdf1", "_logccdf1"}
        ...
        if discrete and method_name in replace_non_integral:
            ...
            any_non_integral = (
                mask_non_integral if mask_non_integral.shape == () else np.any(mask_non_integral)
            )
        ...
        if res.shape != shape:  # faster to check first
            ...
            res_needs_copy = res_needs_copy or any_invalid or any_endpoint or any_non_integral
...
class UnivariateDistribution(_ProbabilityDistribution):
    ...
    def _mode_optimization(self, xatol=None, **params):
        ...
        res = _chandrupatla_minimize(f, res_b.xl, res_b.xm, res_b.xr, args=args, xatol=xatol)
    ...
    def _logcdf2_quadrature(self, x, y, **params):
        logres = self._quadrature(self._logpxf_dispatch, limits=(x, y), log=True, params=params)
        return logres
    ...
    def _logcdf_quadrature(self, x, **params):
        ...
        return self._quadrature(self._logpxf_dispatch, limits=(a, x), params=params, log=True)
    ...
    def _logccdf_quadrature(self, x, **params):
        ...
        return self._quadrature(self._logpxf_dispatch, limits=(x, b), params=params, log=True)
    ...
    def sample(self, shape=(), *, method=None, rng=None):
        ...
        res = self._sample_dispatch(full_shape, method=method, rng=rng, **self._parameters)
...
class DiscreteDistribution(UnivariateDistribution):
    ...
    def _icdf_inversion(self, x, **params):
        return self._base_discrete_inversion(x, self._cdf_dispatch, np.greater_equal, **params)

    def _ilogcdf_inversion(self, x, **params):
        return self._base_discrete_inversion(x, self._logcdf_dispatch, np.greater_equal, **params)

    def _iccdf_inversion(self, x, **params):
        return self._base_discrete_inversion(x, self._ccdf_dispatch, np.less_equal, **params)

    def _ilogccdf_inversion(self, x, **params):
        return self._base_discrete_inversion(x, self._logccdf_dispatch, np.less_equal, **params)
...
def _make_distribution_rv_generic(dist):
    ...
    for shape_info in dist._shape_info():
        domain = _RealInterval(endpoints=shape_info.endpoints, inclusive=shape_info.inclusive)
    ...
    docs = [
        f"This class represents `scipy.stats.{dist.name}` as a subclass of " f"`{new_class}`.",
...
def _make_distribution_custom(dist):
    dist_parameters = dist.parameters if isinstance(dist.parameters, tuple) else (dist.parameters,)
    parameterizations = []
    ...
    if hasattr(dist, "process_parameters"):
        setattr(CustomDistribution, "_process_parameters", getattr(dist, "process_parameters"))

    support_etc = _combine_docs(CustomDistribution, include_examples=False).lstrip()
    docs = [
        f"This class represents `{repr_str}` as a subclass of " "`ContinuousDistribution`.",
        f"The PDF of the distribution is defined {support_etc}",
    ]
    CustomDistribution.__doc__ = "\n".join(docs)
