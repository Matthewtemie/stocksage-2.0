...
def test_zscore(fun, axis, xp):
    mxp, marrays, narrays = get_arrays(2, xp=xp) if fun == stats.zmap else get_arrays(1, xp=xp)
...
def test_power_divergence_chisquare(lambda_, ddof, axis, xp):
    ...
    normalize = np.nansum(narrays[1] * ~common_mask, axis=axis, keepdims=True) / np.nansum(
        narrays[0] * ~common_mask, axis=axis, keepdims=True
    )
...
@skip_backend("torch", reason="array-api-compat#242")
@skip_backend("cupy", reason="special functions won't work")
@pytest.mark.parametrize("method", ["fisher", "pearson", "mudholkar_george", "tippett", "stouffer"])
@pytest.mark.parametrize("axis", [0, 1, None])
def test_combine_pvalues(method, axis, xp):
    ...
    ref = stats.combine_pvalues(narrays[0], weights=narrays[1], nan_policy="omit", **kwargs)
...
def test_length_nonmasked_marray_iterable_axis_raises():
    ...
    with pytest.raises(
        NotImplementedError, match="`axis` must be an integer or None for use with `MArray`"
    ):
...
def test_directional_stats(xp):
    ...
    xp_assert_close(res.mean_resultant_length.data, xp.asarray(ref.mean_resultant_length))
...
@skip_backend("cupy", reason="special functions won't work")
@pytest.mark.parametrize(
    "f", [make_xp_pytest_param(stats.pearsonr), make_xp_pytest_param(stats.pointbiserialr)]
)
