...
class TestMatrixT:
    ...
    def test_bad_input(self):
        ...
        with pytest.raises(ValueError, match="Array `row_spread` must be a scalar or a 2D array."):
        ...
        with pytest.raises(ValueError, match="Array `col_spread` must be a scalar or a 2D array."):
        ...
        with pytest.raises(
            ValueError,
            match="Arrays `mean` and `row_spread` must have the same number " "of rows.",
        ):
            matrix_t(mean=M, row_spread=V)

        with pytest.raises(
            ValueError,
            match="Arrays `mean` and `col_spread` must have the same number " "of columns.",
        ):
        ...
        with pytest.raises(
            ValueError,
            match="The shape of array `X` is not conformal with " "the distribution parameters.",
        ):
    ...
    def test_default_inputs(self):
        ...
        assert_equal(matrix_t.rvs(row_spread=U, col_spread=V).shape, (num_rows, num_cols))
    ...
    def test_covariance_expansion(self):
        ...
        assert_equal(matrix_t(mean=M, row_spread=Uv, col_spread=Vv, df=df).row_spread, 0.2 * Ir)
        assert_equal(matrix_t(mean=M, row_spread=Uv, col_spread=Vv, df=df).col_spread, 0.1 * Ic)
        assert_equal(matrix_t(mean=M, row_spread=Us, col_spread=Vs, df=df).row_spread, 0.2 * Ir)
        assert_equal(matrix_t(mean=M, row_spread=Us, col_spread=Vs, df=df).col_spread, 0.1 * Ic)
    ...
    def test_frozen_matrix_t(self, i, j):
        ...
        rvs1 = frozen.rvs(random_state=1234)
        rvs2 = matrix_t.rvs(mean=M, row_spread=U, col_spread=V, df=df, random_state=1234)
        assert_equal(rvs1, rvs2)
...
class TestSpecialOrthoGroup:
    ...
    def test_haar(self):
        ...
        ks_prob = 0.05
        xs = special_ortho_group.rvs(dim, size=samples, random_state=np.random.default_rng(513))
        ...
        for dim in range(2, 6):
            ...
            def generate_test_statistics(rvs, N=1000, eps=1e-10):
                stats = np.array(
                    [
                        np.sum(
                            (rvs(dim=dim, random_state=rng) - rvs(dim=dim, random_state=rng)) ** 2
                        )
                        for _ in range(N)
                    ]
                )
...
class TestUnitaryGroup:
    ...
    def test_haar(self):
        ...
        for dim in (1, 5):
            ...
            xs = unitary_group.rvs(dim, size=samples, random_state=np.random.default_rng(514))
