...
def test_kde_1d_weighted():
    ...
    pdf = stats.norm.pdf
    assert_allclose(
        gkde(xx), np.sum(pdf(xx[:, None], loc=loc, scale=scale) * gkde.weights, axis=-1), rtol=5e-14
    )
...
@pytest.mark.parametrize("n_basesample", [20, pytest.param(500, marks=[pytest.mark.xslow])])
def test_kde_2d(n_basesample):
    ...
    assert_allclose(gkde(xx.T), pdf(arg, cov=gkde.covariance).sum(axis=-1) / gkde.n, rtol=5e-14)
...
def test_kde_2d_weighted(n_basesample):
    ...
    pdf = stats.multivariate_normal.pdf
    assert_allclose(
        gkde(xx.T), np.sum(pdf(arg, cov=gkde.covariance) * gkde.weights, axis=-1), rtol=5e-14
    )
    ...
    assert_allclose(
        gkde.integrate_box(lo, hi, rng=rng),
        np.sum(cdf(hi_, lower_limit=lo_, cov=gkde.covariance, rng=rng) * gkde.weights, axis=-1),
        rtol=5e-6,
    )
