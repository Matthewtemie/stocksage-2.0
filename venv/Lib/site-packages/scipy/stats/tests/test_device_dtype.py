...
def get_arrays(n_arrays, *, dtype=np.float64, xp=np, shape=(30,), device=None, seed=84912165484321):
...
@skip_xp_backends("array_api_strict", reason="special functions don't work with 'device1'")
...
def test_hypothesis_tests(f_name, dtype, xp, devices):
    ...
    for device in devices:
        ...
        n = (
            2
            if f_name
            in {"ttest_1samp", "ttest_rel", "ttest_ind", "bartlett", "pearsonr", "chisquare"}
            else 1
        )
...
@skip_xp_backends("array_api_strict", reason="special functions don't work with 'device1'")
@skip_xp_backends(cpu_only=True, exceptions=["cupy", "jax.numpy"])
@pytest.mark.parametrize("method", ["fisher", "pearson", "tippett", "stouffer", "mudholkar_george"])
@pytest.mark.parametrize("dtype", dtypes)
