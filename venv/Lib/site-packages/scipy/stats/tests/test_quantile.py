...
def quantile_reference_last_axis(x, p, nan_policy, method):
    ...
    if method == "harrell-davis":
        ...
        res = np.full_like(p, x[0]) if x.size == 1 else stats.mstats.hdquantiles(x, p).data
...
class TestQuantile:
    ...
    @skip_xp_backends(cpu_only=True, reason="PyTorch doesn't have `betainc`.", exceptions=["cupy"])
    ...
    def test_against_reference(self, method, shape_x, shape_p, axis, weights, xp):
        ...
        if weights and (
            method.startswith("_") or method.startswith("round") or method == "harrell-davis"
        ):
    ...
    @skip_xp_backends(
        cpu_only=True, reason="PyTorch doesn't have `betainc`.", exceptions=["cupy", "jax.numpy"]
    )
    ...
    @pytest.mark.parametrize("weights", [False, True])
    def test_against_reference_2(self, axis, keepdims, nan_policy, dtype, method, weights, xp):
        ...
        kwargs = dict(axis=axis, keepdims=keepdims, nan_policy=nan_policy, method=method)
