...
class TestBootstrap:
    ...
    def test_bootstrap_against_theory(self, method, xp):
        ...
        config = dict(
            data=(xp.asarray(data),), statistic=xp.mean, n_resamples=5000, method=method, rng=rng
        )
    ...
    def test_bootstrap_against_R(self, method, ref_low, ref_high, xp):
        ...
        # print(result)
        x = xp.asarray([10, 12, 12.5, 12.5, 13.9, 15, 21, 22, 23, 34, 50, 81, 89, 121, 134, 213])
    ...
    def test_multisample_BCa_against_R(self, xp):
        ...
        x = xp.asarray(
            [0.75859206, 0.5910282, -0.4419409, -0.36654601, 0.34955357, -1.38835871, 0.76735821]
        )
        y = xp.asarray(
            [
                1.41186073,
    ...
    def test_bootstrap_against_itself_1samp(self, method, expected, xp):
        ...
        pvalue = stats.binomtest(int(ci_contains_true), n_replications, confidence_level).pvalue
        assert pvalue > 0.1
    ...
    def test_bootstrap_vectorized_3samp(self, method, axis, dtype, xp):
        ...
        xp_assert_close(
            res1.confidence_interval.low, xp.asarray(res2.confidence_interval.low), rtol=rtol
        )
        xp_assert_close(
            res1.confidence_interval.high, xp.asarray(res2.confidence_interval.high), rtol=rtol
        )
    ...
    def test_bootstrap_vectorized_1samp(self, method, axis, xp):
        ...
        xp_assert_close(res1.confidence_interval.low, xp.asarray(res2.confidence_interval.low))
        xp_assert_close(res1.confidence_interval.high, xp.asarray(res2.confidence_interval.high))
    ...
    def test_bootstrap_gh15678(self, method, xp):
        ...
        xp_assert_close(res.confidence_interval.low, xp.asarray(ref.confidence_interval.low))
        xp_assert_close(res.confidence_interval.high, xp.asarray(ref.confidence_interval.high))
    ...
    def test_bootstrap_min(self, xp):
        ...
        res = bootstrap(
            (data,), xp.min, method="BCa", n_resamples=100, rng=np.random.default_rng(3942)
        )
        xp_assert_equal(res.confidence_interval.low, xp.asarray(true_min))
        res2 = bootstrap(
            (-data,), xp.max, method="BCa", n_resamples=100, rng=np.random.default_rng(3942)
        )
    ...
    def test_re_bootstrap(self, additional_resamples, xp):
        ...
        rng = np.random.default_rng(296689032789913033)
        ref = stats.bootstrap(
            (xp.asarray(x),), xp.mean, n_resamples=n3, rng=rng, confidence_level=0.90, method="BCa"
        )

        xp_assert_close(
            res.confidence_interval.low, xp.asarray(ref.confidence_interval.low), rtol=1e-14
        )
        xp_assert_close(
            res.confidence_interval.high, xp.asarray(ref.confidence_interval.high), rtol=1e-14
        )
    ...
    def test_bootstrap_alternative(self, method, xp):
        ...
        xp_assert_close(l.confidence_interval.high, t.confidence_interval.high, rtol=1e-14)
        xp_assert_close(g.confidence_interval.low, t.confidence_interval.low, rtol=1e-14)
    ...
    @pytest.mark.skip_xp_backends("array_api_strict", reason="Test uses ... + fancy indexing")
    ...
    def test_vector_valued_statistic(self, method, xp):
        ...
        def statistic(data, axis):
            return xp.stack([xp.mean(data, axis=axis), xp.std(data, axis=axis, correction=1)])
        ...
        counts = xp.count_nonzero(
            (res.confidence_interval.low.T < params) & (res.confidence_interval.high.T > params),
            axis=0,
        )
...
class TestPower:
    ...
    def test_vectorization(self, xp):
        ...
        def test(x, alternative, axis=-1):
            ...
            popmeans_expanded = xpx.expand_dims(popmeans, axis=tuple(range(1, x.ndim + 1)))
        ...
        res = stats.power(
            test, self.xp_normal(rng, xp=xp), nx, significance=significance, kwargs=kwargs
        )
            ...
            for i in range(nx.shape[0]):
                ...
                for alternative_i in kwargs["alternative"]:
                    for j in range(popmeans.shape[0]):
                        ...
                        tmp = stats.power(
                            test2, self.xp_normal(rng, xp=xp), nx_i, significance=significance_i
                        )
    ...
    @pytest.mark.skip_xp_backends(cpu_only=True, exceptions=["cupy", "jax.numpy", "dask.array"])
...
class TestPermutationTest:
    ...
    def test_against_wilcoxon(self, alternative, xp):
        ...
        if alternative != "two-sided":
            xp_assert_close(res2.statistic, xp.asarray(expected.statistic), rtol=self.rtol)
    ...
    def test_against_kendalltau(self, xp):
        ...
        x = xp.asarray(x)
        res = permutation_test(
            (x,), statistic, permutation_type="pairings", n_resamples=np.inf, rng=self.rng
        )
    ...
    def test_with_ties(self, case, xp):
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", "Ties preclude use of exact statistic", UserWarning)
            res = permutation_test((x, y), statistic, n_resamples=np.inf, alternative="less")
            res2 = permutation_test((x, y), statistic, n_resamples=np.inf, alternative="two-sided")
        ...
        Pr_gte_S_mean = (
            xp.astype(
                xp.count_nonzero(xp.abs(res.null_distribution - mean) >= xp.abs(S - mean)), S.dtype
            )
            / n
        )
    ...
    def test_against_spearmanr_in_R(self, alternative, expected_pvalue, xp):
        ...
        x = xp.asarray([1.76405235, 0.40015721, 0.97873798, 2.2408932, 1.86755799, -0.97727788])
        y = xp.asarray([2.71414076, 0.2488, 0.87551913, 2.6514917, 2.01160156, 0.47699563])
