...
def _cdf_cvm(x, n=None, *, xp=None):
    ...
    if n is None:
        ...
        y = xpx.at(y)[sup].set(
            _cdf_cvm_inf(x[sup], xp=xp) * (1 + 1.0 / (12 * n)) + _psi1_mod(x[sup], xp=xp) / n
        )
...
def _pval_cvm_2samp_asymptotic(t, N, nx, ny, k, *, xp):
    ...
    p = xpx.apply_where(
        tn >= 0.003, (tn,), lambda tn: xp.clip(1.0 - _cdf_cvm_inf(tn, xp=xp), 0.0), fill_value=1.0
    )
    return p
...
def cramervonmises_2samp(x, y, method="auto", *, axis=0):
    ...
    u = nx * xp.sum((rx - xp.arange(1, nx + 1, dtype=dtype)) ** 2, axis=-1) + ny * xp.sum(
        (ry - xp.arange(1, ny + 1, dtype=dtype)) ** 2, axis=-1
    )
...
class TukeyHSDResult:
    ...
    def __str__(self):
        ...
        s = "Pairwise Group Comparisons" f" ({self._ci_cl*100:.1f}% Confidence Interval)\n"
