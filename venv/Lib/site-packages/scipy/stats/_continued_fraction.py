...
def _continued_fraction_iv(a, b, args, tolerances, maxiter, log):
    ...
    a0, b0, *args = xp_promote(a0, b0, *args, force_floating=True, broadcast=True, xp=xp)
...
def _continued_fraction(a, b, *, args=(), tolerances=None, maxiter=100, log=False):
    ...
    res_work_pairs = [("status", "status"), ("f", "fn"), ("nit", "nit"), ("nfev", "nfev")]
    ...
    def post_func_eval(n, ab, work):
        ...
        denominator = bn + an * work.Dnm1 if not log else _logaddexp(bn, an + work.Dnm1, xp=xp)
    ...
    def check_termination(work):
        ...
        residual = xp.abs(work.CnDn - 1) if not log else xp.real(_logaddexp(work.CnDn, pij, xp=xp))
        i = residual < work.eps
        work.status[i] = eim._ECONVERGED
        stop[i] = True

        # If function value is NaN, report failure.
        i = ~xp.isfinite(work.fn) if not log else ~(xp.isfinite(work.fn) | (work.fn == -xp.inf))
