...
class burr_gen(rv_continuous):
    ...
    def _pdf(self, x, c, d):
        ...
        output = xpx.apply_where(
            ...
            lambda x_, c_, d_: (c_ * d_ * (x_ ** (-c_ - 1.0)) / ((1 + x_ ** (-c_)) ** (d_ + 1.0))),
        )
    ...
    def _munp(self, n, c, d):
        ...
        return xpx.apply_where((c > n) & (n == n) & (d == d), (n, c, d), __munp, fill_value=np.nan)
...
class burr12_gen(rv_continuous):
    ...
    def _munp(self, n, c, d):
        ...
        return xpx.apply_where(c * d > n, (n, c, d), moment_if_exists, fill_value=np.nan)
...
class chi2_gen(rv_continuous):
    ...
    def _entropy(self, df):
        ...
        return xpx.apply_where(half_df < 125, half_df, regular_formula, asymptotic_formula)
...
class f_gen(rv_continuous):
    ...
    def _stats(self, dfn, dfd):
        ...
        mu = xpx.apply_where(v2 > 2, (v2, v2_2), lambda v2, v2_2: v2 / v2_2, fill_value=np.inf)
...
class genpareto_gen(rv_continuous):
    ...
    def _logpdf(self, x, c):
        return xpx.apply_where(
            (x == x) & (c != 0), (x, c), lambda x, c: -sc.xlog1py(c + 1.0, c * x) / c, fill_value=-x
        )
    ...
    def _logsf(self, x, c):
        return xpx.apply_where(
            (x == x) & (c != 0), (x, c), lambda x, c: -sc.log1p(c * x) / c, fill_value=-x
        )
    ...
    def _stats(self, c, moments="mv"):
        ...
        if "v" in moments:
            v = xpx.apply_where(
                c < 1 / 2, c, lambda xi: 1 / (1 - xi) ** 2 / (1 - 2 * xi), fill_value=np.nan
            )
        ...
        if "k" in moments:
            k = xpx.apply_where(
                c < 1 / 4,
                c,
                lambda xi: 3 * (1 - 2 * xi) * (2 * xi**2 + xi + 3) / (1 - 3 * xi) / (1 - 4 * xi)
                - 3,
                fill_value=np.nan,
            )
...
class genextreme_gen(rv_continuous):
    ...
    def _ppf(self, q, c):
        x = -np.log(-np.log(q))
        return xpx.apply_where(
            (x == x) & (c != 0), (x, c), lambda x, c: -sc.expm1(-c * x) / c, fill_value=x
        )
    ...
    def _stats(self, c):
        ...
        def sk1_eval(c, *args):
            ...
            return xpx.apply_where(c >= -1.0 / 3, (c, *args), sk1_eval_f, fill_value=np.nan)
...
class gengamma_gen(rv_continuous):
    ...
    def _logpdf(self, x, a, c):
        return xpx.apply_where(
            (x != 0) | (c > 0),
            (x, c, a),
            lambda x, c, a: (np.log(abs(c)) + sc.xlogy(c * a - 1, x) - x**c - sc.gammaln(a)),
            fill_value=-np.inf,
        )
...
class invgamma_gen(rv_continuous):
    ...
    def _stats(self, a, moments="mvsk"):
        ...
        if "s" in moments:
            g1 = xpx.apply_where(
                a > 3, a, lambda x: 4.0 * np.sqrt(x - 2.0) / (x - 3.0), fill_value=np.nan
            )
        if "k" in moments:
            g2 = xpx.apply_where(
                a > 4,
...
def _ncx2_log_pdf(x, df, nc):
    ...
    return xpx.apply_where(corr > 0, (res, corr), lambda r, c: r + np.log(c), fill_value=-np.inf)
...
class truncnorm_gen(rv_continuous):
    ...
    def _stats(self, a, b, moments="mv"):
        ...
        def _truncnorm_stats_scalar(a, b, pA, pB):
            ...
            m2 = 1 + np.sum(vals)
            vals = xpx.apply_where(cond, (probs, ab - mu), lambda x, y: x * y, fill_value=0)
            # mu2 = m2 - mu**2, but not as numerically stable as:
            # mu2 = (a-mu)*pA - (b-mu)*pB + 1
            mu2 = 1 + np.sum(vals)
            vals = xpx.apply_where(cond, (probs, ab), lambda x, y: x * y**2, fill_value=0)
            m3 = 2 * m1 + np.sum(vals)
            vals = xpx.apply_where(cond, (probs, ab), lambda x, y: x * y**3, fill_value=0)
            m4 = 3 * m2 + np.sum(vals)
