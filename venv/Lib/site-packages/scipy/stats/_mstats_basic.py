...
def trimmed_stde(a, limits=(0.1, 0.1), inclusive=(1, 1), axis=None):
    ...
    if axis is None:
        ...
        if a.ndim > 2:
            raise ValueError(
                f"Array 'a' must be at most two dimensional, " f"but got a.ndim = {a.ndim}"
            )
...
def stde_median(data, axis=None):
    ...
    if axis is None:
        ...
        if data.ndim > 2:
            raise ValueError(
                f"Array 'data' must be at most two dimensional, " f"but got data.ndim = {data.ndim}"
            )
...
def skewtest(a, axis=0, alternative="two-sided"):
    ...
    if np.min(n) < 8:
        raise ValueError(
            f"skewtest is not valid with less than 8 samples; " f"{np.min(n)} samples were given."
        )
...
def kurtosistest(a, axis=0, alternative="two-sided"):
    ...
    if np.min(n) < 20:
        warnings.warn(
            f"kurtosistest only valid for n>=20 ... continuing " f"anyway, n={np.min(n)}",
            stacklevel=2,
        )
...
def friedmanchisquare(*args):
    ...
    k = len(data)
    if k < 3:
        raise ValueError(f"Less than 3 groups ({k}): the Friedman test " f"is NOT appropriate.")
