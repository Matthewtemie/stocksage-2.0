...
class multivariate_normal_gen(multi_rv_generic):
    ...
    def _process_parameters_psd(self, dim, mean, cov):
        ...
        if cov.ndim == 0:
            ...
            raise ValueError(
                f"Array 'cov' must be at most two-dimensional, " f"but cov.ndim = {cov.ndim}"
            )
    ...
    def _cdf(self, x, mean, cov, maxpts, abseps, releps, lower_limit, rng):
        ...
        def func1d(limits):
            ...
            res = _qauto(
                _qmvn, cov, limits[:n], limits[n:], rng, error=abseps, limit=maxpts, n_batches=10
            )
...
class matrix_t_gen(multi_rv_generic):
    ...
    def __init__(self, seed=None):
        super().__init__(seed)
        self.__doc__ = scipy._lib.doccer.docformat(self.__doc__, matrix_t_docdict_params)
    ...
    def _process_quantiles(self, X, dims):
        ...
        if X.shape[-2:] != dims:
            raise ValueError(
                "The shape of array `X` is not conformal with " "the distribution parameters."
            )
        return X
...
for name in ["logpdf", "pdf", "rvs"]:
    ...
    method_frozen.__doc__ = scipy._lib.doccer.docformat(method.__doc__, matrix_t_docdict_noparams)
    method.__doc__ = scipy._lib.doccer.docformat(method.__doc__, matrix_t_docdict_params)
...
class wishart_gen(multi_rv_generic):
    ...
    def _process_parameters(self, df, scale):
        ...
        if scale.ndim == 0:
            ...
            raise ValueError(
                f"Array 'scale' must be at most two-dimensional, " f"but scale.ndim = {scale.ndim}"
            )
...
class multivariate_t_gen(multi_rv_generic):
    ...
    def logpdf(self, x, loc=None, shape=1, df=1):
        ...
        return self._logpdf(
            x, loc, shape_info.U, shape_info.log_pdet, df, dim, shape_info.rank, cov_object
        )
    ...
    def _process_parameters(self, loc, shape, df):
        ...
        if shape.ndim == 0:
            ...
            raise ValueError(
                f"Array 'cov' must be at most two-dimensional, " f"but cov.ndim = {shape.ndim}"
            )
...
class multivariate_t_frozen(multi_rv_frozen):
    ...
    def marginal(self, dimensions):
        return self._dist.marginal(dimensions, self.loc, self.shape, self.df, self.allow_singular)
