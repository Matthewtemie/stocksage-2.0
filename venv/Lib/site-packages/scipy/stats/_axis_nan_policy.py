...
def _add_reduced_axes(res, reduced_axes, keepdims, xp=np):
    ...
    return (
        [
            xpx.expand_dims(output, axis=reduced_axes) if not isinstance(output, int) else output
            for output in res
        ]
        if keepdims
        else res
    ...
    def axis_nan_policy_decorator(hypotest_fun_in):
        ...
        def axis_nan_policy_wrapper(*args, _no_deco=False, **kwds):
            ...
            if is_lazy_array(temp):
                msg = "Use of `nan_policy` and `keepdims` " "is incompatible with lazy arrays."
            ...
            if axis is None:
                ...
                samples = [
                    xp.moveaxis(sample, axis, tuple(range(-len(axis), 0))) for sample in samples
                ]
                ...
                samples = [
                    xp.reshape(sample, new_shape) for sample, new_shape in zip(samples, new_shapes)
                ]
            ...
            if not is_numpy(xp) and "nan_policy" in kwds:
                msg = (
                    "Use of `nan_policy` is incompatible with multidimensional " "non-NumPy arrays."
                )
                raise NotImplementedError(msg)
