...
class BaseScheduler(metaclass=ABCMeta):
    ...
    def configure(self, gconfig={}, prefix="apscheduler.", **options):
        ...
        if prefix:
            ...
            gconfig = dict(
                (key[prefixlen:], value) for key, value in gconfig.items() if key.startswith(prefix)
            )
    ...
    def remove_all_jobs(self, jobstore=None):
        ...
        with self._jobstores_lock:
            if self.state == STATE_STOPPED:
                if jobstore:
                    self._pending_jobs = [
                        pending for pending in self._pending_jobs if pending[1] != jobstore
                    ]
                else:
                    self._pending_jobs = []
            else:
    ...
    def export_jobs(self, outfile, jobstore=None):
        ...
        if self.state == STATE_STOPPED:
            raise RuntimeError("the scheduler must have been started for job export to work")
        ...
        try:
            ...
            if alias in plugin_container:
                ...
                if not issubclass(plugin_cls, base_class):
                    raise TypeError(f"The {type_} entry point does not point to a {type_} class")
            else:
